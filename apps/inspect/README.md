...
window into a running SpiderByte engine (workspaces, sessions, agents, and the
scoped DI registry).
...
1. Start a kap-server with the debug surface mounted (repo dev scripts do this
   ...
   bearer auth.
2. `pnpm --filter @spiderbyte/inspect dev` — the Vite dev server proxies
   `/api` to the server (`SPIDERBYTE_SERVER_URL`, default `http://127.0.0.1:58627`)
   and auto-discovers running instances
   (`~/.spiderbyte/server/instances`); switch servers from the header dropdown.
