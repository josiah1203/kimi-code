 ...
 * `spiderbyte.bearer.<token>` subprotocol (the only credential channel a
...
const WS_BEARER_PROTOCOL_PREFIX = 'spiderbyte.bearer.';
...
export class GlobalEventsWs {
  ...
  private connect(): void {
    ...
    ws.addEventListener('open', () => {
      ...
      this.send({
        type: 'client_hello',
        id: `spiderbyte-inspect-global-${Date.now().toString(36)}`,
        payload: { client_id: 'spiderbyte-inspect', subscriptions: [] },
      });
