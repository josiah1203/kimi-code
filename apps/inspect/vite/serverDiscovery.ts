 ...
 *   current builds  `<SpiderByte home>/server/instances/<serverId>.json`
 *   pre-registry local builds  `<SpiderByte home>/server/lock`
 * and persists the bearer token at `<SpiderByte home>/server.token` (one token per
 ...
 * `~/.spiderbyte/server.token` by hand — no new exposure beyond the local dev
 * session.
 */
...
/** Mirror of the legacy single-server lock written by pre-registry local builds. */
...
/** home resolution per request: `SPIDERBYTE_HOME` env, else `~/.spiderbyte`. */
export function resolveSpiderByteHomeDir(env: NodeJS.ProcessEnv = process.env): string {
  const fromEnv = env['SPIDERBYTE_HOME'];
...
export interface DiscoverOptions {
  /** The dev-proxy target (`SPIDERBYTE_SERVER_URL`); merged as a `proxy` entry. */
...
export async function discoverLocalServers(
  ...
  const home = options.homeDir ?? resolveSpiderByteHomeDir();
...
export function serverDiscoveryPlugin(options: { proxyTarget: string }): Plugin {
  ...
  return {
    name: 'spiderbyte-inspect-server-discovery',
