...
beforeEach(async () => {
  home = await mkdtemp(join(tmpdir(), 'spiderbyte-inspect-discovery-'));
});
...
describe('resolveSpiderByteHomeDir', () => {
  it('honors SPIDERBYTE_HOME, else falls back to ~/.spiderbyte', () => {
    expect(resolveSpiderByteHomeDir({ SPIDERBYTE_HOME: '/tmp/kh' })).toBe('/tmp/kh');
    expect(resolveSpiderByteHomeDir({})).toBe(join(process.env['HOME'] ?? '', '.spiderbyte'));
  });
});
