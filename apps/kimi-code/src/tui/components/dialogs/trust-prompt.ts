  ...
  render(width: number): string[] {
    ...
    const notice =
      this.opts.gatedMcpServers.length > 0
        ? `SpiderByte loads project-level MCP servers (.mcp.json, .kimi-code/mcp.json) only in trusted folders. They run as local processes on your machine. This folder defines: ${this.opts.gatedMcpServers.join(', ')}.`
        : 'SpiderByte loads project-level MCP servers (.mcp.json, .kimi-code/mcp.json) only in trusted folders. They run as local processes on your machine.';
