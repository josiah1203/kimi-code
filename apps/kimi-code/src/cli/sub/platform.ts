...
const selectionPath = join(resolveKimiHome(), 'platform-selection.json');
...
export function registerPlatformCommands(parent: Command, version: string): void {
  ...
  const usage = parent
    .command('usage')
    .description('Show canonical SpiderByte Workspace usage and managed meter attribution.');
  usage
    .option('--workspace <workspace-id>', 'Workspace id; defaults to the selected or current Workspace.')
    .option('--json', 'Print machine-readable JSON.')
    .action(async (options: { readonly workspace?: string; readonly json?: boolean }) => {
      await runPlatformCommand({ stdout: process.stdout, stderr: process.stderr, version }, async (platform) => {
        ...
        writeRows(process.stdout, options.json === true, [summary], (item) =>
          `workspace=${item.workspace_id}\trecords=${item.record_count}\tmanaged_llm_units=${item.managed_llm_units}\tmanaged_compute_seconds=${item.managed_compute_seconds}`,
        );
      });
    });
  ...
  const plugins = parent
    .command('plugins')
    .description('List installed SpiderByte/Kimi plugins through the canonical harness.');
  plugins
    .option('--project <project-id>', 'Business Project id; defaults to the selected Project.')
    .option('--json', 'Print machine-readable JSON.')
  ...
  organization
    ...
    .option('--mode <mode>', 'Organization mode.', 'local')
    ...
    .action(async (options: { readonly name: string; readonly mode: string; readonly actor: string }) => {
      await runPlatformCommand({ stdout: process.stdout, stderr: process.stderr, version }, async (platform) => {
        if (options.mode !== 'local' && options.mode !== 'hosted') {
...
async function runPlatformCommand<T>(
  ...
  operation: (
    platform: NonNullable<KimiHarness['platform']>,
    harness: KimiHarness,
  ) => Promise<T>,
): Promise<void> {
  const harness = createKimiHarnessV2({
    homeDir: resolveKimiHome(),
    identity: createKimiCodeHostIdentity(deps.version),
    uiMode: 'platform-admin',
  });
  try {
    await harness.ensureConfigFile();
    if (harness.platform === undefined) {
      throw new Error('SpiderByte platform services are unavailable; enable platform mode or use the compatibility commands.');
    }
