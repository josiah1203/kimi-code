...
async function runSpyderByte(args) {
  try {
...
async function runSpyderByteWithEnv(args, env) {
  try {
    const { stdout, stderr } = await execFileAsync(executablePath, args, {
      cwd: appRoot,
      env: { ...process.env, ...env },
...
const versionOutput = await runSpyderByte(['--version']);
assertIncludes(versionOutput, expectedVersion, '--version');

const helpOutput = await runSpyderByte(['--help']);
assertIncludes(helpOutput, 'Usage: spyderbyte', '--help');

const exportHelpOutput = await runSpyderByte(['export', '--help']);
assertIncludes(exportHelpOutput, 'Usage: spyderbyte export', 'export --help');

const smokeCache = resolve(smokeHome, 'cache');
await rm(smokeHome, { recursive: true, force: true });
await mkdir(smokeCache, { recursive: true });
try {
  const nativeAssetOutput = await runSpyderByteWithEnv(['--version'], {
    SPIDERBYTE_CACHE_DIR: smokeCache,
    SPIDERBYTE_HOME: smokeHome,
    SPIDERBYTE_NATIVE_ASSET_SMOKE: '1',
  });
