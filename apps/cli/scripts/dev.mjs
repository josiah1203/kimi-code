...
// the whole repo instead of just apps/cli.
const REPO_ROOT = resolve(APP_ROOT, '../..');
// Runtime variable the CLI reads to locate the marketplace JSON.
const MARKETPLACE_ENV = 'SPIDERBYTE_PLUGIN_MARKETPLACE_URL';
// Opt-in for dev: point this run at an external marketplace instead of a local one.
const EXTERNAL_MARKETPLACE_ENV = 'SPIDERBYTE_DEV_MARKETPLACE_URL';
...
if (externalUrl !== undefined && externalUrl.length > 0) {
  ...
  env['SPIDERBYTE_PLUGIN_MARKETPLACE_FROM_DEV_SERVER'] = '1';
...
child.on('error', async (error) => {
  console.error(`Failed to start SpiderByte dev CLI: ${error.message}`);
