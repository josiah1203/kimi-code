...
function shouldAlwaysBundle(id: string): boolean {
  ...
  // Everything else is force-bundled, which covers `@spiderbyte/*` (incl.
  // vis-server for `spyderbyte vis`) plus its transitive `hono` / `@hono/node-server`
  // — so the SEA bundle is self-contained (check-bundle.mjs enforces this).
  return true;
}

function buildTarget(): string {
  return process.env['SPIDERBYTE_BUILD_TARGET'] ?? `${process.platform}-${process.arch}`;
}
...
export default defineConfig({
  ...
  define: {
    ...
    __SPIDERBYTE_VERSION__: JSON.stringify(packageJson.version),
    __SPIDERBYTE_CHANNEL__: JSON.stringify(process.env['SPIDERBYTE_CHANNEL'] ?? ''),
    __SPIDERBYTE_COMMIT__: JSON.stringify(process.env['SPIDERBYTE_COMMIT'] ?? ''),
    __SPIDERBYTE_BUILD_TARGET__: JSON.stringify(buildTarget()),
    __SPIDERBYTE_NATIVE_BUNDLE__: 'true',
  },
  deps: {
