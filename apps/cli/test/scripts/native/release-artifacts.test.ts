...
const executableName = process.platform === 'win32' ? 'spyderbyte.exe' : 'spyderbyte';
...
describe('native release artifacts', () => {
  afterEach(() => {
    ...
    rmSync(resolve(artifactsDir, `spyderbyte-${target}.zip`), { force: true });
    rmSync(resolve(artifactsDir, `spyderbyte-${target}.zip.sha256`), { force: true });
  });
  ...
  it('packages the native binary as a zip archive and checksums the archive', async () => {
    ...
    await execFileAsync(process.execPath, [packageScript], {
      cwd: appRoot,
      env: { ...process.env, SPIDERBYTE_BUILD_TARGET: target },
    });

    const archivePath = resolve(artifactsDir, `spyderbyte-${target}.zip`);
    ...
    expect(readFileSync(checksumPath, 'utf-8')).toBe(
      `${sha256(readFileSync(archivePath))}  spyderbyte-${target}.zip\n`,
    );
  });

  it('produces a manifest from zip archive checksums', async () => {
    const releaseDir = await mkdtemp(join(tmpdir(), 'spiderbyte-manifest-zip-'));
    const archiveBytes = Buffer.from('fake zip bytes');
    const checksum = sha256(archiveBytes);
    await writeFile(join(releaseDir, 'spyderbyte-darwin-arm64.zip'), archiveBytes);
    await writeFile(
      join(releaseDir, 'spyderbyte-darwin-arm64.zip.sha256'),
      `${checksum}  spyderbyte-darwin-arm64.zip\n`,
    );

    await execFileAsync(process.execPath, [manifestScript, releaseDir, '@spiderbyte/cli@0.5.0']);
    ...
    expect(manifest).toEqual({
      version: '0.5.0',
      tag: '@spiderbyte/cli@0.5.0',
      platforms: {
        'darwin-arm64': {
          filename: 'spyderbyte-darwin-arm64.zip',
          checksum,
        },
      },
    });
