...
describe('targetTriple', () => {
  ...
  it('honors SPIDERBYTE_BUILD_TARGET override', () => {
    expect(
      targetTriple({
        platform: 'darwin',
        arch: 'arm64',
        env: { SPIDERBYTE_BUILD_TARGET: 'linux-arm64' },
      }),
    ).toBe('linux-arm64');
  });
});
...
describe('executableName', () => {
  it('returns spyderbyte.exe on win32', () => {
    expect(executableName('win32')).toBe('spyderbyte.exe');
  });

  it('returns spyderbyte on other platforms', () => {
    expect(executableName('darwin')).toBe('spyderbyte');
    expect(executableName('linux')).toBe('spyderbyte');
  });
});
...
describe('path helpers', () => {
  ...
  it('returns absolute bin path with executable name', () => {
    expect(nativeBinPath('darwin-arm64', 'darwin')).toBe(
      p('dist-native/bin/darwin-arm64/spyderbyte'),
    );
    expect(nativeBinPath('win32-x64', 'win32')).toBe(
      p('dist-native/bin/win32-x64/spyderbyte.exe'),
    );
  });

  it('returns intermediate artifact paths', () => {
    expect(nativeJsBundlePath()).toBe(p('dist-native/intermediates/main.cjs'));
    expect(nativeBlobPath()).toBe(p('dist-native/intermediates/spyderbyte.blob'));
