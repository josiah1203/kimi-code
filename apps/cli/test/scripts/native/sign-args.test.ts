...
describe('buildCodesignArgs', () => {
  it('returns ad-hoc args for identity "-"', () => {
    const args = buildCodesignArgs({
      identity: '-',
      executable: '/path/spyderbyte',
      entitlementsPath: '/path/entitlements.plist',
      keychainPath: null,
    });
    expect(args).toEqual(['--sign', '-', '/path/spyderbyte']);
  });

  it('returns hardened-runtime args for Developer ID identity', () => {
    const args = buildCodesignArgs({
      identity: 'Developer ID Application: SpiderByte Contributors (ABCD1234)',
      executable: '/path/spyderbyte',
      entitlementsPath: '/path/entitlements.plist',
    ...
    expect(args).toEqual([
      '--sign',
      'Developer ID Application: SpiderByte Contributors (ABCD1234)',
      '--options',
      'runtime',
      '--entitlements',
