...
describe('commandForExecFile', () => {
  it('returns command as-is on non-Windows', () => {
    const result = commandForExecFile('postject', ['spyderbyte', 'NODE_SEA_BLOB', './blob'], 'darwin');
    expect(result).toEqual({ command: 'postject', args: ['spyderbyte', 'NODE_SEA_BLOB', './blob'] });
  });

  it('returns command as-is on Windows for non-batch files', () => {
    const result = commandForExecFile('postject.exe', ['spyderbyte.exe'], 'win32');
    expect(result).toEqual({ command: 'postject.exe', args: ['spyderbyte.exe'] });
  });

  it('wraps .cmd files through cmd.exe on Windows', () => {
    const result = commandForExecFile('postject.cmd', ['spyderbyte.exe', 'NODE_SEA_BLOB'], 'win32', {
    ...
    expect(result.args).toEqual([
      ...
      '/s',
      '/c',
      '""postject.cmd" "spyderbyte.exe" "NODE_SEA_BLOB""',
    ]);
