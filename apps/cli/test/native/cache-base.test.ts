...
describe('getNativeCacheBase precedence', () => {
  ...
  it('uses SPIDERBYTE_CACHE_DIR when set (highest precedence)', () => {
    const got = getNativeCacheBase({
      ...baseOptions,
      env: { SPIDERBYTE_CACHE_DIR: '/custom/cache' },
    });
    expect(got).toBe('/custom/cache');
  });

  it('ignores SPIDERBYTE_HOME (no longer affects native cache)', () => {
    const got = getNativeCacheBase({
      ...baseOptions,
      env: { SPIDERBYTE_HOME: '/legacy' },
      platform: 'darwin',
    });
    expect(got).toBe('/home/u/Library/Caches/spiderbyte');
  });
  ...
  it('uses XDG_CACHE_HOME on Linux when set', () => {
    ...
    expect(got).toBe('/xdg/spiderbyte');
  });
  ...
  it('uses LOCALAPPDATA on Windows when set', () => {
    ...
    expect(got).toBe('C:\\Users\\u\\AppData\\Local\\spiderbyte');
  });
});
