...
import { SPIDERBYTE_HOME_ENV } from '#/constant/app';
...
function setupTempCache(): { cleanup: () => void } {
  const home = mkdtempSync(join(tmpdir(), 'spiderbyte-home-'));
  const prev = process.env[SPIDERBYTE_HOME_ENV];
  process.env[SPIDERBYTE_HOME_ENV] = home;
  return {
    cleanup: () => {
      if (prev === undefined) delete process.env[SPIDERBYTE_HOME_ENV];
      else process.env[SPIDERBYTE_HOME_ENV] = prev;
...
function makeTempDir(): string {
  return mkdtempSync(join(tmpdir(), 'spiderbyte-src-'));
}
...
describe('extractMediaAttachments', () => {
  ...
  it('inserts a compression caption before an image that was compressed at paste time', () => {
    ...
    const att = store.addImage(new Uint8Array([1, 2, 3]), 'image/png', 2000, 2000, {
      path: '/tmp/spiderbyte-original-images/abc.png',
      width: 2600,
      height: 2600,
      byteLength: 123456,
      mime: 'image/png',
    ...
    expect(caption.text).toContain('/tmp/spiderbyte-original-images/abc.png');
    expect(r.parts[1]).toEqual({
      type: 'image_url',
