...
import type { PromptOrigin as CorePromptOrigin } from '@spiderbyte/agent-core';
...
function message(
  ...
  extra: {
    ...
    readonly origin?: CorePromptOrigin | TaskNotificationOrigin;
  ...
  return {
    ...
    message: {
      ...
      origin: extra.origin as unknown as CorePromptOrigin | undefined,
...
function makeHarness(initialSession: Session) {
  ...
  return {
    getConfig: vi.fn(async () => ({
      models: {
        k2: { model: 'example-model', maxContextSize: 100 },
      },
    })),
...
async function makeDriver(initialSession: Session): Promise<ReplayDriver> {
  const driver = new SpiderByteTUI(
...
describe('SpiderByteTUI resume message replay', () => {
    ...
    const driver = await replayIntoDriver(
      [
        message('user', [{ type: 'text', text: 'Background task lost.' }], {
          ...
          } as unknown as CorePromptOrigin,
        }),
      ],
      {
    ...
    const projection = replayBackgroundProjection(
      [agentTask({ model: 'k2-cheap', thinkingEffort: 'low' })],
      {
        'k2-cheap': {
          ...
          model: 'spiderbyte-k2-cheap',
          displayName: 'SpiderByte K2 Cheap',
        },
      } as never,
    );
