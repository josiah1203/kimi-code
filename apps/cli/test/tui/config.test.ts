...
beforeEach(() => {
  dir = join(tmpdir(), `spiderbyte-tui-config-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('TUI config', () => {
  it('creates the default config when the file does not exist', async () => {
    ...
    expect(text).toContain('Client preferences for spiderbyte.');
...
describe('TUI config status_line round-trip', () => {
  it('preserves an active status_line across save and reload', async () => {
    await saveTuiConfig(
      {
        ...DEFAULT_TUI_CONFIG,
        statusLine: { items: ['model', 'git'], command: '~/.spiderbyte/statusline.sh' },
      },
      filePath,
    );
