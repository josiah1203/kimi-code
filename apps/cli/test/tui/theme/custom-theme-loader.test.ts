...
let home: string;
const originalHome = process.env['SPIDERBYTE_HOME'];

beforeEach(() => {
  home = join(tmpdir(), `spiderbyte-themes-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
afterEach(() => {
  ...
  if (originalHome === undefined) {
    ...
    process.env['SPIDERBYTE_HOME'] = originalHome;
  }
});
