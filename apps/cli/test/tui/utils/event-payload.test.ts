...
describe('error payload formatting', () => {
  ...
  it('shows concise provider filter text from SpiderByteError details', () => {
    const error = new SpiderByteError(ErrorCodes.PROVIDER_API_ERROR, filteredThinkOnlyMessage, {
      details: {
        finishReason: 'filtered',
        rawFinishReason: 'content_filter',
      },
