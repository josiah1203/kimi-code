 ...
 * Run: pnpm -C apps/cli exec vitest run test/tui/commands/secondary-model.test.ts
 */
import type { ModelAlias, ThinkingEffort } from '@spiderbyte/sdk';
...
describe('handleSecondaryModelCommand', () => {
  ...
  it('warns with the env-overridden effective binding instead of the picked model', async () => {
    // SPIDERBYTE_SECONDARY_MODEL / SPIDERBYTE_SECONDARY_EFFORT win over the persisted
    ...
    expect(message).toContain('SPIDERBYTE_SECONDARY_MODEL=cheap');
    expect(message).toContain('SPIDERBYTE_SECONDARY_EFFORT=low');
