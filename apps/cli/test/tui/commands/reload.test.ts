...
const originalSpiderByteHome = process.env['SPIDERBYTE_HOME'];
...
afterEach(async () => {
  ...
  if (originalSpiderByteHome === undefined) {
    delete process.env['SPIDERBYTE_HOME'];
  } else {
    process.env['SPIDERBYTE_HOME'] = originalSpiderByteHome;
  }
});
...
async function writeTuiConfig(text: string): Promise<void> {
  const dir = join(tmpdir(), `spiderbyte-tui-reload-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  tempDirs.push(dir);
