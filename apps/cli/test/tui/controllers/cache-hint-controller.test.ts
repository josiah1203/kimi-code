...
const CONFIG: CacheHintConfig = {
  version: 1,
  config: { 'spiderbyte-k2': { min_tokens_to_hint: 100000, cache_duration: 600 } },
};
...
function makeHost(
  ...
  const state = {
    ...
    appState: {
      model: 'k2',
      availableModels: { k2: { model: 'spiderbyte-k2', provider: 'managed:spiderbyte' } },
      availableProviders: { 'managed:spiderbyte': { oauth: { key: 'spiderbyte' } } },
      sessionId: 's1',
      streamingPhase: 'idle',
      isCompacting: false,
...
describe('CacheHintController scenario 2 (idle submit)', () => {
  ...
  it('does not intercept when the provider is not OAuth-managed', () => {
    ...
    const { host } = makeHost({
      appState: {
        availableProviders: { 'managed:spiderbyte': {} }, // apiKey form: no oauth
      },
    });
    const controller = new CacheHintController(host);
    controller.recordActivity();
