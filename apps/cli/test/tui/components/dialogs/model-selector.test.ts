import type { ModelAlias } from '@spiderbyte/sdk';
...
describe('ModelSelectorComponent', () => {
  it('lays out the provider as a right column and marks the current model', () => {
    const picker = new ModelSelectorComponent({
      models: { local: model('Example Local Model') },
      currentValue: 'local',
    ...
    expect(out).toMatch(/❯ Example Local Model\s+local ← current/);
    // Provider is no longer inlined in parentheses next to the name.
    expect(out).not.toContain('Example Local Model (local)');
  });

  it('toggles thinking with Left/Right (not with "/")', () => {
    const onSelect = vi.fn();
    const picker = new ModelSelectorComponent({
      models: { local: model('Example Local Model', ['thinking']) },
      currentValue: 'local',
      currentThinkingEffort: 'on',
      onSelect,
      onCancel: vi.fn(),
    ...
    expect(onSelect).toHaveBeenLastCalledWith({ alias: 'local', thinking: 'on' });

    // Right arrow flips the draft (true -> false).
    picker.handleInput(RIGHT);
    picker.handleInput('\r');
    expect(onSelect).toHaveBeenLastCalledWith({ alias: 'local', thinking: 'off' });
  ...
  it('forces always-thinking models on and unsupported models off', () => {
    ...
    const picker = new ModelSelectorComponent({
      models: {
        always: model('Thinking Model', ['always_thinking']),
        plain: model('Plain Model', ['tool_use']),
      },
      currentValue: 'always',
      currentThinkingEffort: 'off',
      onSelect,
  ...
  it('renders the unavailable thinking segment muted', () => {
    const picker = new ModelSelectorComponent({
      models: { always: model('Thinking Model', ['always_thinking']) },
      currentValue: 'always',
      currentThinkingEffort: 'on',
      onSelect: vi.fn(),
      onCancel: vi.fn(),
  ...
  it('defaults a thinking-capable model to On but keeps the current model state', () => {
    ...
    const picker = new ModelSelectorComponent({
      models: {
        current: model('Current Model', ['thinking']),
        other: model('Other Model', ['thinking']),
      },
  ...
  it('fuzzy-filters by typing and reports a match count', () => {
    ...
    const picker = new ModelSelectorComponent({
      models: { k2: model('Example Local Model'), turbo: model('Turbo Model') },
      currentValue: 'k2',
      currentThinkingEffort: 'off',
      searchable: true,
      onSelect: vi.fn(),
    ...
    expect(out).toContain('Turbo Model');
    expect(out).not.toContain('Example Local Model');
  ...
  it('invokes onSessionOnlySelect on Alt+S with the effective thinking state', () => {
    ...
    expect(onSessionOnlySelect).toHaveBeenCalledWith({ alias: 'local', thinking: 'off' });
  ...
  it('renders effort segments with the default effort highlighted', () => {
    const picker = new ModelSelectorComponent({
      models: { local: effortModel('Example Local Model', ['low', 'high', 'max'], 'high') },
      currentValue: 'local',
  ...
  it('cycles efforts with Left/Right and clamps at the ends', () => {
    ...
    picker.handleInput(RIGHT);
    picker.handleInput('\r');
    expect(onSelect).toHaveBeenLastCalledWith({ alias: 'local', thinking: 'max' });
  ...
  it('always-on effort models hide Off and clamp selection at the last effort', () => {
    ...
    const picker = new ModelSelectorComponent({
      models: {
        local: effortModel('Example Local Model', ['low', 'high', 'max'], 'high', ['always_thinking']),
      },
      currentValue: 'local',
      currentThinkingEffort: 'high',
      onSelect,
  ...
  it('defaults an effort model without a current level to its defaultEffort', () => {
    ...
    const picker = new ModelSelectorComponent({
      models: {
        other: effortModel('Other Model', ['low', 'high', 'max'], 'max'),
      },
      currentValue: 'current',
      currentThinkingEffort: 'off',
      onSelect,
  ...
  it('falls back to the middle effort when an effort model has no defaultEffort', () => {
    const picker = new ModelSelectorComponent({
      models: {
        other: effortModel('Other Model', ['low', 'medium', 'high']),
      },
      currentValue: 'current',
      currentThinkingEffort: 'off',
      onSelect: vi.fn(),
  ...
  it('renders the warning line directly below the key-hint line when provided', () => {
    ...
    expect(lines.findIndex((l) => l.includes('Example Local Model'))).toBeGreaterThan(hintIdx + 1);
  });
...
describe('ModelSelectorComponent overrides', () => {
  it('uses overridden support_efforts for selectable efforts', () => {
    const picker = new ModelSelectorComponent({
      models: {
        local: {
          ...effortModel('Example Local Model', ['low', 'high', 'max'], 'max'),
