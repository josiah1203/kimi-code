...
function feature(
  ...
  return {
    ...
    surface: 'core',
    env: 'SPIDERBYTE_EXPERIMENTAL_MICRO_COMPACTION',
    defaultEnabled: true,
    enabled: true,
    source: 'default',
    ...overrides,
...
describe('ExperimentsSelectorComponent', () => {
  it('renders searchable feature toggles with source details', () => {
    ...
    expect(out).toContain('    id micro_compaction · config · SPIDERBYTE_EXPERIMENTAL_MICRO_COMPACTION');
  ...
  it('drafts changes with Space and applies them with Enter', () => {
    ...
    expect(text(selector)).toContain(
      '    id micro_compaction · default · SPIDERBYTE_EXPERIMENTAL_MICRO_COMPACTION · modified',
    );
