import type { TUI } from '@spiderbyte/pi-tui';
...
describe('AgentGroupComponent', () => {
  ...
  it('shows the bound model in the row stats once reported', () => {
    ...
    running.updateSubagentMetrics({ modelDisplay: 'SpiderByte K2.5' });
    // Non-phase updates are throttled; flush the pending refresh.
    vi.runOnlyPendingTimers();
    expect(renderText(group)).toContain('explore · inspect project · SpiderByte K2.5 · 0 tools');
