import { visibleWidth, type TUI } from '@spiderbyte/pi-tui';
...
describe('ToolCallComponent', () => {
  ...
  it('shows Read paths relative to the active workspace', () => {
    ...
    const expectedReadPath =
      process.platform === 'win32' ? 'apps\\spiderbyte\\src\\main.ts' : 'apps/cli/src/main.ts';
  ...
  it('shows the bound model in the subagent header and group snapshot once reported', () => {
    ...
    expect(out).not.toContain('SpiderByte K2.5');

    component.updateSubagentMetrics({ modelDisplay: 'SpiderByte K2.5' });

    out = strip(component.render(120).join('\n'));
    expect(out).toContain('Explore Agent Queued (explore project) · SpiderByte K2.5 · 0 tools');
    expect(component.getSubagentSnapshot().model).toBe('SpiderByte K2.5');
  });
