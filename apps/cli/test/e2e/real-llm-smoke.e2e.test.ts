/**
 * Real-LLM smoke. Opt-in only — runs when `SPIDERBYTE_E2E_REAL=1` is set, so
 * `make test` / CI never touches the network. Goes through the SDK's
 * `SpiderByteHarness` entry and round-trips one real prompt. Requires provider
 * credentials on disk.
 *
 * Env knobs:
 *   SPIDERBYTE_E2E_REAL     — set to "1" to enable this suite
 *   SPIDERBYTE_E2E_MODEL    — model alias override (default: config's default)
 *   SPIDERBYTE_E2E_PROMPT   — prompt text (default: "Reply with a single word: hi")
 *   SPIDERBYTE_E2E_WORKDIR  — workspace directory (default: /tmp/spiderbyte-e2e)
 */

import { mkdirSync } from 'node:fs';
import process from 'node:process';

import { createSpiderByteHarness, type Event } from '@spiderbyte/sdk';
import { describe, expect, test } from 'vitest';

import { createSpiderByteHostIdentity, getVersion } from '#/cli/version';

const DEFAULT_PROMPT = 'Reply with a single word: hi';
const DEFAULT_WORKDIR = '/tmp/spiderbyte-e2e';
const TURN_TIMEOUT_MS = 60_000;

const ENABLED = process.env['SPIDERBYTE_E2E_REAL'] === '1';
type TurnEndedEvent = Extract<Event, { readonly type: 'turn.ended' }>;

describe.skipIf(!ENABLED)('SDK e2e — real LLM smoke', () => {
  test(
    'round-trips a single prompt through SpiderByteHarness',
    async () => {
      const workDir = process.env['SPIDERBYTE_E2E_WORKDIR'] ?? DEFAULT_WORKDIR;
      const prompt = process.env['SPIDERBYTE_E2E_PROMPT'] ?? DEFAULT_PROMPT;
      const modelAlias = process.env['SPIDERBYTE_E2E_MODEL'];
