...
const ENABLED = process.env['SPIDERBYTE_E2E'] === '1';
...
beforeEach(async () => {
  homeDir = await mkdtemp(join(tmpdir(), 'spiderbyte-cli-log-home-'));
  workDir = await mkdtemp(join(tmpdir(), 'spiderbyte-cli-log-work-'));
  oldHome = process.env['SPIDERBYTE_HOME'];
  oldLogLevel = process.env['SPIDERBYTE_LOG_LEVEL'];
  process.env['SPIDERBYTE_HOME'] = homeDir;
  process.env['SPIDERBYTE_LOG_LEVEL'] = 'info';
});
...
afterEach(async () => {
  if (oldHome === undefined) {
    ...
    process.env['SPIDERBYTE_HOME'] = oldHome;
  }
  if (oldLogLevel === undefined) {
    delete process.env['SPIDERBYTE_LOG_LEVEL'];
  } else {
    process.env['SPIDERBYTE_LOG_LEVEL'] = oldLogLevel;
  }
...
describe.skipIf(!ENABLED)('local logging export e2e', () => {
  it('exports session log and global log by default, and allows skipping global log', async () => {
    const harness = createSpiderByteHarness({
      homeDir,
      identity: createSpiderByteHostIdentity('0.1.1'),
    });
    try {
      ...
      await runSpiderByteExport([session.id, '-o', defaultZip]);
      ...
      await runSpiderByteExport([session.id, '-o', noGlobalZip, '--no-include-global-log']);
...
async function runSpiderByteExport(args: string[]): Promise<void> {
  ...
  registerExportCommand(program, {
    ...
    exit: (code: number): never => {
      throw new Error(`spyderbyte export exited ${code}: ${stderr.join('')}`);
    },
  });
  await program.parseAsync(['node', 'spyderbyte', 'export', ...args]);
}
