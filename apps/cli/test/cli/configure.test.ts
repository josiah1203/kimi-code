...
const configureMocks = vi.hoisted(() => {
  ...
  return { connection, session, platform, harness, createSpiderByteHarness: vi.fn(() => harness) };
});
...
vi.mock('@spiderbyte/sdk', async (importOriginal) => {
  ...
  return {
    ...actual,
    createSpiderByteHarness: configureMocks.createSpiderByteHarness,
    resolveSpiderByteHome: vi.fn(() => '/tmp/spyderbyte-configure-test'),
  };
});
