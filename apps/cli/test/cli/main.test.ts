...
import { ErrorCodes, SpiderByteError } from '@spiderbyte/sdk';
...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    SpiderByteHarness: vi.fn(),
    createSpiderByteHarness: vi.fn(),
  };
});
...
vi.mock('@spiderbyte/sdk', async () => {
  const actual = await vi.importActual<typeof import('@spiderbyte/sdk')>(
    '@spiderbyte/sdk',
  );
  class MockSpiderByteHarness {
    ...
    constructor(...args: unknown[]) {
      mocks.SpiderByteHarness(...args);
    }
  }
  return {
    ...actual,
    createSpiderByteHarness: (...args: unknown[]) => {
      mocks.createSpiderByteHarness(...args);
      return mocks.harness;
    },
    flushDiagnosticLogs: mocks.flushDiagnosticLogs,
    SpiderByteHarness: MockSpiderByteHarness,
    log: mocks.log,
  };
});
...
describe('main entry command handling', () => {
  ...
  it('sets the process title during startup', () => {
    ...
    try {
      process.title = 'spyderbyte-test-runner';
      main();

      expect(process.title).toBe('spyderbyte');
    } finally {
  ...
  it('initializes and flushes telemetry around the upgrade command', async () => {
    ...
    expect(mocks.createSpiderByteHarness).toHaveBeenCalledWith(expect.objectContaining({
      homeDir: '/tmp/spyderbyte-home',
      telemetry: {
        track: mocks.track,
  ...
  it('formats SpiderByte startup errors with structured fields', () => {
    const error = new SpiderByteError(
