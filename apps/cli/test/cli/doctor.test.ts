...
beforeEach(async () => {
  dir = join(tmpdir(), `spiderbyte-doctor-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('spyderbyte doctor', () => {
  ...
  it('treats a missing explicit target path as an error', async () => {
    ...
    expect(err).toContain('SpiderByte doctor found 1 issue.');
  ...
  it('checks a valid explicit config path routed through commander', async () => {
    ...
    await program.parseAsync(['node', 'spyderbyte', 'doctor', 'config', './candidate-config.toml']);
  ...
  it('checks a valid explicit tui path routed through commander', async () => {
    ...
    await program.parseAsync(['node', 'spyderbyte', 'doctor', 'tui', './candidate-tui.toml']);
...
`,
    ...
    expect(stdout.join('')).toBe('');
    const err = stderr.join('');
    expect(err).toContain('SpiderByte doctor found 2 issues.');
...
`,
    ...
    const err = stderr.join('');
    expect(err).toContain('Validation issues:');
    expect(err).toContain('models.example.max_context_size:');
  });
});

describe('spyderbyte doctor (default v2 config validation)', () => {
  afterEach(() => {
    delete process.env['SPIDERBYTE_LOOP_MAX_RETRIES_PER_STEP'];
    delete process.env['SPIDERBYTE_LOOP_MAX_ATTEMPTS_PER_STEP'];
  });
...
`,
  ...
  it('reports TOML syntax errors with line and column', async () => {
    await writeFile(join(dir, 'config.toml'), '[providers.example\ntype = "openai"\n', 'utf-8');
    const { deps, stderr } = makeDeps();
...
`,
  ...
  it('warns about a deprecated env var that supplies a value', async () => {
    ...
    process.env['SPIDERBYTE_LOOP_MAX_RETRIES_PER_STEP'] = '5';
    const { deps, stdout, stderr } = makeDeps();
    ...
    expect(stdout.join('')).toContain(
      'Environment variable SPIDERBYTE_LOOP_MAX_RETRIES_PER_STEP is deprecated; use SPIDERBYTE_LOOP_MAX_ATTEMPTS_PER_STEP instead.',
    );
  });
  ...
  it('does not warn about the deprecated env var when the primary one is set', async () => {
    await writeFile(join(dir, 'config.toml'), '[loop_control]\n', 'utf-8');
    process.env['SPIDERBYTE_LOOP_MAX_RETRIES_PER_STEP'] = '5';
    process.env['SPIDERBYTE_LOOP_MAX_ATTEMPTS_PER_STEP'] = '5';
    const { deps, stdout } = makeDeps();

    const code = await handleDoctor(deps, { target: 'config' });

    expect(code).toBe(0);
    expect(stdout.join('')).not.toContain('SPIDERBYTE_LOOP_MAX_RETRIES_PER_STEP');
  });
});
