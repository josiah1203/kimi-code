...
vi.mock('../../../src/tui/config', () => ({
  ...
  TuiConfigParseError: class TuiConfigParseError extends Error {
    ...
    constructor(fallback: TuiConfig) {
      super('Invalid client preferences in ~/.spiderbyte/tui.toml; using defaults.');
...
describe('runUpdatePreflight', () => {
  beforeEach(() => {
    ...
    mocks.tryAcquireUpdateInstallLock.mockResolvedValue({
      filePath: '/tmp/spiderbyte-update-install.lock',
  ...
  it('skips all update work when SPIDERBYTE_NO_AUTO_UPDATE is set', async () => {
    vi.stubEnv('SPIDERBYTE_NO_AUTO_UPDATE', '1');
  ...
  it('also honors the legacy SPIDERBYTE_CLI_NO_AUTO_UPDATE alias', async () => {
    vi.stubEnv('SPIDERBYTE_CLI_NO_AUTO_UPDATE', 'true');
    mocks.readUpdateCache.mockResolvedValue(cacheWith('0.5.0'));
    const { options } = captureOutput();
  ...
  it('starts an automatic update from the first fresh check when the cache is empty', async () => {
    ...
    expect(mocks.spawn).toHaveBeenCalledWith(
      ...
      ['install', '-g', '@spiderbyte/cli@0.5.0'],
  ...
  it('npm-global: prompts and spawns npm install -g when automatic updates are disabled', async () => {
    ...
    expect(mocks.promptForInstallChoice).toHaveBeenCalledWith(
      expect.objectContaining({
        installCommand: 'npm install -g @spiderbyte/cli@0.5.0',
    ...
    expect(stdout.join('')).toContain('Updated @spiderbyte/cli to 0.5.0');
  });
  ...
  it('refreshes a stale cached target before showing the foreground install prompt', async () => {
    ...
    expect(mocks.promptForInstallChoice).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        installCommand: 'npm install -g @spiderbyte/cli@0.7.0',
      }),
    );
    expect(mocks.spawn).toHaveBeenCalledWith(
      expect.stringMatching(/^npm(\.cmd)?$/),
      ['install', '-g', '@spiderbyte/cli@0.7.0'],
      { stdio: 'inherit' },
    );
    expect(stdout.join('')).toContain('Updated @spiderbyte/cli to 0.7.0');
  });
    ...
    try {
      ...
      expect(mocks.promptForInstallChoice).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          installCommand: 'npm install -g @spiderbyte/cli@0.6.0',
        }),
      );
    } finally {
      vi.useRealTimers();
  ...
  it('yarn-global: spawns yarn global add', async () => {
    ...
    expect(mocks.spawn).toHaveBeenCalledWith(
      ...
      ['global', 'add', '@spiderbyte/cli@0.5.0'],
      { stdio: 'inherit' },
    );
  });
  ...
  it('unsupported: prints fallback npm command', async () => {
    ...
    const { stdout, options } = captureOutput();
    await expect(runUpdatePreflight('0.4.0', options)).resolves.toBe('continue');
    expect(stdout.join('')).toContain('npm install -g @spiderbyte/cli@0.5.0');
    expect(mocks.spawn).not.toHaveBeenCalled();
  });
  ...
  it('warns and continues when spawn exits non-zero, without claiming success', async () => {
    ...
    expect(stdout.join('')).not.toContain('Updated @spiderbyte/cli');
  });
  ...
  it('shows a one-shot notice after a background update succeeds and the new version starts', async () => {
    ...
    expect(rendered).toContain('SpiderByte updated to v0.5.0');
    expect(rendered).toContain(
      'https://github.com/SpiderByte/spiderbyte/tree/main/docs/en/release-notes/changelog.html',
    );
  ...
  it('infers a background update success notice when the active install version is now running', async () => {
    ...
    expect(stdout.join('')).toContain('SpiderByte updated to v0.5.0');
    expect(writeUpdateInstallState).toHaveBeenCalledWith(expect.objectContaining({
      active: null,
      lastFailure: null,
      lastSuccess: expect.objectContaining({
  ...
  describe('rollout gating', () => {
    ...
    it('SPIDERBYTE_EXPERIMENTAL_FLAG bypasses the rollout: held devices still update', async () => {
    ...
    it('SPIDERBYTE_NO_AUTO_UPDATE still wins over the experimental flag', async () => {
      vi.stubEnv('SPIDERBYTE_EXPERIMENTAL_FLAG', '1');
      vi.stubEnv('SPIDERBYTE_NO_AUTO_UPDATE', '1');
