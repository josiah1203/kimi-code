 ...
 * Run: pnpm -C apps/cli exec vitest run test/cli/options.test.ts
 */
...
function parse(argv: string[]): CLIOptions {
  ...
  program.parse(['node', 'spyderbyte', ...argv]);
...
describe('CLI options parsing', () => {
  ...
  describe('--version', () => {
    it('prints the version string and exits', () => {
      ...
      expect(() => program.parse(['node', 'spyderbyte', '--version'])).toThrow();
    ...
    it('supports -V as a short alias', () => {
      ...
      expect(() => program.parse(['node', 'spyderbyte', '-V'])).toThrow();
  ...
  describe('--model / -m', () => {
    it('parses -m as a model override', () => {
      expect(parse(['-m', 'spiderbyte/k2']).model).toBe('spiderbyte/k2');
    });

    it('parses --model=value as a model override', () => {
      expect(parse(['--model=spiderbyte/k2.5']).model).toBe('spiderbyte/k2.5');
    });
  ...
  describe('SPIDERBYTE_MODEL_OUTPUT_FORMAT', () => {
    ...
    it('rejects an invalid env value', () => {
      ...
      ).toThrow('Invalid SPIDERBYTE_MODEL_OUTPUT_FORMAT value "json"');
    });
  ...
  describe('sub-commands', () => {
    it('routes upgrade without calling the main action', () => {
      ...
      program.parse(['node', 'spyderbyte', 'upgrade']);
    ...
    it('routes update alias to the upgrade handler', () => {
      ...
      program.parse(['node', 'spyderbyte', 'update']);
