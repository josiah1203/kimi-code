...
const mocks = vi.hoisted(() => ({
  ...
  resolveSpiderByteHome: vi.fn((homeDir?: string) => homeDir ?? '/tmp/spyderbyte-export-home'),
...
vi.mock('@spiderbyte/sdk', async (importOriginal) => {
  ...
  const createFakeHarness = (options: { readonly homeDir?: string } | undefined) => {
    const homeDir = options?.homeDir ?? '/tmp/spyderbyte-export-home';
    if (mocks.harnessCreatesDeviceIdOnConstruction) {
      mocks.createSpiderByteDeviceId(homeDir);
    }
    return {
      homeDir,
      auth: {
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'spyderbyte-export-'));
});
...
afterEach(() => {
  ...
  mocks.resolveSpiderByteHome.mockImplementation(
    (homeDir?: string) => homeDir ?? '/tmp/spyderbyte-export-home',
  );
...
describe('spyderbyte export', () => {
  ...
  it('parses --no-include-global-log as an option when no session id is given', async () => {
    ...
    await program.parseAsync(['node', 'spyderbyte', 'export', '--no-include-global-log', '-y']);
  ...
  it('initializes and flushes telemetry around default export tracking', async () => {
    ...
    await program.parseAsync(['node', 'spyderbyte', 'export', 'ses_telemetry', '--output', output], {
      from: 'node',
    });
  ...
  it('passes enabled false when default export config disables telemetry', async () => {
    ...
    await program.parseAsync(['node', 'spyderbyte', 'export', 'ses_disabled', '--output', output], {
      from: 'node',
    });
  ...
  it('tracks first launch around default export telemetry before harness construction can create the device id', async () => {
    ...
    await program.parseAsync(['node', 'spyderbyte', 'export', 'ses_first_launch', '--output', output], {
      from: 'node',
    });
    ...
    expect(mocks.spiderByteHarnessConstructor).toHaveBeenCalledWith(
      expect.objectContaining({ homeDir: '/tmp/spyderbyte-export-home' }),
    );
