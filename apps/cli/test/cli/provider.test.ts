/**
 * `spyderbyte provider` CLI unit tests. The handlers receive an injected `getHarness`
...
vi.mock('@spiderbyte/sdk', async (importOriginal) => {
  ...
  return {
    ...
    createSpiderByteHarness: (...args: unknown[]) => {
      harnessRouting.spiderByteHarnessConstructor(...args);
...
function makeHarness(initial: SpiderByteConfig): {
  harness: FakeHarness;
  current: () => SpiderByteConfig;
  setConfigCalls: Array<Partial<SpiderByteConfig>>;
  removeCalls: string[];
} {
  // `persisted` simulates the on-disk config; the real RPC's `removeProvider`
  // reads from / writes to disk on every call (see
  // `packages/agent-core/src/rpc/core-impl.ts removeProvider`). Tests must
  ...
  // `removeProvider`.
  let persisted: SpiderByteConfig = structuredClone(initial);
  const setConfigCalls: Array<Partial<SpiderByteConfig>> = [];
  ...
  const harness: FakeHarness = {
    ...
    setConfig: async (patch) => {
      ...
      // Mirror the real `setSpiderByteConfig`: deep-merge with undefined keys
      ...
      persisted = next as unknown as SpiderByteConfig;
...
describe('spyderbyte provider add', () => {
  it('imports providers and models from a custom registry, persisting source on each provider', async () => {
    ...
    const { harness, current, setConfigCalls } = makeHarness({ providers: {} } as SpiderByteConfig);
  ...
  it('drops a stale provider before re-applying when the id already exists', async () => {
    mockRegistryFetch();
    const initial: SpiderByteConfig = {
      providers: {
        kohub: {
          type: 'kimi',
    ...
    } as unknown as SpiderByteConfig;
  ...
  it('reads the api key from SPIDERBYTE_REGISTRY_API_KEY when --api-key is omitted', async () => {
    const fetchMock = mockRegistryFetch();
    const { harness } = makeHarness({ providers: {} } as SpiderByteConfig);
    const { deps, exitCodes } = makeDeps(harness, {
      env: { SPIDERBYTE_REGISTRY_API_KEY: 'sk-env-token' },
    });
...
describe('spyderbyte provider remove', () => {
...
describe('spyderbyte provider list', () => {
  const config: SpiderByteConfig = {
    providers: {
      kohub: {
        type: 'anthropic',
        baseUrl: 'https://x',
  ...
  it('renders one row per provider with counts and source labels', async () => {
    ...
    expect(out).toMatch(/managed:spiderbyte\s+type=kimi\s+models=0\s+source=oauth/);
...
describe('registerProviderCommand', () => {
  ...
  it('reports write failures on stderr and exits 1 instead of crashing', async () => {
    ...
    harness.removeProvider = async () => {
      throw new Error(
        'Cannot change settings while config.toml is invalid — fix it first (run `spyderbyte doctor` for details).',
      );
    };
    const { deps, stderr, exitCodes } = makeDeps(harness);
...
describe('spyderbyte provider catalog list', () => {
...
describe('spyderbyte provider catalog add', () => {
  ...
  it('rejects an unknown --default-model with a helpful hint', async () => {
    ...
    expect(err).toContain('spyderbyte provider catalog list anthropic');
  });
  ...
  it('does not persist thinking.enabled=false for first-time setup with --default-model', async () => {
    ...
    // explicit "off" request. A fresh `spyderbyte provider catalog add
  ...
  it('falls back to SPIDERBYTE_REGISTRY_API_KEY when --api-key is omitted', async () => {
    mockRegistryFetch(CATALOG_BODY);
    const { harness, current } = makeHarness({ providers: {} } as SpiderByteConfig);
    const { deps, exitCodes } = makeDeps(harness, {
      env: { SPIDERBYTE_REGISTRY_API_KEY: 'sk-env' },
    });
