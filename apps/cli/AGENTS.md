# apps/cli Development Guide

This file only contains rules local to `apps/cli`. For cross-repo rules, see the root `AGENTS.md`.
...
`apps/cli` is the terminal UI / CLI app. The entry chain is:

`src/main.ts` -> `src/cli/commands.ts` -> `src/cli/run-shell.ts` -> SDK `SpiderByteHarness` -> `src/tui/spiderbyte-tui.ts`
...
- `src/tui/spiderbyte-tui.ts`: the `SpiderByteTUI` coordinator — wires state, layout, editor, session, SDK events, and dialogs together, and dispatches slash-command handlers. Heavy logic is delegated to `controllers/`, not accumulated here.
...
- `SpiderByteTUI` coordinates; it does not accumulate complex business rules. New logic that can be tested independently should be split into `controllers`, `commands`, `components`, `reverse-rpc`, or `utils` first.
- `controllers` own the heavy, independently-testable slices (event routing, streaming render, session replay, tasks browser, editor keyboard, auth). Event-routing and rendering logic belong here, not on the `SpiderByteTUI` class.
- `commands` only owns slash-command declaration, parsing, and the parsed-result types. The actual execution can be dispatched from `SpiderByteTUI`, but complex logic should continue to sink downward.
...
- `apps/cli` may only use core capabilities through `@spiderbyte/sdk`. Do not import `@spiderbyte/legacy-agent-core` directly in app code.
...
- Functions with no state / UI side effects do not belong as private methods on the `SpiderByteTUI` class; put them in external utils.
