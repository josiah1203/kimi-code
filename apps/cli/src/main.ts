 ...
 * the process itself. It reports here whether a headless (`spyderbyte -p`) run
    ...
    (opts) => {
      void handleMainCommand(opts, version)
        .then(async (outcome) => {
          ...
          // from the run can't wedge a completed `spyderbyte -p` until an external
        ...
        // 0 and `process.exit(1)` never runs — headless (`spyderbyte -p`) failures
          ...
          process.stderr.write(`See log: ${resolveGlobalLogPath(resolveSpiderByteHome())}\n`);
          process.exit(1);
        });
    },
    (entry, args) => {
