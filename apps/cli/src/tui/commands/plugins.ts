 ...
 * global facade; an existing session can use its session facade.
 */
  ...
  try {
    ...
    if (sub === 'marketplace') {
      ...
      await showPluginsPicker(host, {
        ...
        // A caller-supplied marketplace is unverified, so show its entries in
        // the third-party tab. The Open Core checkout has no hosted trust
        // authority for an automatic official-install path.
  ...
  const panel = new PluginsPanelComponent({
    ...
    onCancel: () => {
      host.restoreEditor();
        ...
        // Every marketplace tab needs an explicitly configured catalog;
        // Installed and Custom remain usable when that catalog is unavailable.
...
function isDefaultMarketplaceCatalog(
  ...
  if (env[SPIDERBYTE_PLUGIN_MARKETPLACE_URL_ENV] === undefined) return true;
  return env['SPIDERBYTE_PLUGIN_MARKETPLACE_FROM_DEV_SERVER'] === '1';
}
