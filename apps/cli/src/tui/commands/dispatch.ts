import type { Component, Focusable } from '@spiderbyte/pi-tui';
import type { SpiderByteHarness, Session } from '@spiderbyte/sdk';

import type { ColorToken, ThemeName } from '#/tui/theme';
import { PRODUCT_NAME } from '#/constant/app';

import { LLM_NOT_SET_MESSAGE } from '../constant/spiderbyte-tui';
...
import { handleConnectCommand, handleDisconnectCommand } from './connect';
...
export interface SlashCommandHost {
  ...
  /**
   * Rebuild the plugin slash-command list. With no session this reads the
   * app-global plugin commands instead, so `/plugins`
   * mutations apply before the first session exists.
   */
  refreshPluginCommands(session?: Session): Promise<void>;
  /**
   * Rebuild the skill slash-command list. With no session this reads the
...
/** Lazy-create the session for a slash command that needs one. */
...
async function handleBuiltInSlashCommand(
  ...
  switch (name) {
    ...
    case 'copy':
      ...
      return;
    case 'connect':
      await handleConnectCommand(host);
      return;
    case 'disconnect':
      await handleDisconnectCommand(host);
      return;
    case 'undo':
