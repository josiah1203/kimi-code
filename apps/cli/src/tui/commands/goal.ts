import { ErrorCodes, isSpiderByteError, type PermissionMode } from '@spiderbyte/sdk';
...
async function startGoal(
  ...
  try {
    ...
    if (isSpiderByteError(error) && error.code === ErrorCodes.GOAL_ALREADY_EXISTS) {
      host.showError(
...
async function pauseGoal(host: SlashCommandHost): Promise<void> {
  ...
  try {
    ...
    if (isSpiderByteError(error) && error.code === ErrorCodes.GOAL_NOT_FOUND) {
