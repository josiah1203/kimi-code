...
export async function handleReloadCommand(host: SlashCommandHost): Promise<void> {
  ...
  if (sessionless) {
    // Rebuild workspace-level dynamic commands too, so
  ...
  if (session === undefined) {
    // Still session-less: refresh the lazy defaults too, so
...
function applyRuntimeConfig(host: SlashCommandHost, config: SpiderByteConfig): void {
  host.setAppState({
