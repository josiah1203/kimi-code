...
export async function handleYoloCommand(host: SlashCommandHost, args: string): Promise<void> {
  const session = host.session;
  // Session-less mode is recorded in appState and passed to the
...
export async function handleSecondaryModelCommand(host: SlashCommandHost, args: string): Promise<void> {
  ...
  if (Object.keys(models).length === 0) {
    host.showNotice(
      ...
      'Run /connect to add a local or BYOK provider from a model catalog.',
    );
    return;
  }
...
async function performModelSwitch(
  ...
  try {
    if (session === undefined && runtimeChanged) {
      await host.authFlow.activateModel(alias, effort);
...
async function performSecondaryModelSwitch(
  ...
  let updatedConfig: SpiderByteConfig;
  try {
  ...
  // value: SPIDERBYTE_SECONDARY_MODEL / SPIDERBYTE_SECONDARY_EFFORT override the recipe at
  ...
  if (effective?.model !== undefined && effective.model !== alias) {
    envOverrides.push(`SPIDERBYTE_SECONDARY_MODEL=${effective.model}`);
  }
  if (effective?.defaultEffort !== undefined && effective.defaultEffort !== effort) {
    envOverrides.push(`SPIDERBYTE_SECONDARY_EFFORT=${effective.defaultEffort}`);
  }
...
async function applyPermissionChoice(host: SlashCommandHost, mode: PermissionMode): Promise<void> {
  ...
  try {
    ...
    // Session-less mode is recorded in appState and passed to
