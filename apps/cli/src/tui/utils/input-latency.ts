...
//
// Debug-only input→render latency probe, enabled with SPIDERBYTE_TUI_INPUT_LATENCY=1.
...
// Optional JSONL sink: SPIDERBYTE_TUI_INPUT_LATENCY_LOG=<path> appends one record
...
export function installInputLatencyProbe(tui: TUI): void {
  ...
  const logPath = process.env['SPIDERBYTE_TUI_INPUT_LATENCY_LOG'];
