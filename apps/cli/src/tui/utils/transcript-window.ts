...
export const TRANSCRIPT_MAX_TURNS = readEnvInt('SPIDERBYTE_TUI_MAX_TURNS', 15);

/** Only the most recent E turns are allowed to expand (Ctrl+O). `0` disables expanding. */
export const TRANSCRIPT_EXPAND_TURNS = readEnvInt('SPIDERBYTE_TUI_EXPAND_TURNS', 3);

/** Only trim once the window exceeds maxTurns by this much (avoids churn). */
export const TRANSCRIPT_HYSTERESIS = readEnvInt('SPIDERBYTE_TUI_HYSTERESIS', 5);

/** Keep this many recent steps untouched inside a turn; older steps are merged into a summary. `0` disables merging. */
export const TRANSCRIPT_KEEP_RECENT_STEPS = readEnvInt('SPIDERBYTE_TUI_KEEP_RECENT_STEPS', 30);
...
export const TRANSCRIPT_KEEP_RECENT_ASSISTANT = readEnvInt('SPIDERBYTE_TUI_KEEP_RECENT_ASSISTANT', 20);
...
export const TRANSCRIPT_KEEP_RECENT_ASSISTANT_COMPLETED = readEnvInt(
  'SPIDERBYTE_TUI_KEEP_RECENT_ASSISTANT_COMPLETED',
  2,
);
