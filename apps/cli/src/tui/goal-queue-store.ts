...
function normalizeObjective(value: string): string {
  ...
  if (objective.length === 0) {
    throw new SpiderByteError(ErrorCodes.GOAL_OBJECTIVE_EMPTY, 'Goal objective cannot be empty');
  }
...
function findGoalIndex(file: GoalQueueFile, goalId: string): number {
  ...
  if (index === -1) {
    throw new SpiderByteError(ErrorCodes.GOAL_NOT_FOUND, 'No queued goal found');
  }
  return index;
}
