...
import { Container, Spacer, Text, truncateToWidth, visibleWidth } from '@spiderbyte/pi-tui';
import type { Component, TUI } from '@spiderbyte/pi-tui';
...
import type { TokenUsage } from '@spiderbyte/sdk';
...
export class ToolCallComponent extends Container {
   ...
   * `args.plan` is empty. `SpiderByteTUI` calls `setPlanInfo` with
  ...
  // when SpiderByteTUI routes a `subagent.event` with this tool call
   ...
   * this: plan-file mode leaves `args.plan` empty, so `SpiderByteTUI` fetches
  ...
  // ── Subagent API (called by SpiderByteTUI event routing) ───────────────
