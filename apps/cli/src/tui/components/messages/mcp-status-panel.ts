import type { McpServerInfo } from '@spiderbyte/sdk';
...
export function buildMcpStatusReportLines(options: McpStatusReportOptions): string[] {
  ...
  const statusWidth = Math.max(
    'Status'.length,
    ...servers.map((server) => (STATUS_LABEL[server.status] ?? server.status).length),
  );
  const transportWidth = Math.max(
    'Transport'.length,
    ...servers.map((server) => (server.transport ?? 'unknown').length),
  );
  ...
  for (const server of servers) {
    const status = statusPainter(
      server.status,
    )((STATUS_LABEL[server.status] ?? server.status).padEnd(statusWidth));
    lines.push(
      `  ${value(server.name.padEnd(nameWidth))}  ${status}  ${muted(
        (server.transport ?? 'unknown').padEnd(transportWidth),
