import type { PluginInfo, PluginSummary } from '@spiderbyte/sdk';
...
export function buildPluginsInfoLines(input: PluginsInfoPanelInput): readonly string[] {
  ...
  const trustLine = (() => {
    ...
    if (label === 'official') {
      return `${muted('Trust:')}  ${success(OFFICIAL_BADGE)} ${muted('(source metadata only)')}`;
    }
    if (label === 'curated') {
      return `${muted('Trust:')}  ${primary(CURATED_BADGE)} ${muted('(source metadata only)')}`;
    }
