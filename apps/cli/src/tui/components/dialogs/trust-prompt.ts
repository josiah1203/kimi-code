...
const OPTIONS: readonly TrustPromptOption[] = [
  ...
  },
  {
    value: 'distrust',
    label: "Don't trust",
    description: 'Exit SpiderByte. Asked again next launch.',
  },
];
  ...
  render(width: number): string[] {
    ...
    const notice =
      this.opts.gatedMcpServers.length > 0
        ? `SpiderByte loads project-level MCP servers (.mcp.json, .spiderbyte/mcp.json) only in trusted folders. They run as local processes on your machine. This folder defines: ${this.opts.gatedMcpServers.join(', ')}.`
        : 'SpiderByte loads project-level MCP servers (.mcp.json, .spiderbyte/mcp.json) only in trusted folders. They run as local processes on your machine.';
