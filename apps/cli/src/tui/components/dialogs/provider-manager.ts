 ...
 * Provider rows represent local and BYOK configuration only.
 *
 * Keyboard:
 *   - ↑ / ↓             move highlight
 *   - ← / → · PgUp/PgDn page
 ...
 * through callbacks. The host (`SpiderByteTui`) is responsible for performing
 * the harness / config mutations and then pushing a fresh snapshot via
 * `setOptions`.
 */

import type { ProviderConfig } from '@spiderbyte/sdk';
import {
  getOpenPlatformById,
  isOpenPlatformId,
...
// Mirrors `readCustomRegistrySource` in `spiderbyte-tui.ts`. We can't import
