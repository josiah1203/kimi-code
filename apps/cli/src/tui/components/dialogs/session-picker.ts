...
export class SessionPickerComponent extends Container implements Focusable {
  ...
  private renderSessionCard(
    ...
    const imported = session.metadata?.['imported'] === true;
    const baseTitle = (session.title ?? session.id).trim() || session.id;
    const rawTitle = imported ? `[imported] ${baseTitle}` : baseTitle;
