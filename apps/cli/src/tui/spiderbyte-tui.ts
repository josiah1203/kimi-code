import { effectiveModelAlias, log } from '@spiderbyte/sdk';
import type {
  ApprovalRequest,
  ApprovalResponse,
  BackgroundTaskInfo,
...
import { combineStartupNotice, isProviderConfigurationRequiredError } from './utils/startup';
...
export type {
  SpiderByteTUIOptions,
  ProgressSpinnerHandle,
  TUIStartupOptions,
  TUIStartupState,
} from './types';

export interface SpiderByteTUIStartupInput {
...
function createInitialAppState(input: SpiderByteTUIStartupInput): AppState {
...
export class SpiderByteTUI {
  ...
  private skillCommands: readonly SpiderByteSlashCommand[] = [];
  readonly skillCommandMap = new Map<string, string>();
  private pluginCommands: readonly SpiderByteSlashCommand[] = [];
  ...
  track(event: string, properties?: Parameters<SpiderByteHarness['track']>[1]): void {
    this.harness.track(event, properties);
  }

  constructor(harness: SpiderByteHarness, startupInput: SpiderByteTUIStartupInput) {
    this.harness = harness;
    const tuiOptions: SpiderByteTUIOptions = {
  ...
  private getSlashCommands(): readonly SpiderByteSlashCommand[] {
  ...
  async refreshSkillCommands(session?: SkillListSession): Promise<void> {
    if (session === undefined) {
      // Skills live on the workspace handler, not the session, so
      ...
      try {
        ...
        // Skill discovery is optional; a provider/configuration failure should
        // not prevent the canonical local TUI from starting.
      }
      return;
    }
  ...
  async refreshPluginCommands(session?: Session): Promise<void> {
    if (session === undefined) {
      // Enabled plugin commands are an app-global live view,
      ...
      try {
        ...
        // Plugin discovery is optional; keep the local TUI usable when it is
        // unavailable before the first session.
      }
      return;
    }
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      // Debug-only input→render latency overlay (SPIDERBYTE_TUI_INPUT_LATENCY=1).
      if (process.env['SPIDERBYTE_TUI_INPUT_LATENCY']) installInputLatencyProbe(this.state.ui);
  ...
  private async finishStartup(shouldReplayHistory: boolean): Promise<void> {
    ...
    // warning yellow at boot; `run-prompt`/`run-print` print them to
  ...
  private async init(): Promise<boolean> {
    ...
    try {
      if (isResumeStartup) {
        ...
        // Lazy session creation: start session-less and create the
      ...
      if (!isProviderConfigurationRequiredError(error)) throw error;
      this.authFlow.enterProviderConfigurationRequiredStartupState();
      return false;
    }
  ...
  /**
   * Seed appState with the config defaults the Agent Core runtime applies at
  ...
  private async createSessionFromCurrentState(bindStartupAgent = false): Promise<Session> {
    ...
    const explicitPlanMode = this.session !== undefined
  ...
  /**
   * Lazy-create the session on first use during session-less startup.
  ...
  private async showResumeOtherWorkDirHint(session: SessionRow): Promise<void> {
    ...
    const command = `cd ${quoteShellArg(session.work_dir)} && spyderbyte --resume ${quoteShellArg(session.id)}`;
  ...
  /**
   * SpiderByte Agent Core startup gate: before any session is created, ask whether to
