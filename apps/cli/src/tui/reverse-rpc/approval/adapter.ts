import type { ApprovalRequest, ApprovalResponse, ToolInputDisplay } from '@spiderbyte/sdk';
...
export function adaptApprovalRequest(event: ApprovalRequest): ApprovalPanelData {
  ...
  return {
    ...
    tool_name: event.toolName ?? toolNameFromAction(event.action),
    action: event.action ?? '',
...
function toolNameFromAction(action: string | undefined): string {
  return action === undefined || action.length === 0 ? 'tool' : action;
}
