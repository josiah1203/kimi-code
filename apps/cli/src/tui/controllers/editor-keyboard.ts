...
import { compressImageForModel, persistOriginalImage, sessionMediaOriginalsDir } from '@spiderbyte/sdk';
...
export interface EditorKeyboardHost {
  ...
  /**
   * The host's harness (SpiderByteTUI always has one). Its `imageLimits` drives
   * paste-time image compression; hosts without one fall back to the
   * env/built-in default.
   */
  harness?: SpiderByteHarness | undefined;
  ...
  install(): void {
    ...
    editor.onShiftTab = () => {
      ...
      if (host.session === undefined) {
        // Lazy-create the session, then toggle — the same
        // path /plan takes.
