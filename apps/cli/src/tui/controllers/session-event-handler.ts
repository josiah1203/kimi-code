...
import {
  PROVIDER_CONFIGURATION_REQUIRED_CODE,
  PROVIDER_CONFIGURATION_REQUIRED_NOTICE,
...
export class SessionEventHandler {
  ...
  private async handlePlatformRunEvent(
    ...
    // Ordinary SpiderByte prompt Runs remain invisible in the platform projection;
  ...
  private handleStepCompleted(event: TurnStepCompletedEvent): void {
    ...
    const detail = this.isAnthropicSessionActive()
      ? 'If this limit is wrong for your model, set `max_output_size` on the model alias in your spiderbyte config.'
      : undefined;
    this.host.showNotice(title, detail);
  }

  private maybeShowDebugTiming(event: TurnStepCompletedEvent): void {
    if (process.env['SPIDERBYTE_DEBUG'] !== '1') return;
  ...
  private handleThinkingDelta(event: ThinkingDeltaEvent): void {
    ...
    // Encrypted / redacted reasoning (e.g. An external provider over the Anthropic-compatible
  ...
  private handleSessionError(event: ErrorEvent): void {
    ...
    this.host.streamingUI.resetToolUi();
    this.host.streamingUI.finalizeLiveTextBuffers('idle');
    if (event.code === PROVIDER_CONFIGURATION_REQUIRED_CODE) {
      this.host.showError(PROVIDER_CONFIGURATION_REQUIRED_NOTICE);
      return;
    }
