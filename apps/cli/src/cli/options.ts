...
export const OUTPUT_FORMAT_ENV = 'SPIDERBYTE_MODEL_OUTPUT_FORMAT';
 ...
 *
 * Precedence: explicit `--output-format` flag → `SPIDERBYTE_MODEL_OUTPUT_FORMAT` env
 * (prompt mode only) → `text`. The env var is ignored outside prompt mode so an
 * ambient value never affects interactive `spyderbyte`. An invalid env value fails
...
export function validateOptions(
  ...
  // Validate `SPIDERBYTE_MODEL_OUTPUT_FORMAT` eagerly in prompt mode so a typo fails
