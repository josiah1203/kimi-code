...
import { log, type Logger } from '@spiderbyte/sdk';
import type { TelemetryProperties } from '@spiderbyte/telemetry';

import {
  SPIDERBYTE_OFFICIAL_INSTALL_URL,
...
export function installCommandFor(
  ...
  switch (source) {
    ...
    case 'homebrew':
      return 'brew upgrade spiderbyte';
    case 'native':
...
export function spawnForSource(
  ...
  switch (source) {
    ...
    case 'homebrew':
      return { cmd: 'brew', args: ['upgrade', 'spiderbyte'] };
    case 'native':
...
const THIRD_PARTY_SOURCE_NOTE =
  ...
  `For the latest updates, use the official installer: ${SPIDERBYTE_OFFICIAL_INSTALL_URL}\n`;
...
function renderBackgroundInstallSuccessNotice(version: string): string {
  ...
  return `SpiderByte updated to ${displayVersion}\nChangelog: ${CHANGELOG_URL}\n`;
}
...
/**
 * `SPIDERBYTE_NO_AUTO_UPDATE` (or the legacy `SPIDERBYTE_CLI_NO_AUTO_UPDATE` alias)
...
function isAutoUpdateDisabledByEnv(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  return truthy(env['SPIDERBYTE_NO_AUTO_UPDATE']) || truthy(env['SPIDERBYTE_CLI_NO_AUTO_UPDATE']);
}
