...
import { resolveSpiderByteHome } from '@spiderbyte/sdk';
...
export type PassiveUpdateReason =
  ...
  | 'eligible'
  /** SPIDERBYTE_EXPERIMENTAL_FLAG is on: rollout skipped, newest always visible. */
  | 'experimental';
 ...
 *
 * `spyderbyte upgrade` must NOT go through this gate — it selects directly from the
 * raw latest version.
 */
...
export function resolveUpdateDeviceId(): string {
  return readSpiderByteDeviceId(resolveSpiderByteHome()) ?? randomUUID();
}
 ...
 * truthy values as `SPIDERBYTE_NO_AUTO_UPDATE`) — the update preflight runs
 * before the harness exists, so the core flag registry is not consulted.
 * `SPIDERBYTE_NO_AUTO_UPDATE` still wins: disabling updates beats opting in.
 */
export function isRolloutBypassedByExperimentalEnv(
  env: Readonly<Record<string, string | undefined>> = process.env,
): boolean {
  const value = (env['SPIDERBYTE_EXPERIMENTAL_FLAG'] ?? '').trim().toLowerCase();
