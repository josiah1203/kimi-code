...
export interface FetchLatestResult {
  /** Raw newest version — what `spyderbyte upgrade` installs, never rollout-gated. */
...
/**
 * Fetch the latest published SpiderByte version from the CDN.
 *
...
export async function fetchLatestVersionFromCdn(
  ...
  const response = await fetchWithTimeout(fetchImpl, SPIDERBYTE_CDN_LATEST_URL);
  if (!response.ok) {
...
async function fetchUpdateManifestFromCdn(fetchImpl: typeof fetch): Promise<UpdateManifest> {
  const response = await fetchWithTimeout(fetchImpl, SPIDERBYTE_CDN_LATEST_JSON_URL);
  if (!response.ok) {
