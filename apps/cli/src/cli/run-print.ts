 ...
 * runner talks to SpiderByte Agent Core's native DI services directly — no
 ...
 *
 * Selected by the canonical `runPrompt` dispatcher.
 */
...
import { createSpiderByteDefaultHeaders, createSpiderByteDeviceId } from '@spiderbyte/oauth';
...
export async function runPrint(
  ...
  const identity = createSpiderByteHostIdentity(version);
  const hostHeaders = createSpiderByteDefaultHeaders({ homeDir, ...identity });
  ...
  try {
    ...
    stderr.write(`${formatPlatformModeDiagnostic(platformFeature)}\n`);
  } catch {
...
async function findCanonicalDefault(
  ...
  try {
    ...
    // Minimal hosts may not contribute platform connections. The caller then
    // uses the explicitly configured model path and retains the no-model
    // diagnostic when none is configured.
    return undefined;
  }
}
