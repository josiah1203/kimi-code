/**
 * `spyderbyte web` — run the local REST/WebSocket API in the foreground.
 *
...
import { createServerLogger, startServer, type ServerLogger } from '@spiderbyte/kap-server';
import { shutdownTelemetry, track } from '@spiderbyte/telemetry';
...
export interface WebCommandDeps {
   ...
   * a token, the ready banner prints it and the opened API URL carries it in
...
/**
 * Build a browser-openable API URL, carrying the bearer token in the URL fragment.
 *
 ...
 * logged by proxies. An external browser client may read it from `location.hash`.
 */
...
export function buildWebCommand(cmd: Command): Command {
  return cmd
    ...
    .option('--no-open', 'Do not open the API origin in the default browser.', true)
...
export async function handleWebCommand(
  ...
  await run(parsed, {
    onReady: (origin) => {
      ...
      // It is printed in the ready banner and rides in the opened API
...
function formatReadyLine(
  ...
  const notice = dangerousBypassAuth
    ...
    : '';
  return `${notice}SpiderByte server: ${buildOpenableUrl(origin, token)}\n`;
}
...
/**
 * `spyderbyte web` — runs the local server in-process, attached to the current
...
async function runServerInProcess(
  ...
  const v2 = await startServer({
    host: options.host,
    port: options.port,
    // Report the CLI's product version as `server_version` (/meta, browser clients)
    // rather than kap-server's private package version.
    serverVersion: version,
    // The CLI's host identity: feeds the engine's bootstrap client identity
    // and the derived outbound headers (User-Agent + X-Msh-*), so browser
    // clients and model / WebSearch requests carry the CLI identity. The
    // `web` User-Agent suffix distinguishes browser traffic from direct CLI
    ...
    // Attach the engine's telemetry appender (still gated by the config
    // `telemetry` toggle).
    telemetry: true,
  });
  logger.info('serving the local REST/WS API; the browser client is external to this checkout');
  running = {
