/**
 * `spyderbyte web rotate-token` — generate a new persistent server token.
 *
 * Rewrites `<SPIDERBYTE_HOME>/server.token` (0600, atomic). The previous token
...
import { getLiveServerInstance, rotateServerToken } from '@spiderbyte/kap-server';
