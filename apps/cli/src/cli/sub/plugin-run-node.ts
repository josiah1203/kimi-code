...
export async function runPluginNodeEntry(entry: string, args: readonly string[]): Promise<void> {
  const pluginRoot = process.env['SPIDERBYTE_PLUGIN_ROOT'];
  if (pluginRoot === undefined || pluginRoot.trim().length === 0) {
    throw new Error('SPIDERBYTE_PLUGIN_ROOT is required to run a plugin node entry.');
  }
  ...
  if (!isWithin(entryReal, rootReal)) {
    throw new Error(`Plugin node entry must be inside SPIDERBYTE_PLUGIN_ROOT: ${entry}`);
  }
