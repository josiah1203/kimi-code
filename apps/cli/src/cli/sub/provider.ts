/**
 * `spyderbyte provider` sub-command — non-interactive provider management.
 *
 * Mirrors the TUI `/provider` flow (apps/cli/src/tui/commands/provider.ts)
...
export interface ProviderDeps {
  readonly getHarness: () => SpiderByteHarness;
...
type WritableSpiderByteConfig = {
  -readonly [Key in keyof SpiderByteConfig]: SpiderByteConfig[Key];
};
...
export async function handleProviderAdd(
  ...
  if (apiKey === undefined) {
    deps.stderr.write(
      'Missing API key. Pass --api-key <key> or set SPIDERBYTE_REGISTRY_API_KEY.\n',
    );
    deps.exit(1);
  }
...
export async function handleCatalogAdd(
  ...
  if (opts.defaultModel !== undefined && !models.some((m) => m.id === opts.defaultModel)) {
    deps.stderr.write(
      `Model "${opts.defaultModel}" is not in provider "${providerId}". Run "spyderbyte provider catalog list ${providerId}" to see available ids.\n`,
    );
    deps.exit(1);
  }
...
async function loadCatalogOrExit(deps: ProviderDeps, url: string): Promise<Catalog> {
  try {
    const loaded = await fetchCatalogOrBuiltIn(url, { userAgent: createSpiderByteUserAgent() });
...
export function registerProviderCommand(parent: Command, deps?: Partial<ProviderDeps>): void {
  ...
  provider
    ...
    .option('--api-key <key>', 'Registry API key. Falls back to SPIDERBYTE_REGISTRY_API_KEY.')
  ...
  catalog
    ...
    .option('--api-key <key>', 'API key for the provider. Falls back to SPIDERBYTE_REGISTRY_API_KEY.')
...
function resolveDeps(overrides: Partial<ProviderDeps> = {}): ResolvedProviderDeps {
  ...
  return {
    getHarness:
      ...
      (() => {
        harness ??= createSpiderByteHarness({ identity });
        return harness;
      }),
...
function resolveApiKey(flag: string | undefined, env: NodeJS.ProcessEnv): string | undefined {
  ...
  const fromEnv = env['SPIDERBYTE_REGISTRY_API_KEY'];
  if (typeof fromEnv === 'string' && fromEnv.length > 0) return fromEnv;
  return undefined;
}

function asManaged(config: SpiderByteConfig): SpiderByteConfigShape {
  return config as unknown as SpiderByteConfigShape;
}

function providerSourceLabel(provider: SpiderByteConfig['providers'][string]): string {
  const source = provider.source;
  if (source !== undefined) {
