...
export function registerExportCommand(parent: Command, deps?: Partial<ExportDeps>): void {
  parent
    ...
    .option(
      ...
      'Skip bundling the active global diagnostic log (~/.spiderbyte/logs/spiderbyte.log, not rotated .1 files). By default the global log is included.',
    )
...
function createDefaultExportDeps(overrides: Partial<ExportDeps> = {}): ExportDeps & {
  ...
  const getHarness = (): SpiderByteHarness => {
    const currentTelemetryBootstrap = getTelemetryBootstrap();
    harness ??= createSpiderByteHarness({
