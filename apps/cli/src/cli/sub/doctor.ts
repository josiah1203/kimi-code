...
export interface DoctorDeps {
  ...
  readonly configRpc?: SpiderByteConfigRpc;
...
export function registerDoctorCommand(parent: Command, deps?: Partial<DoctorDeps>): void {
  const doctor = parent
    .command('doctor')
    .description('Validate SpiderByte configuration files.')
    .action(async () => {
...
function resolveDeps(deps: Partial<DoctorDeps> | DoctorDeps | undefined): ResolvedDoctorDeps {
  ...
  const getConfigRpc = (): SpiderByteConfigRpc => {
    configRpc ??= createSpiderByteConfigRpc();
    return configRpc;
  };
  ...
  return {
    ...
    validateConfigToml:
      ...
      (async (text, filePath) => {
        const { validateConfigToml } = await import('#/cli/validate-config');
...
function formatFailure(results: readonly CheckResult[], issueCount: number): string {
  return [
    `SpiderByte doctor found ${String(issueCount)} ${issueCount === 1 ? 'issue' : 'issues'}.`,
    '',
    ...formatResults(results),
    '',
  ].join('\n');
...
function findValidationIssues(error: unknown): readonly SpiderByteConfigValidationIssue[] | undefined {
...
function isValidationIssueArray(value: unknown): value is readonly SpiderByteConfigValidationIssue[] {
  return Array.isArray(value) && value.every(isValidationIssue);
}

function isValidationIssue(value: unknown): value is SpiderByteConfigValidationIssue {
