...
import {
  createSpiderByteUserAgent as createOAuthUserAgent,
  SPIDERBYTE_PLATFORM,
...
import { SPIDERBYTE_BUILD_INFO } from './build-info';
...
export function getVersion(): string {
  if (SPIDERBYTE_BUILD_INFO.version !== undefined) {
    return SPIDERBYTE_BUILD_INFO.version;
  }
...
export function createSpiderByteHostIdentity(version = getVersion()): SpiderByteHostIdentity {
  return {
...
/**
 * Product User-Agent (`spiderbyte-cli/<version>`) for ad-hoc outbound fetches
 * that don't go through the provider pipeline (registry / catalog imports).
 */
export function createSpiderByteUserAgent(version = getVersion()): string {
  return createOAuthUserAgent(createSpiderByteHostIdentity(version));
}
