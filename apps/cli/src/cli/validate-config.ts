/**
 * V2 config.toml validation for `spyderbyte doctor`.
 *
 * Loaded lazily (dynamic import) by the doctor command on the default
 * SpiderByte Agent Core path, so the canonical module graph stays off the legacy doctor path.
 ...
 *
 * Semantics deliberately mirror the canonical engine rather than v1:
...
import { collectKeyDeprecations } from '@spiderbyte/agent-core/app/config/deprecations';
import {
  camelToSnake,
  describeTomlSyntaxError,
  isPlainObject,
  transformTomlData,
} from '@spiderbyte/agent-core/app/config/toml';

/**
 * Top-level domains the canonical engine reads via `IConfigService.get` / `inspect`
 ...
 * `SpiderByteConfigValidationIssue` list), so the doctor formatter renders canonical
...
class V2ConfigValidationError extends Error {
  ...
  constructor(issues: readonly V2ConfigValidationIssue[]) {
    super('canonical config validation failed');
    this.details = { validationIssues: issues };
  }
}

/**
 * Validate `text` as config.toml against the canonical engine's section registry.
...
export function validateConfigToml(
  ...
  if (unknownKeys.length > 0) {
    warnings.push(
      `Unknown top-level ${unknownKeys.length === 1 ? 'key' : 'keys'} ignored by the canonical engine: ${unknownKeys.join(', ')}.`,
    );
  }
