...
/**
 * Generic client for an explicitly configured public client-configs endpoint:
 * `POST {SPIDERBYTE_CLIENT_CONFIGS_URL}/client_configs {"name": "<config name>"}` returns
...
export interface ClientConfigFetchOptions {
  ...
  /** Explicit endpoint; Open Core never assumes a hosted SpiderByte service. */
...
export async function fetchClientConfig<S extends z.ZodType>(
  ...
  try {
    const baseUrl = options.baseUrl ?? process.env['SPIDERBYTE_CLIENT_CONFIGS_URL'];
    if (baseUrl === undefined || baseUrl.trim().length === 0) return undefined;
    const response = await fetchFn(`${baseUrl.replace(/\/+$/, '')}${CLIENT_CONFIGS_PATH}`, {
      method: 'POST',
      headers,
