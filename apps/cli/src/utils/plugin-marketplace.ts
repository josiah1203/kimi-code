...
export async function loadPluginMarketplace(
  ...
  const configuredSource = options.source ?? process.env[SPIDERBYTE_PLUGIN_MARKETPLACE_URL_ENV];
  const location = resolveMarketplaceLocation(
    configuredSource ?? SPIDERBYTE_PLUGIN_MARKETPLACE_URL,
    options.workDir,
  );
...
function resolveMarketplaceLocation(source: string, workDir: string): MarketplaceLocation {
  ...
  if (trimmed.length === 0) {
    throw new Error(`${SPIDERBYTE_PLUGIN_MARKETPLACE_URL_ENV} cannot be empty.`);
  }
