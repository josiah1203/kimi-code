...
//
// Debug-only startup phase tracer, enabled with SPIDERBYTE_STARTUP_TRACE=1.
// Each call appends one `<elapsed-ms> <label>` line to the trace file
// (default /tmp/spiderbyte-startup-trace.log, override with
// SPIDERBYTE_STARTUP_TRACE_LOG=<path>), so a slow or BLOCKING startup phase
...
const enabled = process.env['SPIDERBYTE_STARTUP_TRACE'] !== undefined && process.env['SPIDERBYTE_STARTUP_TRACE'] !== '';
const logPath = process.env['SPIDERBYTE_STARTUP_TRACE_LOG'] ?? '/tmp/spiderbyte-startup-trace.log';
