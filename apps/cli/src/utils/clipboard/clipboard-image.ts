 ...
 *
 * SpiderByte Agent Core's LLM pipeline only accepts PNG/JPEG/GIF/WebP, and the
...
function readClipboardImageViaPowerShell(): ClipboardImage | null {
  const tmpFile = join(tmpdir(), `spiderbyte-wsl-clip-${randomUUID()}.png`);
  try {
    ...
    const psScript = [
      ...
      '$path = $env:SPIDERBYTE_WSL_CLIPBOARD_IMAGE_PATH',
    ...
    const result = runCommand('powershell.exe', ['-NoProfile', '-Command', psScript], {
      ...
      env: { ...process.env, SPIDERBYTE_WSL_CLIPBOARD_IMAGE_PATH: winPath },
    });
