...
import { getSearchWorkerRuntimeState } from '@spiderbyte/kap-server/search-worker-runtime';
...
const smokePackages = ['@mariozechner/clipboard', '@spiderbyte/pi-tui'];
...
export function runNativeAssetSmokeIfRequested(): boolean {
  if (process.env['SPIDERBYTE_NATIVE_ASSET_SMOKE'] !== '1') return false;
  void runSmoke().then(
    () => process.exit(0),
    (error: unknown) => {
