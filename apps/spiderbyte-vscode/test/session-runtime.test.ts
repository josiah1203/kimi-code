 ...
 * Run: pnpm exec vitest run --config apps/spiderbyte-vscode/vitest.config.ts apps/spiderbyte-vscode/test/session-runtime.test.ts
 */
...
import type { CompatibilityApprovalFlags } from "../src/runtime/compatibility-approval";
...
const DEFAULT_COMPATIBILITY_APPROVAL: CompatibilityApprovalFlags = { yolo: false, afk: false };
...
function createRuntime(compatibilityApproval = DEFAULT_COMPATIBILITY_APPROVAL) {
...
describe("session runtime (adapts one SDK session for subscribed Webviews)", () => {
  ...
  it("converts compatibility media keys when a prompt crosses the SDK boundary", async () => {
    const { runtime, sdk } = createRuntime();
  ...
  it("broadcasts a compatibility tool call when an SDK tool starts", () => {
    const { sdk, broadcasts } = createRuntime();
  ...
  it("converts compatibility media keys when steering an active response", async () => {
    const { runtime, sdk } = createRuntime();
    void runtime.prompt("hello");
  ...
  it("forwards SDK approval requests to the Webview in compatibility yolo mode", async () => {
  ...
  it("restores core permission when a compatibility flag cannot be persisted", async () => {
    const { runtime, sdk } = createRuntime();
    sdk.rejectNextMetadataUpdate(new Error("state is read-only"));

    await expect(runtime.toggleCompatibilityApproval("afk")).rejects.toThrow("state is read-only");

    expect(sdk.setPermissions).toEqual(["auto", "manual"]);
    expect(runtime.compatibilityApprovalFlags).toEqual({ yolo: false, afk: false });
  });
  ...
  it("keeps SDK questions interactive in compatibility yolo mode", async () => {
    const { runtime, sdk, broadcasts } = createRuntime({ yolo: true, afk: false });
    const pending = sdk.requestQuestion({
