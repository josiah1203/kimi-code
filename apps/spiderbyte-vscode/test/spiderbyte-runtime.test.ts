/**
 * Scenario: the VS Code host owns one SpiderByte harness and routes Webviews to shared SDK sessions.
 * Responsibilities: create/resume/switch, per-session settings, multi-view ownership, detach, and disposal.
 * Wiring: SpiderByteRuntime and SessionRuntime are real; in-memory Session/SpiderByteHarness fakes form the public SDK boundary.
 * Run: pnpm exec vitest run --config apps/spiderbyte-vscode/vitest.config.ts test/spiderbyte-runtime.test.ts
 */
...
import { SpiderByteRuntime, type OpenSessionOptions } from "../src/runtime/spiderbyte-runtime";
...
function createFakeSession(
  ...
  let status: SessionStatus = {
    model: initial.model ?? "spiderbyte-test",
...
function createFakeHarness(
  ...
  } as unknown as SpiderByteHarness;
...
describe("SpiderByte runtime (owns shared SDK sessions for Webviews)", () => {
  it("forwards the requested settings when creating an SDK session", async () => {
    ...
    const opened = await runtime.openSession(
      openOptions({ model: "spiderbyte-k2", effort: "high", yoloMode: true }),
    );
  ...
  it("announces the session's actual status to the attaching view so the display matches it", async () => {
    ...
    expect(broadcasts).toContainEqual({
      ...
      data: {
        type: "StatusUpdate",
        payload: { model: "spiderbyte-test", thinking_effort: "max", plan_mode: true },
        _sessionId: "saved-1",
      },
      webviewId: "view-1",
    });
  ...
  it("lets the global yolo setting override a persisted off flag on resume", async () => {
    ...
    expect(opened.compatibilityApprovalFlags).toEqual({ yolo: true, afk: false });
  });
  ...
  it("lets the global yolo setting disable a persisted session yolo flag on resume", async () => {
    ...
    expect(opened.compatibilityApprovalFlags).toEqual({ yolo: false, afk: false });
  });
  ...
  it("keeps the persisted afk flag while applying the global yolo setting on resume", async () => {
    ...
    expect(opened.compatibilityApprovalFlags).toEqual({ yolo: true, afk: true });
  });
  ...
  it("changes the setting-backed yolo flag without clearing session afk", async () => {
    ...
    const opened = await runtime.openSession(openOptions());
    await opened.toggleCompatibilityApproval("afk");
