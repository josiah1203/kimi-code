/**
 * Scenario: the VS Code host and another Node SDK client share one in-process SpiderByte home.
 ...
 * Wiring: SpiderByteRuntime, SpiderByteHarness, core, storage, and HTTP provider adapter are real; only the remote provider is local.
 * Run: pnpm --filter spiderbyte exec vitest run test/spiderbyte-harness.integration.test.ts
 */
...
} from "../shared/spiderbyte-sdk-types";
...
import { SpiderByteRuntime } from "../src/runtime/spiderbyte-runtime";
...
interface RuntimeRig {
  ...
  readonly runtime: SpiderByteRuntime;
...
async function createRuntimeRig(extraAliases: readonly string[] = []): Promise<RuntimeRig> {
  const rootDir = await mkdtemp(join(tmpdir(), "spiderbyte-vscode-harness-"));
  ...
  const runtime = new SpiderByteRuntime({
    version,
    homeDir,
...
async function createPlainHarness(homeDir: string): Promise<SpiderByteHarness> {
  const harness = createSpiderByteHarness({
    homeDir,
    identity: { productName: "spiderbyte-cli", version: "test", platform: "spiderbyte_code_cli" },
  });
...
async function createMcpHandlerRig(): Promise<McpHandlerRig> {
  const homeDir = await mkdtemp(join(tmpdir(), "spiderbyte-vscode-mcp-handler-"));
...
describe("VS Code SpiderByte harness integration (shares one in-process SDK home)", () => {
  ...
  it("backfills approval flags for a session migrated before the metadata field existed", async () => {
    const rig = await createRuntimeRig();
    const compatibilitySessionDir = join(rig.workDir, "compatibility-session");
    await mkdir(compatibilitySessionDir);
    await writeFile(
      join(compatibilitySessionDir, "state.json"),
    ...
    const migrated = await plain.createSession({
      ...
      workDir: rig.workDir,
      metadata: { spiderbyte_cli_source_path: compatibilitySessionDir },
    });
    await migrated.close();

    const resumed = await openRuntimeSession(rig, migrated.id);

    expect(resumed.compatibilityApprovalFlags).toEqual({ yolo: false, afk: true });
  ...
  it("reports corrupt compatibility approval state and still opens the migrated session", async () => {
    const rig = await createRuntimeRig();
    const compatibilitySessionDir = join(rig.workDir, "corrupt-compatibility-session");
    await mkdir(compatibilitySessionDir);
    await writeFile(join(compatibilitySessionDir, "state.json"), "{not-json", "utf8");
    const plain = await createPlainHarness(rig.homeDir);
    const migrated = await plain.createSession({
    ...
    expect(resumed.compatibilityApprovalFlags).toEqual({ yolo: false, afk: false });
    expect(rig.logs).toContainEqual({
      message: "Unable to restore compatibility session approval settings",
  ...
  it("keeps /yolo and /afk independent when they are combined", async () => {
    ...
    expect(runtime.compatibilityApprovalFlags).toEqual({ yolo: true, afk: false });
    await expect(runtime.session.getStatus()).resolves.toMatchObject({ permission: "yolo" });

    await runSlash(runtime, "/afk");
    expect(runtime.compatibilityApprovalFlags).toEqual({ yolo: true, afk: true });
  ...
  it("applies the global yolo setting when a closed VS Code session reopens", async () => {
    ...
    expect(reopened.compatibilityApprovalFlags).toEqual({ yolo: false, afk: false });
    ...
    expect(yoloReopened.compatibilityApprovalFlags).toEqual({ yolo: true, afk: false });
  ...
  it("exports current context as Markdown under the workspace", async () => {
    ...
    expect(markdown).toContain("# SpiderByte Session Export");
