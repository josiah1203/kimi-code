 ...
 * Run: pnpm --filter spiderbyte exec vitest run --config vitest.config.ts test/bridge-handler.test.ts
 */
...
vi.mock("@spiderbyte/sdk", async (importOriginal) => {
  const original = await importOriginal<typeof import("@spiderbyte/sdk")>();
  return { ...original, createSpiderByteHarness: () => host.harness };
});
...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), "spiderbyte-vscode-bridge-"));
...
describe("Webview RPC boundary (validates requests before host dispatch)", () => {
  ...
  it("does not expose the session storage path when listing sessions", async () => {
    host.harness.listSessions.mockResolvedValueOnce([
      {
        id: "session-1",
        workDir: root,
        sessionDir: "/private/spiderbyte/sessions/session-1",
        updatedAt: 123,
        title: "Visible title",
      },
    ] as never);
    ...
    const result = await bridge.handle(
      { id: "rpc-1", method: Methods.GetSpiderByteSessions },
      "view-1",
    );
    ...
    expect(JSON.stringify(result)).not.toContain("/private/spiderbyte/sessions");
  });
  ...
  it("does not expose the session storage path when forking a session", async () => {
    ...
    const target = {
      id: "session-2",
      workDir: root,
      sessionDir: "/private/spiderbyte/sessions/session-2",
      updatedAt: 124,
    };
    ...
    const result = await bridge.handle(
      {
        id: "rpc-1",
        method: Methods.ForkSpiderByteSession,
  ...
  it("keeps conversation history available when its baseline snapshot disappears", async () => {
    ...
    const result = await bridge.handle(
      {
        id: "rpc-1",
        method: Methods.LoadSpiderByteSessionHistory,
        params: { spiderbyteSessionId: "session-1" },
      },
      "view-1",
    );
...
describe("Webview config saves (thinking effort persistence parity with the TUI)", () => {
  ...
  function mockConfig(thinking?: { enabled: boolean; effort?: string }) {
    host.harness.getConfig.mockResolvedValue({
      defaultModel: "spiderbyte/reasoning",
      thinking,
      models: { "spiderbyte/reasoning": effortModel },
    } as never);
  }
  ...
  it("persists a non-top effort as the global default", async () => {
    ...
    const result = await bridge.handle(
      { id: "rpc-1", method: Methods.SaveConfig, params: { model: "spiderbyte/reasoning", thinking: true, effort: "high" } },
      "view-1",
    );
  ...
  it("keeps the model's top declared tier session-only", async () => {
    ...
    await bridge.handle(
      { id: "rpc-1", method: Methods.SaveConfig, params: { model: "spiderbyte/reasoning", thinking: true, effort: "max" } },
      "view-1",
    );
  ...
  it("leaves the stored effort alone when the pick re-confirms the active effort", async () => {
    ...
    await bridge.handle(
      { id: "rpc-1", method: Methods.SaveConfig, params: { model: "spiderbyte/reasoning", thinking: true, effort: "high", effortChanged: false } },
      "view-1",
    );
...
function createResumedSession(id: string, workDir: string) {
  ...
  const summary = {
    id,
    workDir,
    sessionDir: join("/private/spiderbyte/sessions", id),
    createdAt: 1,
    updatedAt: 2,
