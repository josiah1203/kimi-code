...
const EXTENSION_ID = "spiderbyte-ai.spiderbyte";
const EXPECTED_COMMANDS = [
  "spiderbyte.clearAllState",
  "spiderbyte.focusInput",
  "spiderbyte.insertMention",
  "spiderbyte.logout",
  "spiderbyte.migrateCompatibilityData",
  "spiderbyte.newConversation",
  "spiderbyte.openInSideBar",
  "spiderbyte.openInTab",
  "spiderbyte.resetSpiderByte",
  "spiderbyte.showLogs",
];

exports.run = async function run() {
  const isolatedHome = process.env.SPIDERBYTE_VSCODE_SMOKE_OS_HOME;
  ...
  assert.ok(process.env.SPIDERBYTE_HOME, "SPIDERBYTE_HOME must point at the isolated test home");
  ...
  const config = vscode.workspace.getConfiguration("spiderbyte");
  ...
  await vscode.commands.executeCommand("spiderbyte.openInTab");
  await waitFor(() => {
    return vscode.window.tabGroups.all.some((group) =>
      group.tabs.some((tab) =>
        tab.input instanceof vscode.TabInputWebview && isSpiderBytePanelViewType(tab.input.viewType)));
  }, 5_000, () => `SpiderByte Webview tab did not open; tabs=${describeTabs()}`);

  await vscode.commands.executeCommand("spiderbyte.showLogs");
  await vscode.commands.executeCommand("spiderbyte.resetSpiderByte");
  ...
  assert.ok(process.env.SPIDERBYTE_VSCODE_SMOKE_REPORT, "Extension Host report path must be provided");
  await writeFile(
    process.env.SPIDERBYTE_VSCODE_SMOKE_REPORT,
