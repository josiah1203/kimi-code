 ...
 * Responsibilities: verify compatibility shapes, routing state, and terminal metadata one event at a time.
 * Wiring: the pure adapter and real protocol types are used directly; there are no stubs.
 * Run: pnpm exec vitest run --config apps/spiderbyte-vscode/vitest.config.ts apps/spiderbyte-vscode/test/event-adapter.test.ts
 */
...
describe('event adapter (projects SDK events into the compatibility Webview contract)', () => {
  ...
  it('emits a numbered compatibility step when an SDK step starts', () => {
    const result = adaptSdkEvent(createEventAdapterState(), {
  ...
  it.each([
    ...
    ['Glob', 'Glob'],
  ] as const)('maps the %s tool name to %s when a tool starts', (sdkName, compatibilityName) => {
    const result = adaptSdkEvent(createEventAdapterState(), {
  ...
  it('emits a compatibility result when an SDK tool call finishes', () => {
    const result = adaptSdkEvent(createEventAdapterState(), {
      type: 'tool.result',
      sessionId: 'session-1',
      agentId: 'main',
