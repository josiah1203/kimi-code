 ...
 * Run: pnpm --filter spiderbyte exec vitest run --config vitest.config.ts test/replay-resume.integration.test.ts
 */
...
async function createReplayRig(): Promise<ReplayRig> {
  const rootDir = await mkdtemp(join(tmpdir(), "spiderbyte-vscode-replay-"));
  ...
  const harness = createSpiderByteHarness({
    homeDir,
    identity: { productName: "spiderbyte-vscode", version: "test", platform: "spiderbyte_code_vscode" },
  });
