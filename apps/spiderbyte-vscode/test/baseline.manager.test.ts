 ...
 * Responsibilities: capture originals, show changes, keep/undo, persist compatibility tombstones, and reject unsafe paths.
 * Wiring: real temporary workspace/global-storage/compatibility files; no stubbed collaborators.
 * Run: pnpm --filter spiderbyte test -- baseline.manager.test.ts
 */
...
describe('file baselines (capture, compare, keep, and undo)', () => {
  ...
  it('isolates the same session id between different SpiderByte homes', async () => {
    const session = createSession();
    const filePath = join(workDir, 'app.ts');
...
describe('compatibility baselines (fallback, tombstones, and fork isolation)', () => {
  it('returns the compatibility original when no local baseline exists', async () => {
    const { session } = await createCompatibilitySession('compatibility original\n');
    await writeFile(join(workDir, 'app.ts'), 'current content\n', 'utf-8');

    await expect(manager.getContent(session, 'app.ts')).resolves.toBe('compatibility original\n');
  });

  it('reports a compatibility file when current content differs from its original', async () => {
    const { session } = await createCompatibilitySession('compatibility original\n');
    await writeFile(join(workDir, 'app.ts'), 'current content\n', 'utf-8');
  ...
  it('restores a compatibility original without mutating its snapshot when undo runs', async () => {
    const { session, baselinePath } = await createCompatibilitySession('compatibility original\n');
    await writeFile(join(workDir, 'app.ts'), 'current content\n', 'utf-8');

    await manager.undo(session, 'app.ts');

    await expect(readFile(join(workDir, 'app.ts'), 'utf-8')).resolves.toBe('compatibility original\n');
    await expect(readFile(baselinePath, 'utf-8')).resolves.toBe('compatibility original\n');
  });

  it('persists a keep tombstone for compatibility content without deleting the old snapshot', async () => {
    const { session, baselinePath } = await createCompatibilitySession('compatibility original\n');
  ...
  it('uses a materialized local original before the fork target compatibility fallback', async () => {
    const source = createSession('ses-source');
    const filePath = join(workDir, 'app.ts');
    await writeFile(filePath, 'local original\n', 'utf-8');
    await manager.capture(source, filePath);
    const { session: target } = await createCompatibilitySession('compatibility original\n', 'ses-target');
  ...
  it('keeps a fork baseline readable when the compatibility source is later deleted', async () => {
    const { session: source, compatibilitySessionDir } = await createCompatibilitySession(
      'compatibility original\n',
      'ses-source',
    );
    const target = createSession('ses-target', source.metadata);
    await manager.materializeToFork(source, target);
    await rm(compatibilitySessionDir, { recursive: true, force: true });

    await expect(manager.getContent(target, 'app.ts')).resolves.toBe('compatibility original\n');
  });

  it('keeps the source baseline visible when the fork accepts its own copy', async () => {
    const { session: source } = await createCompatibilitySession('compatibility original\n', 'ses-source');
    const target = createSession('ses-target', source.metadata);
    await manager.materializeToFork(source, target);

    await manager.keep(target, 'app.ts');

    await expect(manager.getContent(source, 'app.ts')).resolves.toBe('compatibility original\n');
  });

  it('preserves accepted compatibility paths when a fork is materialized', async () => {
    const { session: source } = await createCompatibilitySession('compatibility original\n', 'ses-source');
    const target = createSession('ses-target', source.metadata);
...
async function createCompatibilitySession(
  content: string,
  id = 'ses-compatibility',
): Promise<{
  session: BaselineSession;
  compatibilitySessionDir: string;
  baselinePath: string;
}> {
  const compatibilitySessionDir = join(root, `compatibility-${id}`);
  const baselinePath = join(compatibilitySessionDir, 'baseline', 'app.ts');
  await mkdir(join(compatibilitySessionDir, 'baseline'), { recursive: true });
  await writeFile(baselinePath, content, 'utf-8');
  return {
    session: createSession(id, { spiderbyte_cli_source_path: compatibilitySessionDir }),
    compatibilitySessionDir,
    baselinePath,
  };
}
