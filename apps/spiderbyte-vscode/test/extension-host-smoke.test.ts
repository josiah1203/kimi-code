 ...
 * Host cannot discover the developer's real compatibility SpiderByte home.
 * Wiring: real smoke orchestration and filesystem; @vscode/test-electron is the
 * external process/download boundary.
 * Run: pnpm --filter spiderbyte exec vitest run --config vitest.config.ts test/extension-host-smoke.test.ts
 */
...
beforeEach(() => {
  ...
  vscodeTest.runTests.mockImplementation(async (options) => {
    await writeFile(
      options.extensionTestsEnv.SPIDERBYTE_VSCODE_SMOKE_REPORT,
...
describe('installed VSIX Extension Host smoke', () => {
  ...
  it('gives the harness separate SpiderByte and operating-system homes before activation', async () => {
    const fixture = await makeFixture();
    ...
    expect(env.SPIDERBYTE_HOME).not.toBe(env.SPIDERBYTE_VSCODE_SMOKE_OS_HOME);
    expect(env.SPIDERBYTE_VSCODE_SMOKE_OS_HOME).toContain('os-home');
  });
...
async function makeFixture(): Promise<{ vsixPath: string; cachePath: string }> {
  const root = await mkdtemp(join(tmpdir(), 'spiderbyte-extension-host-smoke-'));
  tempDirs.push(root);
  const cachePath = join(root, 'cache');
  const vsixPath = join(root, 'spiderbyte-test.vsix');
