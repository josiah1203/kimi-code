 ...
 * Run: pnpm --filter spiderbyte exec vitest run --config vitest.config.ts test/workspace-paths.test.ts
 */
import { mkdtemp, mkdir, readFile, readdir, rm, stat, symlink, writeFile } from "node:fs/promises";
import { join } from "node:path";
import { tmpdir } from "node:os";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import type { Event, Session } from "@spiderbyte/sdk";
...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), "spiderbyte-vscode-workspace-paths-"));
...
describe("Webview workspace paths (selected-directory containment)", () => {
  ...
  it("omits an outside symlink when an SDK Write event requests baseline capture", async () => {
    const workDir = join(root, "project");
    const outsideRoot = await mkdtemp(join(tmpdir(), "spiderbyte-vscode-baseline-outside-"));
    ...
    const runtime = new SessionRuntime({
      session,
      compatibilityApproval: { yolo: false, afk: false },
      broadcast: vi.fn(),
  ...
  it("builds an absolute editor mention when the file is outside the workspace root", async () => {
    const otherRoot = await mkdtemp(join(tmpdir(), "spiderbyte-vscode-mention-outside-"));
  ...
  it("quotes an absolute editor mention whose path contains spaces", async () => {
    const otherRoot = await mkdtemp(join(tmpdir(), "spiderbyte vscode mention space-"));
    extraRoots.push(otherRoot);
    const outside = join(otherRoot, "App.java");
    await writeFile(outside, "class App {}");
    const bridge = createBridge();
  ...
  it("rejects a selected working directory whose symlink target leaves the workspace", async () => {
    const outside = await mkdtemp(join(tmpdir(), "spiderbyte-vscode-outside-"));
