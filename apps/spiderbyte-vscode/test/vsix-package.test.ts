 ...
 * Run: pnpm --filter spiderbyte exec vitest run --config vitest.config.ts test/vsix-package.test.ts
 */
...
describe('VSIX package CLI (target planning and validation)', () => {
  it('plans all six supported targets when no target is supplied', async () => {
    const outputDir = await makeTempDir('spiderbyte-package-plan-');
    ...
    expect(result.stdout).toContain('spiderbyte-darwin-x64.vsix');
    expect(result.stdout).toContain('spiderbyte-darwin-arm64.vsix');
    expect(result.stdout).toContain('spiderbyte-linux-x64.vsix');
    expect(result.stdout).toContain('spiderbyte-linux-arm64.vsix');
    expect(result.stdout).toContain('spiderbyte-win32-x64.vsix');
    expect(result.stdout).toContain('spiderbyte-win32-arm64.vsix');
  });

  it('accepts a Windows ARM target when the output path contains spaces', async () => {
    const root = await makeTempDir('spiderbyte-package-windows-');
...
describe('Extension Development Host setup (isolated local state)', () => {
  it('creates the complete isolated directory layout for a debug launch', async () => {
    const parent = await makeTempDir('spiderbyte-dev-profile-');
    ...
    await expect(readFile(join(baseDir, 'workspace', 'README.md'), 'utf8')).resolves.toContain(
      'Isolated SpiderByte extension development workspace',
    );
    ...
    await expect(directoryExists(join(baseDir, 'spiderbyte-home'))).resolves.toBe(true);
  });

  it('refuses to clear a directory without the dedicated safety suffix', async () => {
    const unsafeDir = await makeTempDir('spiderbyte-dev-unsafe-');
...
async function makeVsixFixture(target: string): Promise<string> {
  const root = await makeTempDir('spiderbyte-vsix-fixture-');
  ...
  await Promise.all([
    ...
    writeFile(join(distDir, 'webview.js'), 'globalThis.__spiderbyteWebview = true;\n'),
    writeFile(join(distDir, 'spiderbyte-banner-dark.svg'), '<svg />'),
    writeFile(join(distDir, 'spiderbyte-banner-light.svg'), '<svg />'),
    writeFile(join(distDir, 'spiderbyte-logo.png'), 'fixture'),
    writeFile(join(resourcesDir, 'spiderbyte-icon-storefront.png'), 'fixture'),
    writeFile(join(resourcesDir, 'spiderbyte-icon.svg'), '<svg />'),
  ]);
  return root;
}
