 ...
 * Run: pnpm exec vitest run --config apps/spiderbyte-vscode/vitest.config.ts test/settings-store.test.ts
 */
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";

import { MCP_SECRET_MASK } from "../shared/spiderbyte-sdk-types";
...
const MODELS = [
  { id: "plain", name: "Plain", provider: "managed:spiderbyte", capabilities: [] },
  {
    id: "reasoning",
    name: "Reasoning",
  ...
  { id: "always", name: "Always", provider: "managed:spiderbyte", capabilities: ["always_thinking"] },
];
...
describe("Webview model metadata", () => {
  it("keeps same-named models in separate provider groups", () => {
    const groups = groupModelsByProvider([
      { id: "spiderbyte/shared", name: "Shared", provider: "managed:spiderbyte", capabilities: [] },
      { id: "proxy/shared", name: "Shared", provider: "company-proxy", capabilities: [] },
    ]);
...
describe("Webview thinking mode parity with the TUI", () => {
  it("derives thinking modes from metadata only, mirroring the TUI rules", () => {
    ...
    expect(getModelThinkingMode({ ...base, name: "SpiderByte Thinking Pro" })).toBe("none");
