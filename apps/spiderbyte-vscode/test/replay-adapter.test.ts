 ...
 * Run: pnpm exec vitest run --config apps/spiderbyte-vscode/vitest.config.ts test/replay-adapter.test.ts
 */
...
function resumedAgent(
  ...
  return {
    ...
    config: {
      modelAlias: options.modelAlias ?? "spiderbyte-test",
      modelCapabilities: {
        image_in: true,
        video_in: true,
        audio_in: false,
