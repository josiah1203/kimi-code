...
const defaultCachePath = join(tmpdir(), "spiderbyte-vscode-test-cache");
...
export async function runExtensionHostSmoke(options = {}) {
  ...
  try {
    await Promise.all([
      ...
      paths.userData,
      paths.spiderbyteHome,
      paths.osHome,
      paths.workspace,
      paths.harness,
    ].map((path) => mkdir(path, { recursive: true })));
    await writeFile(join(paths.workspace, "README.md"), "# SpiderByte VSIX Extension Host smoke\n", "utf8");
    ...
    await runTests({
      ...
      extensionTestsEnv: {
        SPIDERBYTE_HOME: paths.spiderbyteHome,
        SPIDERBYTE_VSCODE_SMOKE_OS_HOME: paths.osHome,
        SPIDERBYTE_VSCODE_SMOKE_REPORT: paths.report,
        SPIDERBYTE_VSCODE_SMOKE_VSIX: basename(vsixPath),
      },
    });
...
function defaultVsixPath() {
  ...
  return join(appDir, "artifacts", "vsix", `spiderbyte-${process.platform}-${arch}.vsix`);
}
...
async function writeHarnessManifest(directory) {
  await writeFile(
    ...
    JSON.stringify({
      name: "spiderbyte-vscode-extension-host-smoke",
      displayName: "SpiderByte VSCode Extension Host Smoke",
