...
const REQUIRED_WEBVIEW_FILES = [
  'dist/webview.js',
  'dist/spiderbyte-banner-dark.svg',
  'dist/spiderbyte-banner-light.svg',
  'dist/spiderbyte-logo.png',
];
...
export async function verifyVsix(vsixPath, target, options = {}) {
  const extractionRoot = await mkdtemp(join(tmpdir(), 'spiderbyte-vsix-audit-'));
  try {
...
function verifyForbiddenFiles(files) {
  for (const file of files) {
    ...
    if (
      lower.includes('spiderbyte-agent-sdk') ||
      lower.includes('download-cli') ||
      lower.includes('/bin/spiderbyte/') ||
      /(^|\/)uv(?:\.exe)?$/.test(lower)
    ) {
