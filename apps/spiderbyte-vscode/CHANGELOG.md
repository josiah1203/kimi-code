...
- [#2326](https://github.com/SpiderByte/spiderbyte/pull/2326) [`302b2cd`](https://github.com/SpiderByte/spiderbyte/commit/302b2cd680e0ec66f68b4572238de84ce311c5f4) Thanks [@gaoyuan1223m](https://github.com/gaoyuan1223m)! - Fix only the first question being answerable when the agent asked multiple questions at once; each question is now answered one by one and submitted together.

## 0.6.6

### Patch Changes

- [#2393](https://github.com/SpiderByte/spiderbyte/pull/2393) [`6d0a046`](https://github.com/SpiderByte/spiderbyte/commit/6d0a046488edda56219961b253c4787abae7a113) Thanks [@wbxl2000](https://github.com/wbxl2000)! - Fix new users getting stranded on "Model setup required" with no way back to sign-in when the first login finishes authorization but fails to complete model setup; the screen now offers a path back to the sign-in page so login can be retried.
- [#2402](https://github.com/SpiderByte/spiderbyte/pull/2402) [`0f3b106`](https://github.com/SpiderByte/spiderbyte/commit/0f3b106c4260ad626f66bc5c457a535d3163f2bc) Thanks [@wbxl2000](https://github.com/wbxl2000)! - Reword the sign-in waiting message from "Waiting for authorization" to "Waiting for authentication".

- Updated dependencies [[`40172c7`](https://github.com/SpiderByte/spiderbyte/commit/40172c7ca96ca981b043b793588dd32e898979fa)]:
  - @spiderbyte/sdk@0.15.0

## 0.6.5

### Patch Changes

- [#1994](https://github.com/SpiderByte/spiderbyte/pull/1994) [`beeb964`](https://github.com/SpiderByte/spiderbyte/commit/beeb964393c8f9a38c2b1e2273e4415fc434b16d) Thanks [@RealKai42](https://github.com/RealKai42)! - Reduce webview streaming re-render churn: settled assistant messages no longer re-render on every streaming delta, and local images over 10MB are no longer inlined into the webview DOM.
- Updated dependencies [[`ec88d35`](https://github.com/SpiderByte/spiderbyte/commit/ec88d352e8f4dc5e8ffd1212f016138458f69893), [`b5efba7`](https://github.com/SpiderByte/spiderbyte/commit/b5efba7abcaf4041f81ec520097a61e6546e8c50), [`ce0e3ce`](https://github.com/SpiderByte/spiderbyte/commit/ce0e3ceb04223bdaad8e8931bad46eff561055b6), [`e458323`](https://github.com/SpiderByte/spiderbyte/commit/e45832398d0d9cad98dbad1cbf1e5b103a20aace)]:
  - @spiderbyte/sdk@0.14.0
...
- A core error arriving in the middle of a turn no longer corrupts the active
  ...
  a broken state.
- SpiderByte sign-in and connection failures now include the underlying transport
...
- The **Sign in** action in the settings (gear) menu now actually starts the
  SpiderByte login flow and shows an error toast when sign-in fails, instead of
...
- Legacy SpiderByte OAuth credentials and MCP OAuth credentials are deliberately
  not migrated. Sign in to SpiderByte again and re-authorize affected MCP
  servers after upgrading.
- Removed the `spiderbyte.executablePath` and `spiderbyte.environmentVariables` settings.
  The old `spiderbyte.environmentVariables.SPIDERBYTE_SHARE_DIR` value is consulted only to
  discover legacy data during migration; it is not applied to the new runtime.
  The system-level `SPIDERBYTE_HOME` environment variable remains supported.

### Changed

- Replaced the legacy Python/stdio runtime with the in-process SpiderByte Node
  SDK. The extension no longer downloads or starts a separate SpiderByte executable.
- The in-process engine is the same one that powers the SpiderByte CLI, so the
...
- Added an opt-in legacy migration prompt on the first launch that detects data
  ...
  current SpiderByte home and does not delete the legacy source. If migration is
  skipped or needs to be retried, run **SpiderByte: Migrate Legacy Data** from the
  Command Palette.
- When VS Code and the SpiderByte terminal app resolve to the same
  `SPIDERBYTE_HOME`, they use the same configuration and session storage. Running
...
- The model picker groups models by provider when multiple providers are
  ...
  requires dismissing the SpiderByte account login screen on every launch.
- The file changes panel and Undo actions use extension-maintained baselines.
  Files changed through SpiderByte's Write and Edit operations are tracked on a
...
- The `spiderbyte.yoloMode` setting now reaches the permission engine: enabling it
