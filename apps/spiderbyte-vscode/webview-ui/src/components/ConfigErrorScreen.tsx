...
function NoModelsContent({ onRefresh }: Pick<Props, "onRefresh">) {
  return (
    <>
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          Configure a local or BYOK provider and model in your shared SpiderByte <code className="bg-muted px-1 rounded">config.toml</code>.
        </p>
      </div>
      ...
      <div className="bg-muted/50 rounded-lg p-4 text-left space-y-2">
        <div className="flex items-center gap-2 text-xs font-medium">
          ...
          Shared SpiderByte configuration
        </div>
        <p className="text-xs text-muted-foreground">
          VS Code and the terminal UI use the same SpiderByte home, configuration, credentials, and sessions.
        </p>
      </div>
...
export function ConfigErrorScreen({ type, errorMessage, onRefresh }: Props) {
  if (type === "loading") {
    return (
      <div className="h-full flex items-center justify-center p-6">
        <div className="text-center space-y-4">
          <SpiderByteMascot className="h-10 mx-auto opacity-50" />
          <div className="inline-flex items-center gap-2 text-muted-foreground">
            <IconLoader2 className="size-4 animate-spin" />
            <span className="text-sm">Starting SpiderByte...</span>
          </div>
        </div>
      </div>
    );
      ...
      <div className="h-full flex items-center justify-center p-6">
        <div className="max-w-sm text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">Open a folder to start using SpiderByte.</p>
          </div>
          <Button
            onClick={() => {
      ...
      <div className="max-w-sm mx-auto text-center space-y-6">
        ...
        <div className="space-y-2">
          <div className="inline-flex items-center gap-2 text-red-500">
            <IconAlertTriangle className="size-5" />
            <span className="text-sm font-medium">SpiderByte could not start</span>
          </div>
          <p className="text-xs text-muted-foreground">Check the error below. Full diagnostics are available in the SpiderByte output channel.</p>
        </div>
