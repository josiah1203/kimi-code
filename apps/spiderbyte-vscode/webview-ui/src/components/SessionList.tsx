...
export function SessionList({ onClose }: SessionListProps) {
  ...
  const { data: spiderbyteSessions = [], loading, mutate } = useRequest(() => bridge.getAllSpiderByteSessions());
  ...
  const filteredSessions = useMemo(() => {
    if (!searchQuery.trim()) return spiderbyteSessions;
    const q = searchQuery.toLowerCase();
    return spiderbyteSessions.filter((s) => s.brief.toLowerCase().includes(q));
  }, [spiderbyteSessions, searchQuery]);
