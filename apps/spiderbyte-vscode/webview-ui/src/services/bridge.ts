...
class Bridge {
  ...
  constructor() {
    ...
    if (typeof acquireVsCodeApi === "function") {
      ...
      console.warn("[SpiderByte Bridge] Running outside VS Code, using mock");
      this.vscode = {
        postMessage: (msg) => console.log("[SpiderByte Mock]", msg),
        getState: () => undefined,
        setState: () => {},
      };
    }
  ...
  getSlashCommands() {
    return this.call<import("shared/spiderbyte-sdk-types").SlashCommandInfo[]>(Methods.GetSlashCommands);
  }
  ...
  getModels() {
    return this.call<SpiderByteConfig>(Methods.GetModels);
  }
  ...
  authMCP(name: string) {
    return this.call<{ ok: boolean }>(Methods.AuthMCP, { name }, DEFAULT_REQUEST_TIMEOUT_MS);
  }
  ...
  getSpiderByteSessions() {
    return this.call<SessionInfo[]>(Methods.GetSpiderByteSessions);
  }

  getAllSpiderByteSessions() {
    return this.call<SessionInfo[]>(Methods.GetAllSpiderByteSessions);
  }
  ...
  loadSessionHistory(sessionId: string) {
    return this.call<UIStreamEvent[]>(Methods.LoadSpiderByteSessionHistory, { spiderbyteSessionId: sessionId });
  }

  deleteSession(sessionId: string) {
    return this.call<{ ok: boolean }>(Methods.DeleteSpiderByteSession, { sessionId });
  }

  forkSession(sessionId: string, turnIndex: number) {
    return this.call<{ sessionId: string } | null>(Methods.ForkSpiderByteSession, { sessionId, turnIndex });
  }
