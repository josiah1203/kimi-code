...
import { SpiderByteRuntime } from "./runtime/spiderbyte-runtime";
...
export class BridgeHandler {
  ...
  constructor(
    ...
    this.runtime = new SpiderByteRuntime({
  ...
  private trace(id: string, method: string, durationMs: number, ok: boolean): void {
    ...
    console.debug(`[spiderbyte-vscode] ${line}`);
    this.writeLog(line);
  }
  ...
  private logRuntimeError(message: string, error?: unknown): void {
    ...
    console.error(`[spiderbyte-vscode] ${line}`);
    this.writeLog(line);
  }
}
