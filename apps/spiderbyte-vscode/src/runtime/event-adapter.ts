import type { Event } from '@spiderbyte/sdk';
...
import { toCompatibilityDisplay } from './tool-display';
...
export function adaptSdkEvent(
  ...
  const mapped = mapCompatibilityWireEvent(state, sdkEvent, mainAgentId);
...
export function toCompatibilityToolName(name: string): string {
  switch (name) {
    case 'Bash':
      return 'Shell';
    case 'Read':
...
interface MappedCompatibilityWireEvent {
  readonly state: EventAdapterState;
  readonly event?: CompatibilityWireEvent;
}

function mapCompatibilityWireEvent(
  state: EventAdapterState,
  sdkEvent: Event,
  mainAgentId: string,
): MappedCompatibilityWireEvent {
  switch (sdkEvent.type) {
    ...
    case 'tool.call.started': {
      ...
      const display = sdkEvent.display === undefined ? undefined : toCompatibilityDisplay(sdkEvent.display);
      return {
        ...
        event: {
          ...
          payload: {
            ...
            id: toolCallId,
            function: {
              name: toCompatibilityToolName(sdkEvent.name),
    ...
    case 'tool.call.delta': {
      ...
      return { state, event: event as CompatibilityWireEvent };
    }
    case 'tool.result': {
      const toolCallId = scopedToolCallId(
        sdkEvent.agentId,
...
function routeSubagentEvent(
  state: EventAdapterState,
  agentId: string,
  event: CompatibilityWireEvent,
  mainAgentId: string,
): CompatibilityWireEvent | undefined {
...
function withSessionId(event: CompatibilityWireEvent, sessionId: string): AdaptedUIStreamEvent {
