...
import {
  corePermissionForCompatibilityApproval,
  compatibilityApprovalMetadata,
  readCompatibilityApprovalFlags,
  readMigratedCompatibilityApprovalFlags,
  withGlobalYoloMode,
  type CompatibilityApprovalFlags,
} from "./compatibility-approval";
import { SessionRuntime } from "./session-runtime";
import { areSameFsPath } from "../utils/fs-path";

export interface SpiderByteRuntimeOptions {
...
export class SpiderByteRuntime {
  readonly harness: SpiderByteHarness;

  private readonly broadcast: RuntimeBroadcast;
  private readonly captureBaseline: SpiderByteRuntimeOptions["captureBaseline"];
  private readonly log: SpiderByteRuntimeOptions["log"];
  private readonly sessions = new Map<string, SessionRuntime>();
  private readonly sessionByView = new Map<string, string>();
  private closed = false;

  constructor(options: SpiderByteRuntimeOptions) {
    ...
    this.harness =
      ...
      createSpiderByteHarness({
        ...
        identity: {
          ...
          platform: "spiderbyte_code_vscode",
        },
        uiMode: "vscode",
      });
  }
  ...
  async openSession(options: OpenSessionOptions): Promise<SessionRuntime> {
    ...
    if (
      ...
      await applySessionSettings(current.session, options, current.compatibilityApprovalFlags);
    ...
    if (runtime !== undefined) {
      ...
      await applySessionSettings(runtime.session, options, runtime.compatibilityApprovalFlags);
      await this.detachView(options.webviewId);
    } else {
      const defaultApproval: CompatibilityApprovalFlags = { yolo: options.yoloMode, afk: false };
      const session =
        requestedId === undefined
          ? await this.harness.createSession({
              ...
              permission: corePermissionForCompatibilityApproval(defaultApproval),
              metadata: compatibilityApprovalMetadata(defaultApproval),
            })
          : await this.harness.resumeSession({ id: requestedId, includeSubagents: true });
      try {
        assertSessionWorkDir(session, options.workDir);
        const storedApproval = readCompatibilityApprovalFlags(session.summary?.metadata);
        const restoredApproval =
          storedApproval ?? (await this.readMigratedCompatibilityApproval(session)) ?? defaultApproval;
        ...
        if (storedApproval === undefined || flagsDiffer(storedApproval, approval)) {
          await session.updateMetadata(compatibilityApprovalMetadata(approval));
        }
  ...
  async attachResumedSession(
    ...
    if (runtime === undefined) {
      try {
        ...
        const restoredApproval =
          storedApproval ??
          (await this.readMigratedCompatibilityApproval(session)) ??
        ...
        const permission = corePermissionForCompatibilityApproval(approval);
  ...
  async setYoloModeForActiveSessions(enabled: boolean): Promise<void> {
    await Promise.all(
      [...this.sessions.values()].map((session) => session.setCompatibilityYoloMode(enabled)),
    );
  }
  ...
  private wrapSession(session: Session, compatibilityApproval: CompatibilityApprovalFlags): SessionRuntime {
  ...
  private async readMigratedCompatibilityApproval(
    session: Session,
  ): Promise<CompatibilityApprovalFlags | undefined> {
    const metadata = session.summary?.metadata;
    try {
      return await readMigratedCompatibilityApprovalFlags(metadata);
    } catch (error) {
      this.log("Unable to restore compatibility session approval settings", error);
      return undefined;
    }
  }

  private ensureOpen(): void {
    if (this.closed) throw new Error("SpiderByte runtime is closed.");
  }
}
...
async function applySessionSettings(
  ...
  compatibilityApproval: CompatibilityApprovalFlags,
): Promise<void> {
  const status = await session.getStatus();
  ...
  const permission = corePermissionForCompatibilityApproval(compatibilityApproval);
...
function flagsDiffer(a: CompatibilityApprovalFlags, b: CompatibilityApprovalFlags): boolean {
