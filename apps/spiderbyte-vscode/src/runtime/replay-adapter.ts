...
import type {
  ContentPart as CompatibilityContentPart,
  DisplayBlock,
...
import { toCompatibilityToolName } from "./event-adapter";
    ...
    switch (record.type) {
      case "message": {
        ...
        if (message.role === "assistant") {
          if (!turnOpen) break;
          ensureStep();
          for (const part of toCompatibilityContent(message.content)) {
          ...
          for (const call of message.toolCalls) {
            const toolCall: ToolCall = {
              ...
              id: call.id,
              function: {
                name: toCompatibilityToolName(call.name),
            ...
            withSession(
              {
                ...
                payload: {
                  ...
                  return_value: {
                    ...
                    output: toCompatibilityContent(message.content),
                    message: "",
                    display: [...display],
                  },
                },
...
function renderSubagentInvocations(
  ...
  const result: CompatibilityWireEvent[] = [];
...
function toCompatibilityContent(content: readonly ContentPart[]): CompatibilityContentPart[] {
  const result: CompatibilityContentPart[] = [];
...
function replayUserInput(
  ...
  return toCompatibilityContent(content);
}

function importedContextReplay(
  content: readonly ContentPart[],
): { input: CompatibilityContentPart[]; confirmation: string } | undefined {
