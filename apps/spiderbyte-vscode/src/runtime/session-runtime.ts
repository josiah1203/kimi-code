...
import type { ContentPart as CompatibilityContentPart, ApprovalResponse } from "../../shared/spiderbyte-sdk-types";
...
export interface SessionRuntimeOptions {
  ...
  readonly compatibilityApproval: CompatibilityApprovalFlags;
...
interface ActivePrompt {
  readonly input: CompatibilityContentPart[] | string;
  started: boolean;
  settled: boolean;
...
export class SessionRuntime {
  ...
  private compatibilityApproval: CompatibilityApprovalFlags;
  ...
  constructor(options: SessionRuntimeOptions) {
    ...
    this.compatibilityApproval = options.compatibilityApproval;
    ...
    // (mapped from the compatibility flags) already auto-approves what yolo/auto
  ...
  get compatibilityApprovalFlags(): CompatibilityApprovalFlags {
    return this.compatibilityApproval;
  }

  async toggleCompatibilityApproval(kind: keyof CompatibilityApprovalFlags): Promise<CompatibilityApprovalFlags> {
    const next = { ...this.compatibilityApproval, [kind]: !this.compatibilityApproval[kind] };
    await this.applyCompatibilityApproval(next);
    return next;
  }

  async setCompatibilityYoloMode(enabled: boolean): Promise<void> {
    if (this.compatibilityApproval.yolo === enabled) return;
    await this.applyCompatibilityApproval({ ...this.compatibilityApproval, yolo: enabled });
  }
  ...
  async prompt(input: string | CompatibilityContentPart[]): Promise<PromptResult> {
    return this.runTurnAction(input, () => this.session.prompt(toSdkPromptInput(input)));
  }

  async runTurnAction(
    input: string | CompatibilityContentPart[],
    action: () => Promise<void>,
  ...
  beginHostAction(input: string | CompatibilityContentPart[], forkable = false): number {
    this.ensureOpen();
    if (this.isBusy) {
  ...
  async steer(input: string | CompatibilityContentPart[]): Promise<void> {
    this.ensureOpen();
  ...
  private async applyCompatibilityApproval(flags: CompatibilityApprovalFlags): Promise<void> {
    this.ensureOpen();
    const permission = corePermissionForCompatibilityApproval(flags);
    const status = await this.session.getStatus();
    const permissionChanged = status.permission !== permission;
    if (permissionChanged) await this.session.setPermission(permission);
    try {
      await this.session.updateMetadata(compatibilityApprovalMetadata(flags));
    } catch (error) {
    ...
    this.compatibilityApproval = flags;
  }
  ...
  private emitError(error: unknown, phase: ErrorPhase, options?: { readonly terminal?: boolean }): void {
    const code = isSpiderByteError(error) ? error.code : "internal";
...
export function toSdkPromptInput(input: string | CompatibilityContentPart[]): string | PromptInput {
