...
import {
  effectiveModelAlias,
  type SpiderByteConfig as SdkSpiderByteConfig,
  type ModelAlias,
  type ThinkingEffort,
} from "@spiderbyte/sdk";
...
import type {
  SpiderByteConfig as WebviewSpiderByteConfig,
  ModelConfig,
  SlashCommandInfo,
...
const openSettings: Handler<void, { ok: boolean }> = async () => {
  await vscode.commands.executeCommand("workbench.action.openSettings", "spiderbyte");
  return { ok: true };
};

const getModels: Handler<void, WebviewSpiderByteConfig> = async (_, ctx) => {
  const config = await ctx.harness.getConfig({ reload: true });
...
export function toWebviewConfig(config: SdkSpiderByteConfig): WebviewSpiderByteConfig {
