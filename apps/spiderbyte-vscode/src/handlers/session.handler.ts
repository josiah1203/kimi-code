...
import type { SessionSummary } from "@spiderbyte/sdk";

import { Events, Methods } from "../../shared/bridge";
import type { SessionInfo } from "../../shared/spiderbyte-sdk-types";
...
export const sessionHandlers: Record<string, Handler<any, any>> = {
  [Methods.GetSpiderByteSessions]: async (_, ctx): Promise<SessionInfo[]> => {
    if (!ctx.workDir) return [];
    return (await ctx.harness.listSessions({ workDir: ctx.workDir })).map(toSessionInfo);
  },

  [Methods.GetAllSpiderByteSessions]: async (_, ctx): Promise<SessionInfo[]> => {
  ...
  [Methods.LoadSpiderByteSessionHistory]: async (params: LoadHistoryParams, ctx) => {
    if (!ctx.workDir || !isSessionId(params.spiderbyteSessionId)) return [];
    const runtime = await ctx.resumeSession(params.spiderbyteSessionId);
    ...
    try {
      ...
      void Promise.resolve(
        vscode.window.showWarningMessage(
          "SpiderByte: This conversation opened, but its file change history is unavailable.",
          "Show Logs",
        ),
      )
        .then((action) => {
  ...
  [Methods.DeleteSpiderByteSession]: async (params: DeleteSessionParams, ctx): Promise<{ ok: boolean }> => {
  ...
  [Methods.ForkSpiderByteSession]: async (params: ForkSessionParams, ctx) => {
