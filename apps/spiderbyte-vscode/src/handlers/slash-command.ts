    ...
    if (command.name === "import") {
      ...
      if (result.sensitive) {
        void vscode.window.showWarningMessage(
          "SpiderByte: The imported file may contain API keys, tokens, or credentials.",
        );
      }
    } else {
      switch (command.name) {
        ...
        case "reset":
          ...
          break;
        case "yolo":
          await toggleCompatibilityPermission(runtime, "yolo", emit);
          break;
        case "auto":
        case "afk":
          await toggleCompatibilityPermission(runtime, "afk", emit);
          break;
        case "plan":
...
async function toggleCompatibilityPermission(
  runtime: SessionRuntime,
  kind: "yolo" | "afk",
  emit: (text: string) => void,
): Promise<void> {
  const flags = await runtime.toggleCompatibilityApproval(kind);
...
async function exportContext(
  ...
  void vscode.window.showInformationMessage("SpiderByte: Session exported.", "Open File").then((action) => {
...
function defaultExportName(sessionId: string, now: Date): string {
  ...
  return `spiderbyte-export-${sessionId.slice(0, 8)}-${timestamp}.md`;
}
