...
import type {
  GlobalMcpServerConfig as SdkMcpServerConfig,
  McpTestResult,
} from "@spiderbyte/sdk";
...
export const mcpHandlers: Record<string, Handler<any, any>> = {
    ...
    await vscode.window.withProgress(
      {
        ...
        title: `SpiderByte: Authenticating "${name}"...`,
        cancellable: false,
      },
      async () => {
        try {
          ...
          await vscode.window.showInformationMessage(`SpiderByte: OAuth completed for "${name}"`);
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error);
          await vscode.window.showErrorMessage(`SpiderByte: OAuth failed for "${name}": ${message}`);
          throw error;
        }
      },
    );
  ...
  [Methods.ResetAuthMCP]: async ({ name }: NameParams, ctx) => {
    await vscode.window.withProgress(
      {
        location: vscode.ProgressLocation.Notification,
        title: `SpiderByte: Resetting auth for "${name}"...`,
        cancellable: false,
      },
      async () => {
        try {
          await ctx.harness.resetMcpServerAuth(name);
          await vscode.window.showInformationMessage(`SpiderByte: Auth reset for "${name}"`);
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error);
          await vscode.window.showErrorMessage(`SpiderByte: Reset auth failed for "${name}": ${message}`);
          throw error;
        }
      },
    );
  ...
  [Methods.TestMCP]: async ({ name }: NameParams, ctx): Promise<MCPTestResult> => {
    void vscode.window.showInformationMessage(`SpiderByte: Testing MCP server "${name}"...`);
