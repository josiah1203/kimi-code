...
interface BaselineManifestV1 {
  ...
  readonly acceptedCompatibilityPaths: readonly string[];
}
...
interface MutableManifest {
  ...
  acceptedCompatibilityPaths: string[];
}
...
export class BaselineManager {
  ...
  constructor(globalStorageRoot: string, homeNamespace = 'default') {
    ...
    if (homeNamespace.length === 0) {
      throw new BaselineError('The SpiderByte home namespace is empty');
    }
  ...
  async capture(session: BaselineSession, filePath: string): Promise<void> {
    ...
    await this.serialize([session.id], async () => {
      ...
      const accepted = new Set(manifest.acceptedCompatibilityPaths);
      const acceptedPath = equivalentPath(session, accepted, resolved.relativePath);
      if (acceptedPath === undefined) {
        const compatibilityExists = await this.hasCompatibilityBaseline(session, resolved.relativePath);
        if (compatibilityExists) return;
      }
      ...
      next.acceptedCompatibilityPaths = uniquePaths(session, accepted);
  ...
  async keep(session: BaselineSession, filePath: string): Promise<void> {
    ...
    await this.serialize([session.id], async () => {
      ...
      const hasCompatibility = await this.hasCompatibilityBaseline(session, resolved.relativePath);
      if (!hadLocal && !hasCompatibility) return;

      const next = mutableManifest(manifest);
      if (localPath !== undefined) delete next.entries[localPath];
      const accepted = new Set(next.acceptedCompatibilityPaths);
      const acceptedPath = equivalentPath(session, accepted, resolved.relativePath);
      if (acceptedPath !== undefined) accepted.delete(acceptedPath);
      if (hasCompatibility) accepted.add(resolved.relativePath);
      next.acceptedCompatibilityPaths = uniquePaths(session, accepted);
  ...
  async keepAll(session: BaselineSession): Promise<void> {
    await this.serialize([session.id], async () => {
      const manifest = await this.readManifest(session);
      const compatibilityPaths = await this.listCompatibilityPaths(session);
      const next = mutableManifest(manifest);
      next.entries = {};
      next.acceptedCompatibilityPaths = uniquePaths(session, [
        ...next.acceptedCompatibilityPaths,
        ...compatibilityPaths,
      ]);
  ...
  async materializeToFork(source: BaselineSession, target: BaselineSession): Promise<void> {
    ...
    await this.serialize([source.id, target.id], async () => {
      ...
      const accepted = uniquePaths(target, [
        ...next.acceptedCompatibilityPaths,
        ...sourceManifest.acceptedCompatibilityPaths,
      ]);
      ...
      next.acceptedCompatibilityPaths = accepted;
      await this.writeManifest(next);
    });
  }
  ...
  private async effectivePaths(
    ...
    const accepted = new Set(
      manifest.acceptedCompatibilityPaths.map((relativePath) =>
        pathComparisonKey(session, relativePath),
      ),
    );
    for (const relativePath of await this.listCompatibilityPaths(session)) {
  ...
  private async readEffectiveBaseline(
    ...
    if (equivalentPath(session, manifest.acceptedCompatibilityPaths, relativePath) !== undefined) {
      return undefined;
    }
    return this.readCompatibilityBaseline(session, relativePath);
  }
  ...
  private async writeManifest(manifest: BaselineManifestV1): Promise<void> {
    if (
      ...
      manifest.acceptedCompatibilityPaths.length === 0
    ) {
  ...
  private async listCompatibilityPaths(session: BaselineSession): Promise<string[]> {
    const root = compatibilityBaselineRoot(session);
    if (root === undefined) return [];

    const result: string[] = [];
    await walkCompatibilityBaselines(root, '', result);
    return result.toSorted();
  }

  private async hasCompatibilityBaseline(
    session: BaselineSession,
    relativePath: string,
  ): Promise<boolean> {
    const compatibilityPath = compatibilityBaselinePath(session, relativePath);
    if (compatibilityPath === undefined) return false;
    try {
      const info = await stat(compatibilityPath);
      if (!info.isFile()) {
        throw new BaselineError(`Compatibility baseline "${relativePath}" is not a regular file`);
      }
      return true;
    } catch (error) {
      if (isErrorCode(error, 'ENOENT')) return false;
      if (error instanceof BaselineError) throw error;
      throw new BaselineError(`Unable to inspect compatibility baseline "${relativePath}"`, {
        cause: error,
      });
    }
  }

  private async readCompatibilityBaseline(
    session: BaselineSession,
    relativePath: string,
  ): Promise<BaselineValue | undefined> {
    const compatibilityPath = compatibilityBaselinePath(session, relativePath);
    if (compatibilityPath === undefined) return undefined;

    let info;
    try {
      info = await stat(compatibilityPath);
    } catch (error) {
    ...
    try {
      const content = await readFile(compatibilityPath, 'utf-8');
      return { content, existedBefore: content.length > 0 };
    } catch (error) {
      throw new BaselineError(`Unable to read compatibility baseline "${relativePath}"`, {
        cause: error,
      });
    }
  }
...
function emptyManifest(sessionId: string): BaselineManifestV1 {
  return { version: MANIFEST_VERSION, sessionId, entries: {}, acceptedCompatibilityPaths: [] };
}
...
function mutableManifest(manifest: BaselineManifestV1): MutableManifest {
  return {
    ...
    acceptedCompatibilityPaths: [...manifest.acceptedCompatibilityPaths],
  };
}
...
function parseManifest(value: unknown, session: BaselineSession): BaselineManifestV1 {
  ...
  const rawAccepted = value['acceptedCompatibilityPaths'];
  ...
  const acceptedCompatibilityPaths: string[] = [];
  for (const rawPath of rawAccepted) {
    if (typeof rawPath !== 'string') {
      throw new BaselineError(`Invalid accepted compatibility path in session "${session.id}"`);
    }
    const relativePath = resolveSessionFile(session, rawPath).relativePath;
    if (relativePath !== rawPath) {
      throw new BaselineError(`Unsafe accepted compatibility path "${rawPath}" in session "${session.id}"`);
    }
    if (equivalentPath(session, acceptedCompatibilityPaths, relativePath) === undefined) {
      acceptedCompatibilityPaths.push(relativePath);
    }
  }
  ...
  return {
    ...
    entries,
    acceptedCompatibilityPaths: uniquePaths(session, acceptedCompatibilityPaths),
  };
}
...
function compatibilityBaselineRoot(session: BaselineSession): string | undefined {
  const source = session.metadata?.['spiderbyte_cli_source_path'];
...
function compatibilityBaselinePath(
  session: BaselineSession,
  relativePath: string,
): string | undefined {
  const root = compatibilityBaselineRoot(session);
...
async function walkCompatibilityBaselines(
  directory: string,
  ...
  try {
    ...
    throw new BaselineError(`Unable to list compatibility baseline directory "${directory}"`, {
      cause: error,
    });
  }
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      await walkCompatibilityBaselines(path.join(directory, entry.name), relativePath, result);
