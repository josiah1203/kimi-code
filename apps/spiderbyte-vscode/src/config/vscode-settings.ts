...
function getConfig() {
  return vscode.workspace.getConfiguration("spiderbyte");
}
...
export function onSettingsChange(callback: (changedKeys: string[]) => void): vscode.Disposable {
  return vscode.workspace.onDidChangeConfiguration((e) => {
    if (!e.affectsConfiguration("spiderbyte")) {
      return;
    }
    ...
    const changedKeys = keys.filter((key) => e.affectsConfiguration(`spiderbyte.${key}`));
