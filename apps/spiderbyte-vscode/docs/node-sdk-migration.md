...
The `0.5.x` VS Code extension launched a separately installed Python SpiderByte CLI
...
editor and SpiderByte.

Version `0.6.0` moves the extension into this monorepo under `apps/spiderbyte-vscode` and
runs the stable TypeScript v1 engine through `@spiderbyte/sdk` in the
...
- Keep the extension ID `spiderbyte-ai.spiderbyte` so `0.6.0` upgrades existing
  installations.
...
- Share SpiderByte configuration, authentication, MCP configuration, and
  sessions with the TUI when both processes resolve the same SpiderByte home.
...
flowchart LR
  ...
  SDK["@spiderbyte/sdk<br/>SpiderByteHarness and Session"]
  Core["v1 agent-core"]
  Home["SpiderByte home<br/>config, auth, MCP, sessions"]
...
- `productName: "spiderbyte-vscode"`
- `version` from `apps/spiderbyte-vscode/package.json`
- `uiMode: "vscode"`

For `0.6.0`, the normal HTTP User-Agent product is therefore
`spiderbyte-vscode/0.6.0`. The version has one source of truth and is not copied
into runtime code or packaging scripts.

### Package boundaries

- `apps/spiderbyte-vscode` depends on `@spiderbyte/sdk`.
- `apps/spiderbyte-vscode` must not depend directly on `@spiderbyte/legacy-agent-core`.
...
- The Webview communicates only through the typed bridge in
  `apps/spiderbyte-vscode/shared`.

## Main components

| Area | Primary implementation |
|---|---|
| Activation and VS Code commands | `apps/spiderbyte-vscode/src/extension.ts` |
| Webview lifecycle | `apps/spiderbyte-vscode/src/SpiderByteWebviewProvider.ts` |
| Webview RPC boundary | `apps/spiderbyte-vscode/src/bridge-handler.ts`, `apps/spiderbyte-vscode/src/handlers` |
| SDK host | `apps/spiderbyte-vscode/src/runtime/spiderbyte-runtime.ts` |
| Session lifecycle and event routing | `apps/spiderbyte-vscode/src/runtime/session-runtime.ts` |
| SDK-to-Webview event conversion | `apps/spiderbyte-vscode/src/runtime/event-adapter.ts` |
| Session replay | `apps/spiderbyte-vscode/src/runtime/replay-adapter.ts` |
| File changes and baselines | `apps/spiderbyte-vscode/src/managers` |
| Legacy migration coordination | `apps/spiderbyte-vscode/src/migration` |
| React UI | `apps/spiderbyte-vscode/webview-ui` |
| Packaging and smoke tests | `apps/spiderbyte-vscode/scripts`, `.github/workflows/ci.yml` |

## Data ownership

### Shared SpiderByte home

The SDK resolves the home directory using the normal SpiderByte rules:

1. system-level `SPIDERBYTE_HOME`, when set;
2. otherwise `~/.spiderbyte`.

The extension does not add a separate `spiderbyte.homeDir` setting and does not pass
...
- `sessions/`
- `session_index.jsonl`
- other SDK-owned SpiderByte data
...
The old `spiderbyte.environmentVariables` setting existed to populate the environment
...
- proxy and other process-level variables are inherited from the Extension
  Host environment.
- a legacy `SPIDERBYTE_SHARE_DIR` in the removed setting is consulted only as an
...
Migration is opt-in and uses `@spiderbyte/legacy-migration` for detection and
translation. The extension coordinates prompts and reports but does not
maintain another config/session translator.

### Sources and target

- default source: `~/.spiderbyte`;
- optional additional source: a valid legacy `SPIDERBYTE_SHARE_DIR` from the removed
  VS Code setting;
- target: the SDK-resolved SpiderByte home.
...
`SpiderByte: Migrate Legacy Data` remains available for manual runs and retries.

The shared marker `.migrated-to-spiderbyte` can contain multiple target homes.
This prevents duplicate migration when the TUI migrated the same source first,
while still allowing a different `SPIDERBYTE_HOME` to be migrated later.
...
resolved SpiderByte home and session ID. The home namespace prevents sessions
...
2. `apps/spiderbyte-vscode` never imports v1 agent-core directly.
3. Shared config and sessions live in the SDK-resolved SpiderByte home; editor
   preferences and baselines remain VS Code-owned.
4. Legacy migration translation stays in `compat/legacy-migration`.
