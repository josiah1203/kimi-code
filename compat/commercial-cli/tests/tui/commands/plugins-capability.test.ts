...
import { log } from '@spiderbyte/sdk';
import { resetCapabilitiesCache, setCapabilities, type Component } from '@spiderbyte/pi-tui';
...
describe('plugins command capability surface', () => {
  ...
  it('removePlugin notes that capability runtimes are left untouched', async () => {
    ...
    expect(statuses.some((s) => s.includes('Restart SpiderByte before reinstalling'))).toBe(true);
  ...
  it('treats only the default catalog (and the dev server) as injectable', () => {
    ...
    expect(
      isDefaultMarketplaceCatalog(undefined, {
        SPIDERBYTE_PLUGIN_MARKETPLACE_URL: 'http://127.0.0.1:60056/marketplace.json',
        SPIDERBYTE_PLUGIN_MARKETPLACE_FROM_DEV_SERVER: '1',
      }),
    ).toBe(true);
    expect(
      isDefaultMarketplaceCatalog(undefined, {
        SPIDERBYTE_PLUGIN_MARKETPLACE_URL: 'https://example.test/marketplace.json',
      }),
    ).toBe(false);
