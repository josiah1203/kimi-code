...
function makeRefreshHost(initial: SpiderByteConfig): {
  current: () => SpiderByteConfig;
  removeProvider: ReturnType<typeof vi.fn<(providerId: string) => Promise<SpiderByteConfig>>>;
  setConfig: ReturnType<typeof vi.fn<(patch: Partial<SpiderByteConfig>) => Promise<SpiderByteConfig>>>;
} {
  ...
  const setConfig = vi.fn(async (patch: Partial<SpiderByteConfig>) => {
...
describe('refreshAllProviderModels', () => {
  ...
  it('refreshes managed Kimi Code against environment endpoints over persisted config', async () => {
    ...
    vi.stubEnv('SPIDERBYTE_BASE_URL', envBaseUrl);
    vi.stubEnv('SPIDERBYTE_OAUTH_HOST', envOauthHost);
    ...
    expect(result.unchanged).toEqual([SPIDERBYTE_PROVIDER_NAME]);
    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(resolveOAuthToken).toHaveBeenCalledWith(SPIDERBYTE_PROVIDER_NAME, envOauthRef);
  });
  ...
  it('matches the managed endpoint even with a trailing slash on the configured baseUrl', async () => {
    vi.stubEnv('SPIDERBYTE_BASE_URL', 'https://api.managed.example.test/coding/v1');
    const host = makeRefreshHost({
      providers: {
        'my-kimi': {
          type: 'kimi',
  ...
  it('refreshes a hand-written managed:kimi-code provider that uses an API key instead of OAuth', async () => {
    ...
    expect(host.current().providers[SPIDERBYTE_PROVIDER_NAME]).toEqual({
      type: 'kimi',
      baseUrl,
      apiKey: 'sk-distributed-key',
    });
