...
import type { CapabilityStatus, PluginSummary } from '@spiderbyte/sdk';
...
describe('plugins selector dialogs', () => {
  it('trusts only built-in Kimi CDN plugin paths', () => {
    ...
    expect(pluginTrustLabel({
      ...
      hasErrors: false,
      source: 'zip-url',
      originalSource: 'https://code.kimi.com/spiderbyte/plugins/curated/superpowers.zip',
    })).toBe('curated');
    expect(pluginTrustLabel({
      id: 'kimi-cu',
      displayName: 'Kimi Computer Use',
    ...
    expect(pluginTrustLabel({
      ...
      hasErrors: false,
      source: 'local-path',
      originalSource: 'https://code.kimi.com/spiderbyte/plugins/official/local',
    })).toBe('third-party');
  });
  ...
  it('recognizes installed plugins by official provenance', () => {
    ...
    expect(isOfficialPluginInstall({
      ...base,
      source: 'zip-url',
      originalSource: 'http://127.0.0.1:58627/spiderbyte/plugins/official/kimi-datasource.zip',
    })).toBe(false);
    expect(isOfficialPluginInstall({
      ...base,
      source: 'zip-url',
      originalSource: 'https://example.test/spiderbyte/plugins/official/kimi-datasource.zip',
    })).toBe(false);
  });
  ...
  it('shows installed Kimi Computer Use and WebBridge plugins as official', () => {
    const installed: PluginSummary[] = [
      ...
      },
      {
        ...
        source: 'zip-url',
        originalSource: 'https://code.kimi.com/spiderbyte/plugins/official/kimi-webbridge.zip',
      },
    ];
  ...
  it('treats only the official Kimi CDN path as a trusted install source', () => {
    expect(isOfficialPluginSource('https://code.kimi.com/spiderbyte/plugins/official/kimi-datasource.zip')).toBe(true);
    ...
    expect(isOfficialPluginSource('https://code.kimi.com/spiderbyte/plugins/curated/superpowers.zip')).toBe(false);
    expect(isOfficialPluginSource('https://code.kimi.com/spiderbyte/plugins/foo.zip')).toBe(false);
    ...
    expect(isOfficialPluginSource('https://example.test/spiderbyte/plugins/official/x.zip')).toBe(false);
    expect(isOfficialPluginSource('http://code.kimi.com/spiderbyte/plugins/official/x.zip')).toBe(false);
    expect(isOfficialPluginSource('http://127.0.0.1:58627/spiderbyte/plugins/official/x.zip')).toBe(false);
