...
import {
  SPIDERBYTE_PLUGIN_MARKETPLACE_URL,
  SPIDERBYTE_PLUGIN_MARKETPLACE_URL_ENV,
} from '#/constant/app';
...
describe('loadPluginMarketplace', () => {
  ...
  it('loads the default CDN marketplace with injectable fetch', async () => {
    ...
    const marketplace = await loadPluginMarketplace({
      workDir: '/tmp/work',
      source: SPIDERBYTE_PLUGIN_MARKETPLACE_URL,
      fetchImpl,
    });

    expect(fetchImpl).toHaveBeenCalledWith(SPIDERBYTE_PLUGIN_MARKETPLACE_URL);
  ...
  it('falls back to the source checkout marketplace when the default CDN cannot be fetched', async () => {
    const previous = process.env[SPIDERBYTE_PLUGIN_MARKETPLACE_URL_ENV];
    delete process.env[SPIDERBYTE_PLUGIN_MARKETPLACE_URL_ENV];
    const fetchImpl = vi.fn(async () => {
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env[SPIDERBYTE_PLUGIN_MARKETPLACE_URL_ENV] = previous;
      }
    }
  });
  ...
  describe('version derivation from a GitHub source', () => {
    ...
    it('does not derive a version from a non-GitHub URL', async () => {
      const entry = await loadEntry('https://code.kimi.com/spiderbyte/plugins/curated/superpowers.zip');
      expect(entry.version).toBeUndefined();
    });
