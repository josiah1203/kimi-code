...
import { resolveKimiCodeOAuthKey } from '@spiderbyte/oauth';
...
describe('kimi-datasource MCP server', () => {
  ...
  it('uses env-scoped credentials and derives the datasource URL from SPIDERBYTE_BASE_URL', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'kimi-datasource-plugin-'));
    const kimiHome = join(tempDir, 'kimi-home');
    const requests: unknown[] = [];
    let child: ChildProcessWithoutNullStreams | undefined;
    ...
    try {
      ...
      child = spawn(process.execPath, [SERVER_ENTRY], {
        ...
        env: {
          ...process.env,
          SPIDERBYTE_HOME: kimiHome,
          SPIDERBYTE_BASE_URL: baseUrl,
          SPIDERBYTE_OAUTH_HOST: oauthHost,
  ...
  it('returns the complete data-source routing contract when tools are listed', async () => {
    ...
    try {
      ...
      child = spawn(process.execPath, [SERVER_ENTRY], {
        cwd: REPO_ROOT,
        env: { ...process.env, SPIDERBYTE_HOME: kimiHome },
        stdio: ['pipe', 'pipe', 'pipe'],
      });
      const client = createRpcClient(child);
...
// truth in @spiderbyte/oauth. The credential file name is the OAuth
