# `@spiderbyte/legacy-agent-core` Dependency Injection Container
...
- **Testing** — `TestInstantiationService` (subpath
  `@spiderbyte/legacy-agent-core/di/test`) exposes direct `.get` / `.stub` so
...
import {
  ...
  ServiceCollection,
  SyncDescriptor,
} from '@spiderbyte/legacy-agent-core';
...
Test files import from the subpath `@spiderbyte/legacy-agent-core/di/test`
(NOT the main package entry — keeps production bundles clean):

```ts
import { TestInstantiationService } from '@spiderbyte/legacy-agent-core/di/test';
...
├── test.ts                      ← subpath barrel (`@spiderbyte/legacy-agent-core/di/test`)
