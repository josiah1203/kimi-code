...
`@spiderbyte/legacy-agent-core` (merged from the former `@moonshot-ai/services`
...
from `index.ts` so importing `@spiderbyte/legacy-agent-core` runs the registration
...
2. **Consumers seed from `getSingletonServiceDescriptors()` directly**.
   Importing `@spiderbyte/legacy-agent-core` loads the package barrel, whose impl
