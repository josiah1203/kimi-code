...
import { log, type GoalSnapshot } from '@spiderbyte/sdk';
import { describe, expect, it, vi } from 'vitest';

import { BannerProvider } from '#/tui/banner/banner-provider';
import { readBannerDisplayState } from '#/tui/banner/state';
import { handleConnectCommand, handleDisconnectCommand } from '#/tui/commands/connect';
...
interface StartupDriver {
  ...
  handleConnectCommand(): Promise<void>;
  handleDisconnectCommand(): Promise<void>;
...
interface StartupMainTuiDriver extends StartupDriver {
  initMainTui(): Promise<boolean>;
}

function makeStartupInput(
  cliOptions: Partial<SpiderByteTUIStartupInput['cliOptions']> = {},
  tuiConfig: Partial<SpiderByteTUIStartupInput['tuiConfig']> = {},
...
function loginRequiredError(): Error & { readonly code: string } {
  return Object.assign(new Error('OAuth provider "managed:spiderbyte" requires login.'), {
...
function makeDriver(harness: ReturnType<typeof makeHarness>, input: SpiderByteTUIStartupInput) {
  const driver = new SpiderByteTUI(harness as never, input) as unknown as StartupDriver;
...
describe('SpiderByteTUI startup', () => {
  ...
  it('hydrates permission/plan defaults after a session-less v2 login', async () => {
    ...
    vi.mocked(promptPlatformSelection).mockResolvedValue('spiderbyte');
    await handleConnectCommand(driver as any);
  ...
  it('passes the CLI model override when creating a fresh startup session', async () => {
    ...
    const driver = makeDriver(harness, makeStartupInput({ model: 'spiderbyte/k2.5' }));

    await expect(driver.init()).resolves.toBe(false);

    expect(harness.createSession).toHaveBeenCalledWith({
      workDir: '/tmp/proj-a',
      model: 'spiderbyte/k2.5',
  ...
  it('applies the CLI model override when resuming a startup session', async () => {
    ...
    const driver = makeDriver(
      harness,
      makeStartupInput({ continue: true, model: 'spiderbyte/k2.5' }),
    );

    await expect(driver.init()).resolves.toBe(true);

    expect(session.setModel).toHaveBeenCalledWith('spiderbyte/k2.5');
    expect(driver.state.appState.model).toBe('spiderbyte/k2.5');
  });
  ...
  it('exits after picking another cwd from the startup picker', async () => {
    ...
    await expect((driver as unknown as StartupMainTuiDriver).initMainTui()).resolves.toBe(false);
    await (driver as unknown as { bootstrapFromPicker(): Promise<void> }).bootstrapFromPicker();
  ...
  it('tracks login with already_logged_in when a token already exists', async () => {
    ...
    const harness = makeHarness(session, {
      auth: {
        status: vi.fn(async () => ({
          providers: [{ providerName: 'managed:spiderbyte', hasToken: true }],
        })),
        login: vi.fn(async () => {}),
        logout: vi.fn(),
        getManagedUsage: vi.fn(),
  ...
  it('logs login failures with session context', async () => {
    ...
    const session = makeSession();
    const loginError = new Error('Failed to list SpiderByte models (HTTP 402).');
    const harness = makeHarness(session, {
      auth: {
        status: vi.fn(async () => ({ providers: [] })),
        login: vi.fn(async () => {
    ...
    try {
      ...
      expect(warn).toHaveBeenCalledWith(
        'login failed',
        expect.objectContaining({
          providerName: 'managed:spiderbyte',
          alreadyLoggedIn: false,
          sessionId: 'ses-1',
          error: expect.objectContaining({
            message: 'Failed to list SpiderByte models (HTTP 402).',
          }),
        }),
      );
    } finally {
  ...
  it('tracks logout after managed credentials and session state are cleared', async () => {
    ...
    const harness = makeHarness(session, {
      getConfig: vi.fn(async () => ({
        models: {
          k2: { provider: 'managed:spiderbyte', model: 'moonshot-v1', maxContextSize: 100 },
        },
        providers: { 'managed:spiderbyte': { type: 'kimi' } },
      })),
      auth: {
        status: vi.fn(async () => ({
          providers: [{ providerName: 'managed:spiderbyte', hasToken: true }],
    ...
    vi.mocked(promptLogoutProviderSelection).mockResolvedValue('managed:spiderbyte');
    await handleDisconnectCommand(driver as any);

    expect(harness.auth.logout).toHaveBeenCalledWith('managed:spiderbyte');
    ...
    expect(harness.track).toHaveBeenCalledWith('logout', { provider: 'managed:spiderbyte' });
  });
  ...
  it('can log out a stale managed entry even after the OAuth token is gone', async () => {
    ...
    const harness = makeHarness(session, {
      ...
      auth: {
        ...
        status: vi.fn(async () => ({
          providers: [{ providerName: 'managed:spiderbyte', hasToken: false }],
        })),
        login: vi.fn(async () => {}),
        logout: vi.fn(),
        getManagedUsage: vi.fn(),
  ...
  it('does not mount the footer when resuming a missing session fails', async () => {
    ...
    ) as unknown as StartupMainTuiDriver;
  ...
  it('writes display state after rendering a once banner', async () => {
    ...
    process.env['SPIDERBYTE_HOME'] = dir;
