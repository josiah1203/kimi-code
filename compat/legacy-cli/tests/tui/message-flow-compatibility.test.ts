...
import { SPIDERBYTE_PLUGIN_MARKETPLACE_URL } from '#/constant/app';
...
import { SpiderByteTUI, type SpiderByteTUIStartupInput, type TUIState } from '#/tui/spiderbyte-tui';
...
function makeStartupInput(): SpiderByteTUIStartupInput {
  return {
    cliOptions: {
      session: undefined,
      continue: false,
...
async function makeDriver(
  ...
  startupInput: SpiderByteTUIStartupInput = makeStartupInput(),
): Promise<{
  ...
  const driver = new SpiderByteTUI(harness as never, startupInput) as unknown as MessageDriver;
...
const originalKimiCodeHome = process.env['SPIDERBYTE_HOME'];
const originalPluginMarketplaceUrl = process.env['SPIDERBYTE_PLUGIN_MARKETPLACE_URL'];
const originalVisual = process.env['VISUAL'];
const originalEditor = process.env['EDITOR'];

async function makeTempHome(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'spiderbyte-tui-'));
  tempDirs.push(dir);
  return dir;
}

async function makeExportedSessionZip(content = 'session zip'): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'spiderbyte-feedback-export-'));
  tempDirs.push(dir);
...
afterEach(async () => {
  ...
  if (originalKimiCodeHome === undefined) {
    ...
    process.env['SPIDERBYTE_HOME'] = originalKimiCodeHome;
  }
  ...
  if (originalPluginMarketplaceUrl === undefined) {
    delete process.env['SPIDERBYTE_PLUGIN_MARKETPLACE_URL'];
  } else {
    process.env['SPIDERBYTE_PLUGIN_MARKETPLACE_URL'] = originalPluginMarketplaceUrl;
  }
...
describe('SpiderByteTUI message flow', () => {
  ...
  it('lazily creates the session on the first message (v2 engine)', async () => {
    ...
    const startupInput: SpiderByteTUIStartupInput = {
      ...makeStartupInput(),
  ...
  it('hydrates lazy config defaults on a sessionless /reload (v2 engine)', async () => {
    ...
    process.env['SPIDERBYTE_HOME'] = homeDir;
    const session = makeSession({ id: 'ses-lazy' });
  ...
  it('tracks theme changes from slash commands', async () => {
    process.env['SPIDERBYTE_HOME'] = await makeTempHome();
    const { driver, harness } = await makeDriver();
    harness.track.mockClear();
...
`,
  ...
  it('removes debug timing status from undone turns', async () => {
    const { driver, session } = await makeDriver();
    const previousDebug = process.env['SPIDERBYTE_DEBUG'];
    process.env['SPIDERBYTE_DEBUG'] = '1';
    try {
      driver.handleUserInput('hello');
      driver.sessionEventHandler.handleEvent(
        {
      ...
      if (previousDebug === undefined) {
        delete process.env['SPIDERBYTE_DEBUG'];
      } else {
        process.env['SPIDERBYTE_DEBUG'] = previousDebug;
      }
    }
  });
  ...
  it('renders /status using the active session runtime status', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(output).toContain('>_ SpiderByte');
  ...
  it('shows a quota note after installing a quota-consuming official plugin', async () => {
    const session = makeSession({
      installPlugin: vi.fn(async () => ({
        ...
        hasErrors: false,
        source: 'zip-url',
        originalSource: 'https://code.kimi.com/spiderbyte/plugins/official/kimi-datasource.zip',
      })),
    });
    const { driver } = await makeDriver(session);
    ...
    driver.handleUserInput(
      '/plugins install https://code.kimi.com/spiderbyte/plugins/official/kimi-datasource.zip',
    );
  ...
  it('loads a local plugin marketplace file and installs from it', async () => {
        ...
        plugins: [
          {
            ...
            source: 'https://code.kimi.com/spiderbyte/plugins/official/kimi-datasource.zip',
          },
        ],
      }),
      'utf8',
    );
    process.env['SPIDERBYTE_PLUGIN_MARKETPLACE_URL'] = marketplacePath;
    const session = makeSession();
    const { driver } = await makeDriver(session);
    ...
    await vi.waitFor(() => {
      expect(session.installPlugin).toHaveBeenCalledWith(
        'https://code.kimi.com/spiderbyte/plugins/official/kimi-datasource.zip',
      );
    });
    await vi.waitFor(() => {
      const transcript = stripSgr(renderTranscript(driver));
  ...
  it('installs default marketplace entries through plain install', async () => {
    ...
    try {
      ...
      expect(globalThis.fetch).toHaveBeenCalledWith(SPIDERBYTE_PLUGIN_MARKETPLACE_URL);
    } finally {
  ...
  it('shows an inline Official error when the marketplace is unreachable, keeping the panel open', async () => {
    const originalFetch = globalThis.fetch;
    process.env['SPIDERBYTE_PLUGIN_MARKETPLACE_URL'] = 'https://example.test/marketplace.json';
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => {
  ...
  it('applies /model selection with inline thinking state', async () => {
    ...
    expect(pickerOutput).toMatch(/Kimi K2\s+SpiderByte ← current/);
    expect(pickerOutput).toMatch(/❯ Kimi Turbo\s+SpiderByte/);
  ...
  it('refreshes only OAuth provider models before opening /model picker', async () => {
    ...
    const tui = driver as unknown as SpiderByteTUI;
    ...
    const refreshOAuthProviderModels = vi.fn(async () => {
      ...
      return { changed: [], unchanged: ['managed:spiderbyte'], failed: [] };
    });
    (
