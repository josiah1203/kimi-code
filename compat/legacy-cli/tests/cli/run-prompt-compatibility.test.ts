 ...
 * Run: pnpm -C apps/cli exec vitest run test/cli/run-prompt.test.ts
 */
...
const LEGACY_PLATFORM_MODE_DIAGNOSTIC =
  'SpiderByte compatibility mode: the legacy Kimi engine is active. Platform services are unavailable; this was requested by SPIDERBYTE_LEGACY_FLAG.\n';
...
// exercise the dispatch without pulling in the real SpiderByte Agent Core graph.
vi.mock('../../src/cli/run-print', () => ({
...
describe('runPrompt', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('SPIDERBYTE_EXPERIMENTAL_FLAG', '');
  ...
  it('uses the CLI model override when creating a fresh prompt session', async () => {
    await runPrompt(opts({ model: 'spiderbyte/k2.5' }), '1.2.3-test', {
    ...
    expect(mocks.initializeTelemetry).toHaveBeenCalledWith(
      expect.objectContaining({ model: 'spiderbyte/k2.5' }),
    );
  });
  ...
  it('applies the CLI model override to resumed prompt sessions', async () => {
    await runPrompt(opts({ session: 'ses_existing', model: 'spiderbyte/k2.5' }), '1.2.3-test', {
      stdout: { write: vi.fn(() => true) },
      stderr: { write: vi.fn(() => true) },
    });

    expect(mocks.harnessResumeSession).toHaveBeenCalledWith({ id: 'ses_existing' });
    expect(mocks.session.setModel).toHaveBeenCalledWith('spiderbyte/k2.5');
    expect(mocks.initializeTelemetry).toHaveBeenCalledWith(
      expect.objectContaining({ model: 'spiderbyte/k2.5' }),
    );
  });
  ...
  it('emits the version first in text mode on the default v2 engine', async () => {
    vi.stubEnv('SPIDERBYTE_LEGACY_FLAG', '');
    vi.stubEnv('SPIDERBYTE_EXPERIMENTAL_FLAG', '');
    const stdout = writer();
    const stderr = writer();
    ...
    expect(mocks.runPrint).toHaveBeenCalled();
  ...
  it('uses the legacy engine when legacy wins over the experimental flag', async () => {
    ...
    expect(mocks.runPrint).not.toHaveBeenCalled();
