...
import type { createSpiderByteDeviceId as createSpiderByteDeviceIdFn } from '@spiderbyte/oauth';
...
type CreateSpiderByteDeviceId = typeof createSpiderByteDeviceIdFn;
...
const mocks = vi.hoisted(() => {
  ...
  class TuiConfigParseError extends Error {
    ...
    constructor(fallback: TuiConfigFallback) {
      super('Invalid TUI config in ~/.spiderbyte/tui.toml; using defaults.');
  ...
  return {
    ...
    spiderByteHarnessConstructor: vi.fn(),
    ...
    createSpiderByteDeviceId: vi.fn<CreateSpiderByteDeviceId>(() => 'device-1'),
...
vi.mock('@spiderbyte/sdk', async (importOriginal) => {
  ...
  const makeHarnessStub = (args: unknown[]) => {
    ...
    const homeDir = options?.homeDir ?? '/tmp/spiderbyte-test-home';
    return {
      homeDir,
      auth: {
  ...
  return {
    ...
    createSpiderByteHarness: (...args: unknown[]) => {
      mocks.spiderByteHarnessConstructor(...args);
...
vi.mock('@spiderbyte/oauth', async () => {
  ...
  return {
    ...
    SPIDERBYTE_PROVIDER_NAME: 'spiderbyte',
  };
});
...
describe('runShell', () => {
  beforeEach(() => {
    vi.stubEnv('SPIDERBYTE_LEGACY_FLAG', '1');
  });
  ...
  afterEach(() => {
    ...
    mocks.createSpiderByteDeviceId.mockImplementation(() => 'device-1');
    mocks.resolveSpiderByteHome.mockImplementation(
      (homeDir?: string) => homeDir ?? '/tmp/spiderbyte-test-home',
    );
  ...
  it('constructs SpiderByteHarness and SpiderByteTUI with startup input', async () => {
    mocks.loadTuiConfig.mockResolvedValue({
      theme: 'dark',
      editorCommand: null,
      notifications: { enabled: true, condition: 'unfocused' },
    ...
    expect(mocks.spiderByteHarnessConstructor).toHaveBeenCalledWith(
    ...
    expect(mocks.createSpiderByteDeviceId).toHaveBeenCalledWith(
      '/tmp/spiderbyte-test-home',
      expect.any(Object),
    );
    expect(mocks.initializeTelemetry).toHaveBeenCalledWith({
      homeDir: '/tmp/spiderbyte-test-home',
      deviceId: 'device-1',
      enabled: true,
  ...
  it('tracks first launch when device id creation reports first launch', async () => {
    ...
    mocks.tuiStart.mockResolvedValue(undefined);
    mocks.createSpiderByteDeviceId.mockImplementationOnce((homeDir, options) => {
  ...
  it('registers first launch before harness construction can create the device id', async () => {
    ...
    mocks.createSpiderByteDeviceId.mockImplementation((homeDir, options) => {
      const deviceId = `device-for-${homeDir}`;
    ...
    expect(mocks.createSpiderByteDeviceId).toHaveBeenNthCalledWith(
      1,
      '/tmp/spiderbyte-test-home',
      expect.objectContaining({ onFirstLaunch: expect.any(Function) }),
    );
    expect(mocks.createSpiderByteDeviceId.mock.invocationCallOrder[0]).toBeLessThan(
      mocks.spiderByteHarnessConstructor.mock.invocationCallOrder[0]!,
    );
    expect(mocks.spiderByteHarnessConstructor).toHaveBeenCalledWith(
      expect.objectContaining({ homeDir: '/tmp/spiderbyte-test-home' }),
    );
    expect(mocks.harnessTrack).toHaveBeenCalledWith('first_launch');
  });
  ...
  it('detects auto theme and forwards config parse warnings as startup notice', async () => {
    ...
    const [, , startupInput] = mocks.kimiTuiConstructor.mock.calls[0]!;
    expect(startupInput).toMatchObject({
      startupNotice: 'SpiderByte compatibility mode: the legacy Kimi engine is active. Platform services are unavailable; this was requested by SPIDERBYTE_LEGACY_FLAG.\nInvalid TUI config in ~/.spiderbyte/tui.toml; using defaults.',
      tuiConfig: {
        theme: 'auto',
        editorCommand: 'vim',
        notifications: { enabled: true, condition: 'always' },
  ...
  it('surfaces an invalid target config as an error for spyderbyte migrate, not silently', async () => {
    mocks.loadTuiConfig.mockResolvedValue({
      theme: 'dark',
      editorCommand: null,
      notifications: { enabled: true, condition: 'unfocused' },
    ...
    mocks.harnessGetConfig.mockRejectedValue(
      new Error('Invalid configuration in ~/.spiderbyte/config.toml'),
    );

    // A broken config.toml must fail loudly — `spyderbyte migrate` must not swallow
