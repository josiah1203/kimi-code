 ...
 *     --location=global @spiderbyte/cli` — verified on npm
 *     11.13.0.
...
//     do on POSIX)
const PACKAGE_NAME_MARKERS = ['@spiderbyte/cli', '@moonshot-ai\\spiderbyte'];
...
export async function userShellPath() {
  ...
  return new Promise((resolve) => {
    ...
    const probe =
      'printf "<<<SPIDERBYTE_PATH_BEGIN>>>%s<<<SPIDERBYTE_PATH_END>>>\\n" "$PATH"';
    ...
    child.on('close', (code) => {
      ...
      const match = stdout.match(
        /<<<SPIDERBYTE_PATH_BEGIN>>>([\s\S]*?)<<<SPIDERBYTE_PATH_END>>>/,
      );
