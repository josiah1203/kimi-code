/**
 * Scenario: SpiderByteHarness session creation and resume transport behavior.
 ...
 * Run: pnpm -C packages/sdk exec vitest run test/create-session-transport.test.ts
 */
...
import { createSpiderByteHarness, SpiderByteHarness } from '#/index';
import type { SpiderByteError } from '#/index';
...
async function writeReviewerAgent(workDir: string): Promise<void> {
  const agentDir = join(workDir, '.spiderbyte', 'agents');
...
describe('SpiderByteHarness.createSession transport link', () => {
...
`,
  ...
  it('rebinds an active session when resumeSession receives a new Kaos', async () => {
    ...
    const harness = new SpiderByteHarness(rpc, {
      homeDir: '/tmp/home',
...
function coreSessionIds(harness: SpiderByteHarness): readonly string[] {
  const core = (
