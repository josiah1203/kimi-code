 ...
 *     all from `@spiderbyte/legacy-agent-core/session/store`.
 *   - `Session` (constructor + `resume()` + `getReadyAgent()`), from
 *     `@spiderbyte/legacy-agent-core`; `localKaos` from `@spiderbyte/kaos`. After
...
import {
  ...
  encodeWorkDirKey,
  normalizeWorkDir,
} from '@spiderbyte/legacy-agent-core/session/store/index';
import { Session, type SDKSessionRPC } from '@spiderbyte/legacy-agent-core';
import { LocalKaos } from '@spiderbyte/kaos';
