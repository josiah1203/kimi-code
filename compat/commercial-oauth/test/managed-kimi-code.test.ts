...
describe('provisionManagedKimiCodeConfig', () => {
  ...
  it('writes the managed provider, models, services, and default model through an adapter', async () => {
    const config: ManagedSpiderByteConfigShape = {
      providers: {
        custom: {
          type: 'kimi',
          apiKey: 'sk-existing',
    ...
    expect(config.providers[SPIDERBYTE_PROVIDER_NAME]).toMatchObject({
      type: 'kimi',
      baseUrl: 'https://api.kimi.com/coding/v1',
      apiKey: '',
  ...
  it('removes managed provider, models, services, and default model on logout', () => {
    const config: ManagedSpiderByteConfigShape = {
      ...
      raw: {
        ...
        providers: {
          [SPIDERBYTE_PROVIDER_NAME]: { type: 'kimi' },
          custom: { type: 'kimi' },
        },
        models: {
          'kimi-code/kimi-for-coding': {
    ...
    expect(config.providers[SPIDERBYTE_PROVIDER_NAME]).toBeUndefined();
...
describe('supports_thinking_type', () => {
  ...
  it('forces default thinking on when the selected default model is thinking-only', async () => {
    const config: ManagedSpiderByteConfigShape = { providers: {}, thinking: { enabled: false } };
...
describe('applyManagedApiKeyProviderModels', () => {
  ...
  it('writes protocol routing fields for anthropic-protocol models', () => {
    const config: ManagedSpiderByteConfigShape = { providers: {}, models: {} };
...
describe('managed protocol routing', () => {
  ...
  it('keeps the kimi protocol and baseUrl when the model has no anthropic protocol', () => {
    ...
    expect(config.models?.['kimi-code/kimi-for-coding']?.provider).toBe(SPIDERBYTE_PROVIDER_NAME);
