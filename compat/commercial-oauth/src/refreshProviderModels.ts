 ...
 * {@link ManagedSpiderByteConfigShape} (the oauth package's own minimal config shape)
 * rather than the SDK's full `SpiderByteConfig`, so this module has no dependency on
 * `agent-core` / the SDK and can be reused by both the CLI and the daemon.
 */
export interface RefreshProviderHost {
  getConfig(): Promise<ManagedSpiderByteConfigShape>;
  removeProvider(providerId: string): Promise<ManagedSpiderByteConfigShape>;
  setConfig(patch: ManagedSpiderByteConfigShape): Promise<ManagedSpiderByteConfigShape>;
  resolveOAuthToken(providerName: string, oauthRef?: ManagedSpiderByteOAuthRef): Promise<string>;
  /**
...
interface ProviderView {
  ...
  readonly oauth?: ManagedSpiderByteOAuthRef;
...
function readProvider(
  config: ManagedSpiderByteConfigShape,
  providerId: string,
...
function providerAliasKeys(config: ManagedSpiderByteConfigShape, providerId: string): Set<string> {
...
function providerModelsEqual(
  config: ManagedSpiderByteConfigShape,
  nextConfig: ManagedSpiderByteConfigShape,
  providerId: string,
  aliasKeys: ReadonlySet<string>,
): boolean {
  return (
...
function providerConfigSnapshot(config: ManagedSpiderByteConfigShape, providerId: string): string {
...
function clampDanglingDefault(config: ManagedSpiderByteConfigShape): void {
...
export async function refreshProviderModels(
  ...
  // ---------------------------------------------------------------------------
  const managedProvider = readProvider(config, SPIDERBYTE_PROVIDER_NAME);
  const managedWanted = targetId === undefined || targetId === SPIDERBYTE_PROVIDER_NAME;
  if (
    managedWanted &&
    ...
    try {
      ...
      const accessToken = await host.resolveOAuthToken(SPIDERBYTE_PROVIDER_NAME, auth.oauthRef);
      ...
      if (models.length > 0) {
        ...
        restoreProviderAliases(
          next,
          preserveUserProviderAliases(config, SPIDERBYTE_PROVIDER_NAME, refreshedAliasKeys),
        );
        ...
        if (providerModelsEqual(config, next, SPIDERBYTE_PROVIDER_NAME, refreshedAliasKeys)) {
          unchanged.push(SPIDERBYTE_PROVIDER_NAME);
        } else {
          ...
          await host.removeProvider(SPIDERBYTE_PROVIDER_NAME);
          ...
          changed.push({
            providerId: SPIDERBYTE_PROVIDER_NAME,
      ...
      failed.push({
        provider: SPIDERBYTE_PROVIDER_NAME,
  ...
  for (const providerId of Object.keys(config.providers)) {
    ...
    try {
      ...
      const aliasPrefix =
        providerId === SPIDERBYTE_PROVIDER_NAME ? `${SPIDERBYTE_PLATFORM_ID}/` : `${providerId}/`;
      const next = structuredClone(config);
  ...
  >();
  for (const providerId of Object.keys(config.providers)) {
    if (providerId === SPIDERBYTE_PROVIDER_NAME) continue;
    if (isOpenPlatformId(providerId)) continue;
    const provider = readProvider(config, providerId);
    if (provider === undefined) continue;
