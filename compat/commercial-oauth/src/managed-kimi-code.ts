...
export const SPIDERBYTE_PLATFORM_ID = 'kimi-code';
export const SPIDERBYTE_PROVIDER_NAME = 'managed:kimi-code';
...
export interface ManagedKimiCodeProvisionResult {
  readonly providerName: typeof SPIDERBYTE_PROVIDER_NAME;
...
export interface ManagedSpiderByteOAuthRef {
  readonly storage: 'file' | 'keyring';
  readonly key: string;
  readonly oauthHost?: string | undefined;
}

export interface ManagedSpiderByteOAuthRefInput {
  readonly storage?: 'file' | 'keyring' | undefined;
  readonly key?: string | undefined;
  readonly oauthHost?: string | undefined;
}

export interface ManagedSpiderByteRuntimeAuth {
  readonly baseUrl?: string | undefined;
  readonly oauthRef: ManagedSpiderByteOAuthRef;
}

export interface ManagedKimiLoginAuth {
  readonly baseUrl?: string | undefined;
  readonly oauthHost?: string | undefined;
  readonly oauthRef?: ManagedSpiderByteOAuthRef | undefined;
}
...
export interface ManagedKimiProviderConfig {
  ...
  oauth?: ManagedSpiderByteOAuthRef | undefined;
...
export interface ManagedSpiderByteConfigShape {
...
export interface ManagedSpiderByteConfigAdapter<TConfig> {
...
export interface ProvisionManagedKimiCodeConfigOptions<TConfig> {
  readonly adapter: ManagedSpiderByteConfigAdapter<TConfig>;
  readonly accessToken: string;
  readonly baseUrl?: string | undefined;
  readonly oauthKey?: string | undefined;
  readonly oauthHost?: string | undefined;
...
function managedModelKey(modelId: string): string {
  return `${SPIDERBYTE_PLATFORM_ID}/${modelId}`;
}
...
function configuredOAuthRef(
  oauthRef: ManagedSpiderByteOAuthRefInput | undefined,
): ManagedSpiderByteOAuthRef | undefined {
...
export function resolveKimiCodeRuntimeAuth(options: {
  ...
  readonly configuredOAuthRef?: ManagedSpiderByteOAuthRefInput | undefined;
  readonly env?: ManagedKimiEnv | undefined;
}): ManagedSpiderByteRuntimeAuth {
...
export function applyManagedKimiCodeConfig(
  ...
  config.providers[SPIDERBYTE_PROVIDER_NAME] = {
    type: 'kimi',
    baseUrl,
    apiKey: '',
    oauth,
  ...
  for (const [key, model] of Object.entries(existingModels)) {
    if (
      isRecord(model) &&
      model['provider'] === SPIDERBYTE_PROVIDER_NAME &&
  ...
  for (const model of options.models) {
    ...
    existingModels[key] = mergeRefreshedModelAlias(
      existing,
      toManagedModelAlias(SPIDERBYTE_PROVIDER_NAME, model),
...
export function applyManagedKimiCodeLogoutConfig(config: ManagedSpiderByteConfigShape): void {
  delete config.providers[SPIDERBYTE_PROVIDER_NAME];

  let removedDefaultModel = false;
  const existingModels = config.models ?? {};
  for (const [key, model] of Object.entries(existingModels)) {
    if (!isRecord(model) || model['provider'] !== SPIDERBYTE_PROVIDER_NAME) continue;
    delete existingModels[key];
  ...
  if (config['defaultProvider'] === SPIDERBYTE_PROVIDER_NAME) {
...
function canPreserveDefaultModel(
  ...
  return isRecord(existing) && existing['provider'] !== SPIDERBYTE_PROVIDER_NAME;
}

export function clearManagedKimiCodeConfig(
  config: ManagedSpiderByteConfigShape,
): ManagedKimiCodeCleanupResult {
  const removedProvider = Object.hasOwn(config.providers, SPIDERBYTE_PROVIDER_NAME);
  delete config.providers[SPIDERBYTE_PROVIDER_NAME];
  ...
  return {
    providerName: SPIDERBYTE_PROVIDER_NAME,
    removedProvider,
    removedModels,
    defaultModelCleared,
    removedServices,
...
export async function provisionManagedKimiCodeConfigAfterLogin(
  options: ProvisionManagedKimiCodeConfigOptions<ManagedSpiderByteConfigShape>,
