 ...
 * `process.env.SPIDERBYTE_SERVER_URL ?? http://127.0.0.1:58627`.
 *
...
const BASE_URL = process.env['SPIDERBYTE_SERVER_URL'] ?? 'http://127.0.0.1:58627';
...
describe('DaemonClient session action helpers', () => {
  ...
  it('model catalog helpers call the catalog and action-suffix routes', async () => {
    ...
    const calls: FetchCall[] = [];
    const model = testModel({ model: 'spiderbyte/kimi-for-coding' });
    ...
    expect(calls.map((call) => [call.init.method, call.url])).toEqual([
      ...
      ['POST', 'http://server.example.test/api/v1/models/spiderbyte%2Fkimi-for-coding:set_default'],
