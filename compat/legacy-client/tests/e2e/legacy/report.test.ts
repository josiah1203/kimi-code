...
describe('server-e2e report', () => {
  ...
  it('records createCaseLogger entries under the active case', () => {
    const reportDir = tmpReportDir();
    resetReportDir(reportDir);
    const previousReportDir = process.env['SPIDERBYTE_SERVER_E2E_REPORT_DIR'];
    process.env['SPIDERBYTE_SERVER_E2E_REPORT_DIR'] = reportDir;
    try {
      ...
      if (previousReportDir === undefined) {
        delete process.env['SPIDERBYTE_SERVER_E2E_REPORT_DIR'];
      } else {
        process.env['SPIDERBYTE_SERVER_E2E_REPORT_DIR'] = previousReportDir;
      }
    }
  });
