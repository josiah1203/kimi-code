...
const WINDOWS_DOCTOR_SCRIPT =
  '$candidates = @($env:SPIDERBYTE_CU_WINDOWS_EXE); ' +
  "if ($env:SPIDERBYTE_CU_WINDOWS_HOME) { $candidates += (Join-Path $env:SPIDERBYTE_CU_WINDOWS_HOME 'kimi-cu.exe') }; " +
  ...
  async function install(report: CapabilityInstallReporter): Promise<void> {
    ...
    if (installPlugin) {
      ...
      try {
        ...
        throw new Error(
          'Kimi Computer Use plugin files are still in use by the current SpiderByte process. Restart SpiderByte, then install again.',
          { cause: error },
        );
      }
    }
