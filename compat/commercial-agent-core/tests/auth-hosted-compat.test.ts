...
describe('OAuthService', () => {
  ...
  it('startLogin honors SPIDERBYTE_BASE_URL / SPIDERBYTE_OAUTH_HOST for the login environment', async () => {
    vi.stubEnv('SPIDERBYTE_BASE_URL', 'https://env-api.example.com/coding/v1');
    vi.stubEnv('SPIDERBYTE_OAUTH_HOST', 'https://env-auth.example.com');
    stubManagedModelsFetch();
    toolkit.login.mockImplementation((_provider, options) => {
      options.onDeviceCode(deviceAuth);
      return Promise.resolve({ providerName: OAUTH_PROVIDER, ok: true });
