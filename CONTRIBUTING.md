...
SpiderByte already has opinions on CLI/TUI behavior, agent workflows, and public APIs. If your change shifts that direction, open an issue first so we can align before you invest time in a PR.
...
- `apps/cli` — canonical CLI / TUI (`spyderbyte`)
- `apps/inspect` — private local inspection application
- `packages/sdk` — public TypeScript SDK (`@spiderbyte/sdk`)
- `packages/agent-core`, `kosong`, `kaos`, `oauth`, `telemetry` — Open Core packages
- `compat/` — explicitly excluded legacy and commercial compatibility material
