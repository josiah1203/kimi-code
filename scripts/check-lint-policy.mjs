...
if (result.error !== undefined || result.status !== 0 || (result.stdout ?? '').trim().length === 0) {
  const detail = result.error?.message ?? (result.stderr?.trim() || `oxlint exited with status ${String(result.status)}`);
  console.error(`Lint policy could not run oxlint: ${detail}`);
  process.exitCode = 1;
} else {
