...
- web: 侧边栏会话列表新增平铺视图。
- SpiderByte Computer Use 插件新增 Windows x64 支持，通过 `/plugins` 安装。
- 会话空闲过久后恢复或发送消息时，现将会弹出缓存过期提醒。将 [cache_expiry_hint](https://github.com/SpiderByte/spiderbyte/tree/main/docs/zh/configuration/config-files.html#tui-toml) 设为 `false` 可关闭。
...
- 安装 SpiderByte WebBridge 后现在会显示浏览器扩展链接与激活步骤。
...
- 修复 `spyderbyte -p` 未等待后台任务与子 Agent 完成就退出的问题。
...
- `/plugins` 市场新增 SpiderByte Computer Use 与 SpiderByte WebBridge 官方内置插件，安装时自动配置托管运行时，中断后可重试。
...
- 交互式 TUI 启动时不再立即创建会话。
- 插件市场的合作伙伴标签页更名为 Curated，并说明其内容为 SpiderByte 合作伙伴提供的第三方插件。
...
- CLI 各界面（交互式 TUI、`spyderbyte -p`、`spyderbyte acp` 等）默认运行在 SpiderByte Agent Core 引擎上；设置 `SPIDERBYTE_LEGACY_FLAG=1` 可回退旧引擎。

## 0.32.0（2026-08-04）

### 新功能

- 新增四个 hook 事件：`TurnStarted`、`UserPromptQueued`、`TaskStarted` 和 `SessionHeartbeat`。在 `config.toml` 的 `[[hooks]]` 下配置，详见 [Hooks](https://github.com/SpiderByte/spiderbyte/tree/main/docs/zh/customization/hooks.html)。

### 优化

- `[loop_control]` 两个配置键改名：`max_retries_per_step` → `max_attempts_per_step`、`max_steps_per_run` → `max_steps_per_turn`；旧键不再生效，启动时会有改名警告，详见 [loop_control](https://github.com/SpiderByte/spiderbyte/tree/main/docs/zh/configuration/config-files.html#loop-control)。
- 新增 `[token_counting]` 配置节：供应商不上报 token 用量时，可将上下文大小显示切换为本地估算，详见 [token_counting](https://github.com/SpiderByte/spiderbyte/tree/main/docs/zh/configuration/config-files.html#token-counting)。
...
- 修复启动 spyderbyte web 时偶发的 “model is not configured” 错误。
...
- 插件可贡献系统提示词，通过 `spyderbyte.plugin.json` 中的 `systemPrompt` 或 `systemPromptPath` 声明。
...
- 安装会计入套餐额度的官方插件（如 SpiderByte Datasource）后，显示额度说明。
...
- web: 支持 Markdown 文件定义 agent，声明 system prompt、名称、描述和工具权限。[查看文档](https://github.com/SpiderByte/spiderbyte/tree/main/docs/en/customization/agents.html#agent-file-format)
- web: 可通过 SYSTEM.md 永久覆盖主 agent 的系统提示。[查看文档](https://github.com/SpiderByte/spiderbyte/tree/main/docs/en/customization/agents.html#overriding-the-main-agent-s-system-prompt-with-system-md)
- web: 可通过 config.toml 在所有会话中统一启用/禁用工具。[查看文档](https://github.com/SpiderByte/spiderbyte/tree/main/docs/en/configuration/config-files.html#tools)
- 附加到提示词的视频现在会随提示词一起送达模型，无需额外的工具轮次。
- ACP 客户端现支持选择思考强度。
- 新增 Agent 循环与后台任务限制的环境变量覆盖：`SPIDERBYTE_LOOP_MAX_STEPS_PER_TURN`、`SPIDERBYTE_LOOP_MAX_RETRIES_PER_STEP` 和 `SPIDERBYTE_BACKGROUND_MAX_RUNNING_TASKS`。
...
- web: 在透明图片下方显示棋盘格画布。
- 移除定时任务工具描述中对不存在的 `spyderbyte resume` 命令的引用。
...
- `spyderbyte web` 服务器改为全程前台运行：`/web` 斜杠命令现在总是启动新服务器，`spyderbyte web kill` 与 `spyderbyte web ps` 子命令已移除，前台服务器按 Ctrl+C 即可停止。`spyderbyte server kill` 保留为废弃回退，仅能停止 0.28.0 之前版本启动的服务器。
...
- **破坏性变更：** 
  - `spyderbyte server` 命令树已被废弃，请使用 `spyderbyte web` 代替。
  - `spyderbyte web` 现在在当前终端前台运行并打开浏览器，按 Ctrl+C 停止。
...
- 使用 API key 调用 SpiderByte 编程模型时，现在会自动拉取最新模型列表。
...
- 修复对同一会话重复执行 /export-debug-zip 或 spyderbyte export 会覆盖上一份压缩包的问题；文件名现包含时间戳。
...
- 修复经 Anthropic 协议接入的 SpiderByte 供应商模型错误显示思考强度选项的问题。
...
- Anthropic 兼容与 SpiderByte 的 preserved-thinking 端点现在原样回放空思考内容，不再替换为占位空格。
- 旧版迁移在多个 SpiderByte 主目录之间保持幂等，损坏或无法映射的会话现在会明确报告，不再静默跳过。
...
- 新增内置 `/check-spiderbyte-docs` Skill，自动基于官方文档回答 SpiderByte 产品问题并附来源链接。

### 优化

- 对齐 `spyderbyte -p` 在各引擎的行为：`print_background_mode` 与 `print_max_turns` 生效，`/goal` 会运行到目标结束。
- `spyderbyte -p` 默认在后台任务未完成时保持运行，等待与轮次实际上不设上限，并把完成结果反馈给主 Agent。如需恢复旧的一轮后退出，可设置 `print_background_mode = "exit"` 或 `"drain"`。
- `spyderbyte -p` 后台任务和子 Agent 默认不再超时（交互模式不变）；如需恢复限制，可设置 `[background] bash_task_timeout_s` 或 `[subagent] timeout_ms`。
- 子 Agent 超时统一默认为 2 小时，可通过 `[subagent] timeout_ms` 或 `SPIDERBYTE_SUBAGENT_TIMEOUT_MS` 覆盖。
...
- `spyderbyte web` 现在会记录失败请求和关键操作，便于诊断服务问题。
...
- 修复 preserved-thinking 历史包含空推理步骤时，SpiderByte 会话卡住的问题。
- 修复模型供应商在会话启动后才就绪时，内置工具不可用的问题。
- 修复思考强度（thinking effort）路由问题：非 SpiderByte 供应商现在保留配置值，SpiderByte 模型会校验运行时选择，并在模型解析时安全回退。
...
- 请求供应商 registry（api.json）和模型目录时携带 spiderbyte-cli 的 User-Agent，便于 registry 识别客户端版本。
...
- `spyderbyte web` 默认切换到重构后的 Agent 引擎。
...
- 新增 `subagent.timeout_ms` 配置项（或 `SPIDERBYTE_SUBAGENT_TIMEOUT_MS` 环境变量），控制单个子代理的超时时间，默认从 30 分钟提高到 2 小时。
- 新增 print 模式后台策略：设置 `[background].print_background_mode = "steer"` 后，`spyderbyte -p` 在后台任务完成后保持运行，继续引导主 Agent 进入后续轮次。

### 修复

- web: 修复断线重连后会话卡在发送状态的问题，断线期间完成的轮次现在能正常结束加载状态并发送下一条消息。
- web: 修复启动或更新 web UI 后首次访问时，初始鉴权检查失败跳转到登录页的问题；现在停留在连接界面，显示连接错误并持续重试。
- 修复 `spyderbyte -p` 在目标仍活跃或有定时任务待触发时主轮次结束即退出的问题，目标续跑与定时任务触发现在能正常执行对应轮次。
...
- `/plugins` 面板的 Official 标签页新增 SpiderByte WebBridge 入口，可在浏览器中打开 WebBridge 安装页。

### 修复

- 控制图片较多会话的请求体积：超大体量的模型读取与粘贴图片（含 WebP）会自动压缩、缩小；HEIC/HEIF 图片会给出对应平台的转换命令，而非污染会话；HTTP 413 请求过大现可自动恢复——请求和 `/compact` 会用文本标记替换旧媒体后重试。相关限制可通过 `config.toml` 的 `[image]`（或 `SPIDERBYTE_IMAGE_*` 环境变量）配置，且每个 core 独立保存设置，重新加载某客户端的配置不再影响其他客户端的图片压缩。
...
- 修复 `spyderbyte -p` 在轮次失败时仍以退出码 0 退出的问题。
...
- 修复 `spyderbyte -p` 会丢弃启动较晚或运行时间较长的后台子 Agent、导致结果无法返回主 Agent 的问题。
...
- Anthropic 供应商（Claude 与 SpiderByte 的 Anthropic 兼容模式）现在默认保留历史轮次的思考内容，与 SpiderByte 默认行为一致；可通过 `[thinking] keep = "off"` 或 `SPIDERBYTE_MODEL_THINKING_KEEP=off` 关闭。
...
- web: 在桌面通知中显示 SpiderByte 图标与更清晰的标题。
...
- SpiderByte 模型开启 Thinking 时默认跨轮次保留推理，可设置 `[thinking] keep = "off"` 关闭。

## 0.22.3（2026-07-04）

### 修复

- `spyderbyte -p` 会在后台子 Agent 完成并返回结果后再退出，避免提前结束本轮。
- web: 修复 web 聊天中已上传视频无法播放的问题。
- 回退近期 TUI 对话渲染改动，恢复上游原始行为，修复相关渲染问题。

### 优化

- `spyderbyte server run` 新增 `--dangerous-bypass-auth` 与 `--keep-alive` 选项，可在可信网络中跳过 token 校验运行服务器，并突破空闲超时保持存活。
...
- 修复 Windows 上 `spyderbyte upgrade` 在安装新版本时因 spawn 错误而失败的问题。
...
- 启动时从用户登录 shell 补充 PATH，使 shell 命令能找到用户自行安装的工具（如 Homebrew 的 `gh`），即使 spiderbyte 启动时未继承完整的 profile PATH。
...
- `spyderbyte -p` 运行时，若启用了 `background.keep_alive_on_exit`，退出前会等待后台子 Agent 完成。设置 `keep_alive_on_exit = true` 可让并发的后台子 Agent 执行完毕。
...
- 强制退出无头运行（`spyderbyte -p`），以免运行残留的引用句柄让已完成的运行一直存活到外部超时；同时为 prompt 清理加上时限，避免某个卡住的关闭步骤拖挂整个关闭流程。
...
- SpiderByte 现支持 Anthropic 兼容协议，并支持视频输入。
- web UI 新增完成提示音与问题通知，并在设置中分别提供完成通知、问题通知和提示音的开关。问题通知默认关闭，仅在用户主动开启后才会将问题文本发送到桌面。
- 新增 `SPIDERBYTE_CUSTOM_HEADERS` 环境变量，用于自定义出站 LLM 请求头，并向非 SpiderByte 供应商发送 `User-Agent` 请求头。将 `SPIDERBYTE_CUSTOM_HEADERS` 设为由换行分隔的 `Name: Value` 行。
...
- 将 Anthropic 兼容协议上的 SpiderByte 模型改走 beta Messages API。
...
- CLI 新增 `--host` 选项，可通过 `spyderbyte web --host` 将服务器暴露到互联网，并加固 token 鉴权、限流等安全措施。
- Web UI 支持渲染 LaTeX 行间公式（`$$…$$`）。

### 修复

- 修复 Linux 上由未处理的原生剪贴板错误导致的启动崩溃。
- 修复当 CLI 通过 npm/pnpm 安装或从源码运行时，`spyderbyte web` 和 `/web` 在 Windows 上因 `spawn EFTYPE` 无法启动后台服务器守护进程的问题。官方单二进制安装脚本不受影响。
...
- 将 `/plugins` 重新设计为单个标签页面板：**Installed**（管理已安装插件——切换、移除、MCP、详情、重新加载）、**Official**（SpiderByte 维护的 marketplace 插件）、**Third-party**（来自其他发布者的 marketplace 插件）以及 **Custom**（直接从 GitHub URL、zip URL 或本地路径安装）。使用 `Tab` / `Shift-Tab` 切换标签页。
...
- 新增添加额外工作区目录的能力：
  ...
  - 使用 `spyderbyte --add-dir <path>` 在启动时添加它们。
  - 项目级本地配置现在由 `.spiderbyte/local.toml` 管理；我们建议将其添加到你的 `.gitignore` 中。
...
- 修复 `spyderbyte web` 命令无法在后台启动的问题。
- 阻止后台本地服务器锁定启动时所在的目录。
- 防止点击背景时关闭 web 登录对话框。
...
- 新增 SpiderByte Web 模式，可通过 `spyderbyte web` 或 CLI 内的 `/web` 启动，在浏览器中的聊天界面继续会话。
...
- 新增内置的 `spyderbyte vis` 命令，可在浏览器中启动会话可视化工具，并指向本地会话。支持 `--port`/`--host`、`--no-open` 以及 `spyderbyte vis <sessionId>` 深度链接。
...
- 修复 SpiderByte Datasource，使其在当前 SpiderByte 环境中使用匹配的 OAuth 凭证和服务端点。
...
- 新增自定义颜色主题。在 `~/.spiderbyte/themes/` 中以 JSON 文件定义自己的调色板，或使用内置的 `/custom-theme` Skill 命令生成。
...
- 重构 TUI 中的文件引用补全。
- 当设置了 `SPIDERBYTE_HOME` 时，从该路径加载 SpiderByte 特定的用户 Skills 和全局 Agent 指令。

## 0.11.0（2026-06-05）

### 新功能

- 新增由环境变量 `SPIDERBYTE_EXPERIMENTAL_SUB_SKILL` 控制的实验性子 Skill 发现能力。随附 `sub-skill` 内置包（`sub-skill.review`、`sub-skill.consolidate`），用于盘点 Skill 并将其整理为分层分组。
- 新增以下环境变量：
  - `SPIDERBYTE_MODEL_TEMPERATURE`、`SPIDERBYTE_MODEL_TOP_P` —— 全局应用于任意 `spyderbyte` 供应商的采样参数（不绑定到 `SPIDERBYTE_MODEL_NAME`）。
  - `SPIDERBYTE_MODEL_THINKING_KEEP` —— Moonshot 的 preserved-thinking 透传（`thinking.keep`），仅在开启 Thinking 时注入。
  - `SPIDERBYTE_NO_AUTO_UPDATE`（旧别名 `SPIDERBYTE_CLI_NO_AUTO_UPDATE`）—— 完全禁用更新预检（不检查、不后台安装、不提示）。
...
- 支持 `spyderbyte acp` 子命令：spiderbyte 现在可通过 stdio 使用 [Agent Client Protocol 0.23](https://agentclientprotocol.com/)，因此 IDE（Zed、JetBrains AI Chat、自定义客户端）可以直接驱动会话；覆盖矩阵、Zed 配置和破坏性预发布说明见 [spyderbyte acp 子命令页面](https://github.com/SpiderByte/spiderbyte/tree/main/docs/zh/reference/kimi-acp.html)。
...
- 新增实验性 goal 模式，用于需要多轮处理的较长任务。在启动 SpiderByte 前设置 `SPIDERBYTE_EXPERIMENTAL_GOAL_COMMAND=1` 即可开启。
  在终端界面中使用 `/goal <objective>` 让 SpiderByte 跨轮次持续专注于同一任务。例如：
  ```text
  /goal Fix the failing checkout test
  ```
  SpiderByte 会在终端界面中显示目标，并在工作过程中保持进度可见。使用 `/goal status`、`/goal pause`、`/goal resume`、`/goal cancel` 和 `/goal replace <objective>` 来管理该目标。该功能仍处于实验阶段，欢迎试用并反馈改进建议。
- 新增 `spyderbyte provider` CLI 子命令，支持 `add`、`remove`、`list` 以及 `catalog list` / `catalog add` 操作，可在不启动终端界面的情况下导入和管理来自自定义 registry（api.json）或公开 models.dev 目录的供应商。
- 新增后台结构化提问，让 Agent 在等待用户回答时也能继续工作。
- 新增后台自动更新，可在 tui.toml 中关闭。
- 新增 `/undo` 斜杠命令，用于从对话历史中撤回上一条提示词，并在撤回时保持回放记录同步。
- 新增 `spyderbyte upgrade` 命令，用于手动检查并升级 SpiderByte CLI。
...
- 新增 `SPIDERBYTE_MODEL_ADAPTIVE_THINKING`（以及对应的 `adaptive_thinking` 模型别名字段），用于强制开启或关闭自适应 thinking（`thinking: { type: 'adaptive' }`），覆盖基于 Anthropic 模型名的版本推断。这样一来，背后由支持自适应能力的模型驱动、且使用自定义名称的兼容端点，即使模型名没有编码出可解析的 Claude 版本，也能选择启用该能力。
...
- 明确 SpiderByte Platform API 密钥登录的标签和提示细节。
- 优化终端界面中的一处细微视觉交互。
...
- 新增 `SPIDERBYTE_MODEL_*` 环境变量通道，让你无需编辑 `config.toml` 即可让 SpiderByte 使用指定模型（供应商类型、base URL、API 密钥、上下文大小、能力以及 thinking 设置）。
...
- 改进终端界面中的用量信息展示。
- 将 plugin 信任徽章限制为仅匹配 SpiderByte 托管的 plugin CDN URL 模式。
...
- 增强 `spyderbyte export`，在 manifest 中记录更多诊断信息。
...
- 在控制终端消失时及时退出。终端界面现在会处理 `SIGHUP` / `SIGTERM` 信号以及 stdout/stderr 的 `EIO` / `EPIPE` / `ENOTCONN` 错误，避免父 shell 或终端复用器异常退出后残留占用 CPU 核心的 `spyderbyte` 进程。
...
- 在首次启动迁移期间，将用户 Skill 从 `~/.spiderbyte/skills/` 迁移到 `~/.spiderbyte/skills/`；已存在的目标 Skill 会被保留。
...
- 默认让 SpiderByte 请求使用剩余的上下文窗口作为补全 token 的额度，同时将显式设置的环境变量上限作为硬上限保留。
...
- 当未配置模型时，`/model` 和欢迎面板现在会引导用户使用 `/login`（针对 SpiderByte）和 `/connect`（针对其他供应商）。
