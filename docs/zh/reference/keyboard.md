...
SpiderByte CLI 的 TUI 交互模式支持一套键盘快捷键。键位按使用场景分为五组：通用输入、模式切换、流式输出期间、工具输出控制、审批面板，以及弹窗浏览。在 TUI 中输入 `/help` 可随时打开内置快捷键清单。
...
| `Ctrl-D` | 在输入框为空时退出 SpiderByte CLI |
...
- [spyderbyte 命令](./spyderbyte-command.md) — 启动参数与子命令完整参考
