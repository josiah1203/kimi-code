...
`spyderbyte acp` 把 SpiderByte CLI 切换到 **ACP (Agent Client Protocol)** 模式：在标准输入/输出上以 JSON-RPC 形式与 ACP 客户端（如 Zed、JetBrains AI Chat 等）对话，让 IDE 直接驱动 SpiderByte 的会话、prompt 与工具调用。

```sh
spyderbyte acp
```

启动后命令不会打印任何 banner，立刻等待 ACP 客户端在 stdin 上发出 `initialize` 请求。日志会写到标准错误（以及 `~/.spiderbyte/logs/` 下的诊断日志），所以 ACP 通道本身保持干净。

::: tip 谁会调用它？
你通常不需要手动跑 `spyderbyte acp`——这个命令是给 IDE 的子进程入口准备的。IDE 端的配置见[在 IDE 中使用](../guides/ides.md)。
:::
...
| 方法 | 状态 | 说明 |
| --- | --- | --- |
| `initialize` | 是 | 版本协商；返回 `agentInfo: { name: 'SpiderByte CLI', version }`、能力矩阵、`authMethods` |
...
- `http` → SpiderByte 的 `transport: 'http'` 配置
- `stdio` → SpiderByte 的 `transport: 'stdio'` 配置
- `sse` → SpiderByte 的 `transport: 'sse'` 配置
- `acp` → 丢弃并写一条 warn 日志

## 下一步

- [在 IDE 中使用](../guides/ides.md) — Zed / JetBrains 配置步骤和故障排查
- [SpiderByte 命令参考](./spyderbyte-command.md) — 完整子命令列表
