...
Hooks（钩子）是一种自动触发机制：你预先告诉 SpiderByte CLI"每当发生 X，运行这个脚本"。脚本在你的本机执行，你可以在里面写任何逻辑。典型的使用场景：
...
所有 hook 规则写在 `~/.spiderbyte/config.toml` 的 `[[hooks]]` 数组里，每一项是一条规则：
...
阻断后，SpiderByte CLI 会把阻断原因写回上下文，模型可以据此选择更安全的替代方案。
