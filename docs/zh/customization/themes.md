...
SpiderByte CLI 可以使用内置配色，也可以使用自定义 JSON 主题文件。自定义文件放在主题目录下，会和内置选项一起出现在 `/theme` 里。
...
你不需要手写 JSON。运行内置 `/custom-theme [附加文本]` skill 命令进入自定义主题流程；这个 skill 可以帮你选颜色，把文件写到 `~/.spiderbyte/themes/`，校验十六进制色值，并告诉你如何应用。
...
- `~/.spiderbyte/themes/`
- 如果设置了 `SPIDERBYTE_HOME` 环境变量，则是 `$SPIDERBYTE_HOME/themes/`
