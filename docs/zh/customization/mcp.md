...
[Model Context Protocol（MCP）](https://modelcontextprotocol.io/) 是一个开放协议，让模型可以安全地调用外部进程或服务暴露的工具——例如读取 GitHub issues、查询数据库、操作本地文件系统。SpiderByte CLI 作为 MCP client 接入这些外部工具，并把它们与内置工具（`Read`、`Bash`、`Grep` 等）一起暴露给 Agent 使用，行为上没有差异。

## 接入方式

SpiderByte CLI 支持三种 MCP server 接入方式：
...
- **用户级**：`~/.spiderbyte/mcp.json`（或 `$SPIDERBYTE_HOME/mcp.json`），跨项目共享
- **项目级**：工作目录下的 `.spiderbyte/mcp.json`，只对当前仓库生效
