# `spyderbyte acp` Subcommand

`spyderbyte acp` switches SpiderByte CLI to **ACP (Agent Client Protocol)** mode: it communicates with an ACP client (such as Zed, JetBrains AI Chat, etc.) via JSON-RPC over stdin/stdout, letting the IDE directly drive SpiderByte's sessions, prompts, and tool calls.

```sh
spyderbyte acp
```

Once started, the command prints no banner and immediately waits for the ACP client to send an `initialize` request on stdin. Logs are written to stderr (as well as the diagnostic log under `~/.spiderbyte/logs/`), so the ACP channel itself stays clean.

::: tip Who calls this?
You typically do not need to run `spyderbyte acp` manually — this command is the subprocess entry point for IDEs. For IDE-side configuration, see [Using in IDEs](../guides/ides.md).
:::
...
| --- | --- | --- |
| `initialize` | Yes | Version negotiation; returns `agentInfo: { name: 'SpiderByte CLI', version }`, capability matrix, and `authMethods` |
...
- `http` → SpiderByte's `transport: 'http'` configuration
- `stdio` → SpiderByte's `transport: 'stdio'` configuration
- `sse` → SpiderByte's `transport: 'sse'` configuration
- `acp` → discarded with a warn log entry

## Next steps

- [Using in IDEs](../guides/ides.md) — Zed / JetBrains configuration steps and troubleshooting
- [`spyderbyte` Command Reference](./spyderbyte-command.md) — Complete subcommand list
