...
SpiderByte saves the objective, sends it as the next user message, and starts goal mode. After each turn, it checks whether the goal is complete, blocked, paused, or still active.
...
1. Use goals for work with a clear finish line and verifiable evidence.
    ...
    SpiderByte can inspect test output, change files, rerun checks, and decide when the goal is complete.
...
- **complete**: the objective is done, SpiderByte clears the goal, and the agent summarizes how it completed the work
...
- **blocked**: SpiderByte needs input, cannot complete the goal as stated, or reached a budget limit. When the agent blocks a goal, it writes a short message explaining why.
...
Upcoming goals are not visible to the agent while the current goal is running. When the current goal completes, SpiderByte starts the first upcoming goal in the same way as users enter `/goal <objective>`.
...
If the current goal is paused, canceled, or blocked, SpiderByte does not start the next upcoming goal. When a goal blocks and upcoming goals exist, the TUI reminds you that they wait for completion.
