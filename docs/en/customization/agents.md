...
SpiderByte CLI includes three built-in sub-agents, ready to use out of the box, each aimed at a different task shape:
...
SpiderByte CLI discovers agent files by scope; more specific scopes take higher priority: **Explicit (`--agent-file`) > Project > Extra > User > Plugin > Built-in**. When two files define the same `name`, the higher-priority scope wins. Each directory is scanned recursively for `.md` files.

**User level** (applies to all projects):
- `$SPIDERBYTE_HOME/agents/` (default: `~/.spiderbyte/agents/`)
- `~/.agents/agents/`

The SpiderByte-specific user agent directory moves with `SPIDERBYTE_HOME`, while the generic `~/.agents/agents/` directory stays under the real OS home so it can be shared across tools.
...
`model_preference` applies only to newly spawned subagents when the secondary-model experiment is enabled — set `SPIDERBYTE_EXPERIMENTAL_SECONDARY_MODEL=1`, or the master `SPIDERBYTE_EXPERIMENTAL_FLAG=1`. It takes effect in every launch mode, including the interactive TUI. The field never names a concrete model alias, and resumed subagents keep their existing model. The selected preference is shown to the main agent alongside the profile description so it can still pass an explicit `model` when a task needs a different choice.
...
Two CLI flags select which agent drives a new session, in both print mode (`spyderbyte -p`) and the interactive TUI:
...
```sh
spyderbyte --agent reviewer
spyderbyte -p --agent reviewer "Review the changes on this branch"
```
...
```markdown
You are SpiderByte, running at ${cwd} on ${os}.
...
Global SpiderByte-specific instructions can live at `$SPIDERBYTE_HOME/AGENTS.md` (default: `~/.spiderbyte/AGENTS.md`). When you relocate the data root with `SPIDERBYTE_HOME`, this global instruction file moves with it. Generic cross-tool instructions can still live under `~/.agents/AGENTS.md` in the real OS home, and project-level instructions remain under the project tree, for example `.spiderbyte/AGENTS.md` or `AGENTS.md`.
