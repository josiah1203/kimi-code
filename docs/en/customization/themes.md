...
SpiderByte CLI can use a built-in color scheme or a custom JSON theme file. Custom files live in the themes directory and appear in `/theme` alongside the built-in choices.
...
You do not need to write the JSON by hand. Run the built-in `/custom-theme [extra text]` skill command to enter the custom-theme workflow; the skill can choose colors, write the file under `~/.spiderbyte/themes/`, validate the hex values, and tell you how to apply it.
...
- `~/.spiderbyte/themes/`
- or `$SPIDERBYTE_HOME/themes/` when the `SPIDERBYTE_HOME` environment variable is set
