 ...
 * external provider (protocol `openai` + provider `type: kimi` — the trait-composition
...
import type { ContentPart } from '@spiderbyte/agent-core/kosong/contract/message';
...
const M_EXTERNAL = 'matrix-external';
const M_ANTHROPIC = 'matrix-anthropic';
const M_GOOGLE = 'matrix-google';

const EXTERNAL_PROVIDER = 'matrix-external-provider';
...
beforeAll(async () => {
  ...
  await klient.global.kosong.addProvider(EXTERNAL_PROVIDER, {
    type: 'kimi',
  ...
  // M_EXTERNAL needs a `provider` reference to EXTERNAL_PROVIDER so the engine
  // resolves external-provider traits (uploadVideo). The facade's addProvider()
  // doesn't support provider-linkage, so call modelService directly.
  await app!.accessor.get(IModelService).set(M_EXTERNAL, {
    model: 'external-model-matrix',
...
describe('image blocks with invalid data', () => {
  it('a data-URL image with an unaccepted mime is replaced at prompt ingestion on EVERY provider (l2)', async () => {
    ...
    const cases = [
      ...
      { label: 'bmp-external', model: M_EXTERNAL, reply: OK_OPENAI },
  ...
  it('external provider (composed): same media-strip fallback as plain openai (l3 + engine fallback)', async () => {
    const ctx = await newCase(M_EXTERNAL, 'external-image-base64');
    resetMock(queueScript(jsonError(400, 'Invalid image data'), OK_OPENAI));
...
describe('video blocks', () => {
  ...
  it('external provider (composed): ReadMediaFile on a video uploads via the files API (uploadVideo trait)', async () => {
    const ctx = await newCase(M_EXTERNAL, 'external-video-upload');
    ...
    // The external-provider file client client POSTs multipart form data to {baseUrl}/files.
...
describe('tool exchange structure', () => {
  ...
  it('media tool result: plain openai extracts text + appends a media user message; external provider keeps parts in place', async () => {
    ...
    // Composed external provider: trait mode hands shaping to the trait — the image part
    // stays inside the tool message content; no extra user message appears.
    const providerWire = await runMediaToolResultCase(M_EXTERNAL, 'media-result-external');
    const providerTool = providerWire.find((message) => message['role'] === 'tool');
    const providerContent = providerTool?.['content'] as { type: string }[];
    expect(Array.isArray(providerContent)).toBe(true);
    expect(providerContent.some((part) => part.type === 'image_url')).toBe(true);
    expect(providerWire.at(-1)?.['role']).toBe('tool');
  }, 60_000);
