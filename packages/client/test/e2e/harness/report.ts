...
export function defaultReportDir(): string {
  return resolve(process.env['SPIDERBYTE_SERVER_E2E_REPORT_DIR'] ?? join(process.cwd(), 'reports', 'latest'));
}
...
function normalizeEvent(event: ReportEvent): StoredReportEvent {
  ...
  return {
    ...
    caseName: event.caseName ?? getActiveReportCase() ?? process.env['SPIDERBYTE_SERVER_E2E_CASE_NAME'] ?? 'unassigned',
    pid: process.pid,
    ordinal: ordinal++,
  };
}
