 ...
 * `SpiderByte Agent Core/kosong/model/model.ts` (`ModelRecordSchema`, including its
 * passthrough of unknown keys) and `SpiderByte Agent Core/kosong/protocol/protocol.ts`
 * (`ProtocolSchema` — the real wire protocols; provider names are separate
 * from protocols, and Vertex AI is a `providerOptions` mode of
