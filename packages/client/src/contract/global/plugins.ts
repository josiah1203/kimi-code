 ...
 * `SpiderByte Agent Core/app/plugin/plugin.ts` and `SpiderByte Agent Core/app/plugin/types.ts`;
 * nested `McpServerConfig` mirrors `SpiderByte Agent Core/mcpCore/config-schema.ts`,
 * `HookDefConfig` mirrors `SpiderByte Agent Core/agent/externalHooks/configSection.ts`.
...
const pluginManifestKindSchema = z.enum(['spiderbyte-plugin-root', 'spiderbyte-plugin-dir']);
