...
import type { SessionMeta } from '@spiderbyte/agent-core/session/sessionMetadata/sessionMetadata';
import type { Page } from '@spiderbyte/agent-core/persistence/interface/queryStore';
import type {
  Workspace,
  WorkspaceUpdate,
...
import type { AuthStatus } from '@spiderbyte/agent-core/app/auth/auth';
...
import { platformLifecycleEventSchema } from '@spiderbyte/protocol';
import type {
  PluginCommandDef,
  PluginInfo,
  PluginSummary,
...
// ---------------------------------------------------------------------------
// Wire-type aliases for shapes the engine sources from `@spiderbyte/protocol`
...
export interface GlobalAuthFacade {
  ...
  /** The engine's local/BYOK auth-readiness probe for a model. */
...
export interface GlobalPlatformFacade {
  /** Resolve a workspace catalog id for a native SpiderByte work directory. */
