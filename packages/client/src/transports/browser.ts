 ...
 *
 * This is intentionally a small HTTP/WS projection over the canonical SpiderByte
...
export interface BrowserPlatformClientOptions {
  /** Origin or API prefix, for example `https://spiderbyte.example.test`. */
...
export class BrowserPlatformClient {
  ...
  async webSocketProtocols(): Promise<readonly string[] | undefined> {
    ...
    return [`spiderbyte.bearer.${token}`];
  }
}
