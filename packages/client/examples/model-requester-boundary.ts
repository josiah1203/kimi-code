 ...
 *
 * Part 1 — real config: bootstraps the SpiderByte Agent Core App scope on the
 * local SpiderByte home (`SPIDERBYTE_HOME` or `~/.spiderbyte`), resolves `IModelCatalog`
 ...
 *   pnpm -C packages/client smoke:boundary
 *
 * Env:
 *   SPIDERBYTE_HOME        — default `~/.spiderbyte`
 *   SPIDERBYTE_BOUNDARY_MODELS  — comma-separated model ids to ping (default: all)
 *   SPIDERBYTE_BOUNDARY_SKIP_LIVE — set to `1` to skip part 1 (no real API calls)
 */
...
import { isError2 } from '@spiderbyte/agent-core/_base/errors/errors';
...
} from '@spiderbyte/agent-core/kosong/contract/errors';
import type { ToolCall } from '@spiderbyte/agent-core/kosong/contract/message';
...
async function probeRealConfig(): Promise<void> {
  ...
  try {
    ...
    const filter = process.env['SPIDERBYTE_BOUNDARY_MODELS']?.split(',').map((s) => s.trim());
...
async function main(): Promise<void> {
  if (process.env['SPIDERBYTE_BOUNDARY_SKIP_LIVE'] !== '1') {
