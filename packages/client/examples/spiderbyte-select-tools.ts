 ...
 * external-provider types, based on the real providers/auth in
 * `~/.spiderbyte/config.toml`.
 *
 ...
 *   1. spyderbyte providerType: the tool-declaration message reaches the wire with
 *      an embedded `tools` array.
 *   2. no providerType (plain openai): the same message is dropped — the
 *      boundary non-spyderbyte providers cannot cross.
 ...
 *
 * Part B — live, per provider adapter model from the real config: a two-step flow that
 ...
 *
 * Run:
 *   pnpm -C packages/client exec tsx --tsconfig ./tsconfig.examples.json \
 *     --import ../../build/register-raw-text-loader.mjs examples/spiderbyte-select-tools.ts
 *
 * Env:
 *   SPIDERBYTE_HOME              — default `~/.spiderbyte`
 *   SPIDERBYTE_SELECT_TOOLS_MODELS    — comma-separated model ids for the live parts (default: all provider adapter-type)
 *   SPIDERBYTE_SELECT_TOOLS_SKIP_LIVE — set to `1` to skip part B (no real API calls)
 *   SPIDERBYTE_SELECT_TOOLS_TAP       — set to `1` to run part C instead of B: route
...
import { renderLoadableToolsAnnouncement } from '@spiderbyte/agent-core/agent/toolSelect/dynamicTools';
import { UNKNOWN_CAPABILITY } from '@spiderbyte/agent-core/kosong/contract/capability';
import type { Message } from '@spiderbyte/agent-core/kosong/contract/message';
import type { Tool } from '@spiderbyte/agent-core/kosong/contract/tool';
import type { AuthProvider, Model } from '@spiderbyte/agent-core/kosong/model/catalog';
import { IModelCatalog } from '@spiderbyte/agent-core/kosong/model/catalog';
import type {
  ModelRequestInput,
  ModelRequester,
} from '@spiderbyte/agent-core/kosong/model/modelRequester';
import { ModelRequesterImpl } from '@spiderbyte/agent-core/kosong/model/modelRequesterImpl';
import { IProtocolAdapterRegistry } from '@spiderbyte/agent-core/kosong/protocol/protocol';
import { ProtocolAdapterRegistry } from '@spiderbyte/agent-core/kosong/provider/protocolAdapterRegistry';
...
// ---------------------------------------------------------------------------
// The simulated "provider adapter computer use" MCP tool pool.
// ---------------------------------------------------------------------------
...
async function probeWireEncoding(): Promise<void> {
  ...
  try {
    // 1) spyderbyte providerType: the declaration message reaches the wire, with the
    ...
    const providerBody = lastBody;
    const declaration = providerBody?.messages?.find((m) => Array.isArray(m.tools));
    assert(declaration !== undefined, 'provider adapter: a wire message carries the loaded tool schemas');
    assert(
      declaration.tools?.some((t) => t.function?.name === 'computer_screenshot') === true,
      'provider adapter: declaration embeds computer_screenshot',
    );
    assert(
      providerBody?.tools?.some((t) => t.function?.name === 'select_tools') === true,
      'provider adapter: top-level tools[] keeps select_tools',
    );
    assert(
      providerBody?.tools?.some((t) => t.function?.name === 'computer_screenshot') !== true,
      'provider adapter: deferred tool stays OUT of top-level tools[]',
    );
    console.log(
      `[ok] provider adapter: declaration on wire (role=${String(declaration.role)}), ` +
        `top-level tools=[${(providerBody?.tools ?? []).map((t) => t.function?.name ?? '?').join(', ')}]`,
    );

    // 2) plain openai: the same declaration message is dropped outright — the
    // dynamic-tool schema can never reach a non-provider adapter wire. (3 wire messages =
    ...
    console.log(
      '[ok] openai: declaration message dropped — dynamic tools cannot cross a non-provider adapter wire',
    );
  } finally {
...
// ---------------------------------------------------------------------------
// Part B — live two-step select_tools flow per external-provider model.
// ---------------------------------------------------------------------------
...
async function probeLiveExternalProviders(): Promise<void> {
  const homeDir = process.env['SPIDERBYTE_HOME'] ?? join(homedir(), '.spiderbyte');
  console.log(`\n=== part B: live select_tools flow on real spyderbyte providers (${homeDir}) ===`);
  ...
  try {
    ...
    const filter = process.env['SPIDERBYTE_SELECT_TOOLS_MODELS']?.split(',').map((s) => s.trim());
    ...
    assert(targets.length > 0, 'at least one provider adapter-type model configured');
...
async function main(): Promise<void> {
  ...
  if (process.env['SPIDERBYTE_SELECT_TOOLS_TAP'] === '1') {
    await probeTappedContext();
  } else if (process.env['SPIDERBYTE_SELECT_TOOLS_SKIP_LIVE'] !== '1') {
    await probeLiveExternalProviders();
  }
  console.log('\nselect-tools: OK');
}
