 ...
 *   SPIDERBYTE_EXAMPLE_MODEL=... SPIDERBYTE_EXAMPLE_API_KEY=... \
 ...
 *
 * Env:
 *   SPIDERBYTE_EXAMPLE_MODEL      — gateway model id to seed (required)
 *   SPIDERBYTE_EXAMPLE_API_KEY    — API key for the seeded model (required)
 *   SPIDERBYTE_EXAMPLE_BASE_URL   — optional gateway base URL for the seeded model
 *   SPIDERBYTE_EXAMPLE_PROTOCOL   — optional wire protocol for the seeded model (default `openai`)
 */
...
async function main(): Promise<void> {
  const seedModel = process.env['SPIDERBYTE_EXAMPLE_MODEL'];
  const seedKey = process.env['SPIDERBYTE_EXAMPLE_API_KEY'];
  if (seedModel === undefined || seedKey === undefined) {
    throw new Error('SPIDERBYTE_EXAMPLE_MODEL and SPIDERBYTE_EXAMPLE_API_KEY are required (see header)');
  }
  ...
  try {
    ...
    await klient.global.kosong.addProvider({
      id: SEEDED_MODEL_ID,
      model: seedModel,
      protocol: (process.env['SPIDERBYTE_EXAMPLE_PROTOCOL'] ?? 'openai'),
      baseUrl: process.env['SPIDERBYTE_EXAMPLE_BASE_URL'] ?? 'http://127.0.0.1:1',
