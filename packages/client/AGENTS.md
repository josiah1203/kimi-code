...
- **Contract** (`src/contract/`) — zod input/output schemas for every wire
  ...
  SpiderByte Agent Core types and pinned by the compile-time parity assertions in
...
- `test/e2e/legacy/` + `test/e2e/harness/` — the legacy `/api/v1` live
  suites (moved from server-e2e). They skip unless `SPIDERBYTE_SERVER_URL` points
...
- `pnpm --filter @spiderbyte/client test` — all Vitest suites (unit +
  conformance + e2e; live cases skip without their env).
- `SPIDERBYTE_SERVER_URL=http://127.0.0.1:58627 pnpm --filter @spiderbyte/client test`
  — include the live legacy cases against a running server.
- `pnpm --filter @spiderbyte/client docker:e2e` — docker e2e; the run
  derives its runner name/namespace from the current workspace to avoid
  cross-workspace conflicts.
- `pnpm --filter @spiderbyte/client typecheck` / `pnpm smoke` (in-process
  smoke over the memory transport; see `examples/smoke.ts`).
- `pnpm --filter @spiderbyte/client smoke:boundary` — ModelRequester boundary
  probe: pings every model configured in the real `~/.spiderbyte/config.toml`
...
- `pnpm --filter @spiderbyte/client smoke:select-tools` — select_tools
  (progressive tool disclosure) probe for external-provider types: stub-verifies
  the provider adapter-only wire encoding of dynamic tool declarations, then runs a live
  two-step select→use flow per external-provider model (see
  `examples/spiderbyte-select-tools.ts`).
