...
import { createKlient } from '@spiderbyte/client/memory';   // or '/ipc'
...
|---|---|---|
| `@spiderbyte/client/ipc` | `{ socketPath, token? }` | same socket |
| `@spiderbyte/client/memory` | `{ scope }` (a bootstrapped engine app scope) | direct emitter/bus subscription |
...
- `test/e2e/legacy/` + `test/e2e/harness/` — the legacy `/api/v1` live suites
  and their client harness (skip unless `SPIDERBYTE_SERVER_URL` is set; the v1
...
readings through a real prompt (requires `SPIDERBYTE_EXAMPLE_MODEL` +
`SPIDERBYTE_EXAMPLE_API_KEY`).
