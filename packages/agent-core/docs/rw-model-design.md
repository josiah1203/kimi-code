...
> 目标：为 SpiderByte Agent Core 定义一套**唯一**的读写模型，统一 view、topic、写 operation、
> 订阅方式，消解回环、定义方式不一致、事件可见性混乱等问题。本文基于对
> SpiderByte Agent Core / server-v2 / TUI（apps/cli）三方现状的完整调研，
> 所有断言均有 file:line 证据。
>
...
> SpiderByte Agent Core 的 wire record 类型。后续的重构（v1 vocabulary 对齐）已删除这三个
