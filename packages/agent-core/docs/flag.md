...
> Experimental feature-flag gating for SpiderByte Agent Core — a App-scope `IFlagService` resolver plus a writable `IFlagRegistry` catalog that domains contribute their flags to, backed by the `[experimental]` config section.
...
- `src/flag/flagService.ts` — `FlagService` impl + `MASTER_ENV` (`SPIDERBYTE_EXPERIMENTAL_FLAG`) + `EXPERIMENTAL_SECTION` (`experimental`); reads definitions from `IFlagRegistry`; self-registers at App scope.
...
1. L1 master env `SPIDERBYTE_EXPERIMENTAL_FLAG` truthy → every flag on.
2. L2 per-feature `def.env` (e.g. `SPIDERBYTE_EXPERIMENTAL_MY_FEATURE`) → forces on/off.
...
export const myFeatureFlag: FlagDefinitionInput = {
  ...
  title: 'My feature',
  description: '...',
  env: 'SPIDERBYTE_EXPERIMENTAL_MY_FEATURE',
  default: false,
  surface: 'both',
};
...
- `env` must start with `SPIDERBYTE_EXPERIMENTAL_`, be unique, and not equal `SPIDERBYTE_EXPERIMENTAL_FLAG`.
...
- `packages/agent-core/src/flag/` — implementation (`IFlagRegistry` + `IFlagService`).
- `packages/agent-core/src/agent/toolSelect/flag.ts` — example per-domain flag contribution.
- `packages/agent-core/test/flag/flag.test.ts` — precedence + config subscription tests.
- `packages/agent-core/src/flags/` — v1 source this was ported from.
- `packages/agent-core/GAP_ANALYSIS.md` §2.1 — gap closure note.
