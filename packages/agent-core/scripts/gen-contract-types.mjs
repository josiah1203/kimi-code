/**
 * Generates a black-box "contract" declaration tree for SpiderByte Agent Core.
 *
 ...
 * DI primitives. Consumers (spiderbyte-mini-bench) type-check against this tree
...
const PKG = join(__dirname, '..'); // packages/agent-core
const SRC = join(PKG, 'src');
const TMP = join(PKG, '.contract-types-tmp');
const TSCONFIG = join(PKG, 'tsconfig.contract.json');

const repoRoot = join(PKG, '..', '..');
const defaultOut = join(repoRoot, '..', 'spiderbyte-mini-bench', 'types', 'SpiderByte Agent Core');
