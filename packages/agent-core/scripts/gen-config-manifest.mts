 ...
 *
 * Usage:
 *   pnpm --filter @spiderbyte/agent-core gen:config-manifest          # write the file
 *   pnpm --filter @spiderbyte/agent-core gen:config-manifest --check  # freshness check (CI-style)
 *
...
export async function buildConfigManifest(): Promise<string> {
  ...
  const out: string[] = [
    ...
    '# Regenerate with: pnpm --filter @spiderbyte/agent-core gen:config-manifest',
    '#',
