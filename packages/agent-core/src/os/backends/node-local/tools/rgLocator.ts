 ...
 * pinned ripgrep archive into `<SPIDERBYTE_HOME|~/.spiderbyte>/bin` when the
...
async function downloadAndInstallRg(shareDir: string): Promise<string> {
  ...
  const url = `${RG_BASE_URL}/${RG_VERSION}/${archiveName}`;

  const binDir = join(shareDir, 'bin');
  await mkdir(binDir, { recursive: true });
  const destination = join(binDir, rgBinaryName());

  const tmp = await mkdtemp(join(tmpdir(), 'spiderbyte-rg-'));
  try {
