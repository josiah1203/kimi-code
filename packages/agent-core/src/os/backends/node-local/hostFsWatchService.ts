...
import { Emitter, Event, type Event as EventType } from '#/_base/event';
...
export class HostFsWatchService implements IHostFsWatchService {
  ...
  watch(path: string, options?: HostFsWatchOptions): IHostFsWatchHandle {
    if (process.env['SPIDERBYTE_DISABLE_FS_WATCH'] === '1') {
      return new DisabledHostFsWatchHandle();
    }
...
/** Explicit test/CI knob; local production runs retain real filesystem watches. */
class DisabledHostFsWatchHandle implements IHostFsWatchHandle {
  readonly ready = Promise.resolve();
  readonly onDidChange: EventType<HostFsChange> = Event.None as EventType<HostFsChange>;
