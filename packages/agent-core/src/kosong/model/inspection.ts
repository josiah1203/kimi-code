...
import { parseSpiderByteCustomHeaders } from '@spiderbyte/oauth';
...
function attributeHeaders(
  ...
  const envLayer = parseSpiderByteCustomHeaders();
  ...
  for (const key of Object.keys(model.headers)) {
    ...
    if (key in customLayer) {
      ...
      sources.set(path, { kind: 'env', detail: 'SPIDERBYTE_CUSTOM_HEADERS' });
    }
  }
}
