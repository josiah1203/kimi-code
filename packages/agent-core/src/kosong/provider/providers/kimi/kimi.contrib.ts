 ...
 *     - request params: the `SPIDERBYTE_API_KEY` / `SPIDERBYTE_BASE_URL` endpoint
...
export const SPIDERBYTE_API_KEY_ENV = 'SPIDERBYTE_API_KEY';
export const SPIDERBYTE_BASE_URL_ENV = 'SPIDERBYTE_BASE_URL';
...
function resolveFiles(ctx: TraitContext): KimiFiles {
  ...
  if (files === undefined) {
    files = new KimiFiles({
      apiKey: ctx.config.apiKey ?? firstEnv(SPIDERBYTE_API_KEY_ENV),
      baseUrl: ctx.config.baseUrl ?? firstEnv(SPIDERBYTE_BASE_URL_ENV) ?? KIMI_DEFAULT_BASE_URL,
      defaultHeaders:
...
export const kimiOpenAITrait: ProtocolTrait = {
  ...
  endpoint: () => ({
    apiKeyEnv: SPIDERBYTE_API_KEY_ENV,
    baseUrlEnv: SPIDERBYTE_BASE_URL_ENV,
