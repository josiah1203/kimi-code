 ...
 *
 * Builds the `LoggingConfig` from `SPIDERBYTE_LOG_*` environment variables plus
...
export function resolveSessionLogPath(sessionDir: string): string {
  return join(sessionDir, 'logs', 'spiderbyte.log');
}

export function resolveLoggingConfig(input: ResolveLoggingInput): LoggingConfig {
  const env = input.env;
  return {
    level: parseLevel(env['SPIDERBYTE_LOG_LEVEL']) ?? DEFAULT_LOG_LEVEL,
    globalLogPath: resolveGlobalLogPath(input.homeDir),
    globalMaxBytes: parsePositiveInt(env['SPIDERBYTE_LOG_GLOBAL_MAX_BYTES']) ?? DEFAULT_GLOBAL_MAX_BYTES,
    globalFiles: parsePositiveInt(env['SPIDERBYTE_LOG_GLOBAL_FILES']) ?? DEFAULT_GLOBAL_FILES,
    sessionMaxBytes:
      parsePositiveInt(env['SPIDERBYTE_LOG_SESSION_MAX_BYTES']) ?? DEFAULT_SESSION_MAX_BYTES,
    sessionFiles: parsePositiveInt(env['SPIDERBYTE_LOG_SESSION_FILES']) ?? DEFAULT_SESSION_FILES,
  };
}
