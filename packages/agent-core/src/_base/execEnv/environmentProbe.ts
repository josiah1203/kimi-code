 ...
 * in the message. Set `SPIDERBYTE_SHELL_PATH` to override.
 *
...
async function locateWindowsGitBash(deps: HostEnvironmentProbeDeps): Promise<string> {
  ...
  const override = deps.env['SPIDERBYTE_SHELL_PATH']?.trim();
  ...
  throw new Error(
    `Git Bash was not found on this Windows host. Install Git for Windows from https://gitforwindows.org/ or set SPIDERBYTE_SHELL_PATH to a bash.exe. Checked: ${checked.join(', ')}.`,
  );
}
