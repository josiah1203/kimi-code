 ...
 * SpiderByte home, project-root `.mcp.json` — the root discovered through the
 * `git` domain's work-tree probe — and `.spiderbyte/mcp.json` under the cwd)
...
import { resolveSpiderByteHome } from '#/app/bootstrap/bootstrap';
...
export async function resolveMcpJsonPaths(input: ResolveMcpJsonPathsInput): Promise<McpJsonPaths> {
  ...
  return {
    user: join(resolveSpiderByteHome(input.homeDir), 'mcp.json'),
    projectRoot: join(projectRoot, '.mcp.json'),
    project: join(input.cwd, '.spiderbyte', 'mcp.json'),
  };
}
