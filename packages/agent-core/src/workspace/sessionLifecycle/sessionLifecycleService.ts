...
export class SessionLifecycleService extends Disposable implements ISessionLifecycleService {
  ...
  async fork(opts: ForkSessionOptions): Promise<ISessionScopeHandle> {
    ...
    try {
      ...
      for (const agentId of orderedAgentIds) {
        const copied = await this.copyAgentWire({
          sourceHandle,
          sourceSessionId: sourceId,
          agentId,
          targetSessionId: targetCtx.sessionId,
          turnIndex: agentId === MAIN_AGENT_ID ? opts.turnIndex : undefined,
          cutoffTime: mainFork.cutoffTime,
        });
        if (agentId === MAIN_AGENT_ID) mainFork = copied;
        if (copied.retained) retainedAgentIds.add(agentId);
        else if (agentId !== MAIN_AGENT_ID) {
          await this.hostFs.remove(join(targetCtx.sessionDir, 'agents', agentId)).catch(() => {});
        }
      }

      if (!mainFork.retained) {
        throw new Error2(
          ErrorCodes.REQUEST_INVALID,
          `Session "${sourceId}" has no main agent journal`,
        );
      }
      ...
      await targetMeta.update({
        ...
        forkedFrom: sourceId,
        archived: false,
        lastPrompt: mainFork.lastPrompt ?? sourceMeta?.lastPrompt,
      ...
      if (opts.turnIndex === undefined) await this.duplicateCronTasks(sourceId, targetId);

      for (const agentId of agentIds) {
        if (!retainedAgentIds.has(agentId)) continue;
  ...
  private async copyAgentWire(args: {
    ...
    const sourceRecords = await collect(
      this.appendLogStore.read<WireRecord>(
        agentScopeOf(sessionScopeOf(this.handlerScope, args.sourceSessionId), args.agentId),
        AGENT_WIRE_RECORD_KEY,
      ),
    );
    let records: WireRecord[] = [...sourceRecords];
    let lastPrompt: string | undefined;
    let cutoffTime = args.cutoffTime;
    if (args.turnIndex !== undefined) {
      const sliced = sliceWireRecordsAtTurn(sourceRecords, args.sourceSessionId, args.turnIndex);
      records = [...sliced.records];
      lastPrompt = sliced.lastPrompt;
      cutoffTime = sliced.cutoffTime;
    } else if (args.cutoffTime !== undefined) {
      records = sourceRecords.filter(
        (record) => record.type === 'metadata' || record.time === undefined || record.time <= args.cutoffTime!,
      );
      const retainedData = records.some((record) => record.type !== 'metadata');
      if (!retainedData) return { retained: false, cutoffTime };
    }
    if (records.length === 0) {
    ...
    return { retained: true, cutoffTime, lastPrompt };
  }
...
interface CopiedAgentWire {
  readonly retained: boolean;
  readonly cutoffTime?: number;
  readonly lastPrompt?: string;
}

interface WireTurnSlice {
...
function assertForkTurnIndex(turnIndex: number | undefined): void {
  ...
  throw new Error2(
    ErrorCodes.REQUEST_INVALID,
    'fork turnIndex must be a non-negative safe integer',
    { details: { turnIndex } },
  );
}

function sliceWireRecordsAtTurn(
  records: readonly WireRecord[],
  sourceSessionId: string,
  turnIndex: number,
): WireTurnSlice {
  const turnStarts = records.flatMap((record, index) =>
    record.type === 'turn.prompt' && isUserVisiblePromptOrigin(record['origin']) ? [index] : [],
  );
  ...
  const retained = records.slice(0, end).filter((record) => record.type !== 'forked');
  const times = retained
    .map((record) => record.time)
    .filter((time): time is number => typeof time === 'number');
  const promptRecord = records[start];
  return {
    records: retained,
    cutoffTime: times.length === 0 ? undefined : Math.max(...times),
    lastPrompt: promptText(promptRecord?.['input']),
  };
}

function isUserVisiblePromptOrigin(origin: unknown): boolean {
  if (origin === null || typeof origin !== 'object' || Array.isArray(origin)) return false;
  const value = origin as { readonly kind?: unknown; readonly trigger?: unknown; readonly phase?: unknown };
  if (value.kind === 'user') return true;
  if (value.kind === 'skill_activation' || value.kind === 'plugin_command') {
    return value.trigger === 'user-slash';
  }
  return value.kind === 'shell_command' && value.phase === 'input';
}
