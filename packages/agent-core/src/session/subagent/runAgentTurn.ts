...
import { Error2, ErrorCodes, toSpiderByteErrorPayload, type SpiderByteErrorPayload } from '#/errors';
...
function classifyTurnResult(result: TurnResult): void {
  switch (result.type) {
    ...
    case 'failed': {
      ...
      const payload = toSpiderByteErrorPayload(error);
...
function providerRateLimitErrorFromPayload(error: SpiderByteErrorPayload): APIProviderRateLimitError {
  const requestId =
