...
const SESSION_LOG_REL = 'logs/spiderbyte.log';
const GLOBAL_LOG_REL = 'logs/global/spiderbyte.log';
const WEB_LOG_REL = 'logs/spiderbyte-web.jsonl';
const DESKTOP_LOG_REL = 'logs/spiderbyte-desktop.log';
    ...
    return exportSessionDirectory({
      ...
      desktopLogPath:
        input.includeDesktopLog === true
          ? join(this.bootstrap.homeDir, 'logs', 'spiderbyte-desktop.log')
          : undefined,
...
function defaultExportZipName(sessionId: string, now: Date): string {
  ...
  return `spiderbyte-debug-${shortId}-${timestamp}.zip`;
}
