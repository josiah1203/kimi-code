 ...
 *  - The env-synthesized `__spiderbyte_env__` slice is never written to config:
...
interface StaticExclusion {
  ...
  readonly thinking?: SpiderByteConfigShape['thinking'];
}
...
export class ProviderDiscoveryService implements IProviderDiscoveryService {
  ...
  private computeStaticExclusion(): StaticExclusion {
    ...
    const thinking = this.config.inspect<SpiderByteConfigShape['thinking']>(
      THINKING_SECTION,
    ).userValue;
    return {
  ...
  private readUserConfigShape(exclusion: StaticExclusion = EMPTY_EXCLUSION): SpiderByteConfigShape {
    const providers =
      this.config.inspect<Record<string, ProviderConfig>>(PROVIDERS_SECTION).userValue ?? {};
    const models =
      this.config.inspect<Record<string, ModelRecord>>(MODELS_SECTION).userValue ?? {};
    const defaultModel = this.config.inspect<string>(DEFAULT_MODEL_SECTION).userValue;
    const thinking =
      this.config.inspect<SpiderByteConfigShape['thinking']>(THINKING_SECTION).userValue;
    return {
      providers: withoutKeys(providers, exclusion.providers) as SpiderByteConfigShape['providers'],
      models: withoutKeys(models, exclusion.models) as SpiderByteConfigShape['models'],
      defaultModel,
      thinking: thinking === undefined ? undefined : { ...thinking },
    };
  }

  private shapeWithoutProvider(providerId: string): Promise<SpiderByteConfigShape> {
  ...
  private async applyRefreshPatch(
    patch: SpiderByteConfigShape,
    exclusion: StaticExclusion,
  ): Promise<SpiderByteConfigShape> {
    const userProviders =
      this.config.inspect<Record<string, ProviderConfig>>(PROVIDERS_SECTION).userValue ?? {};
    const userModels =
      this.config.inspect<Record<string, ModelRecord>>(MODELS_SECTION).userValue ?? {};
    ...
    return {
      providers:
        patch.providers !== undefined
          ? ({ ...exclusion.providers, ...patch.providers } as SpiderByteConfigShape['providers'])
          : (userProviders as SpiderByteConfigShape['providers']),
      models:
        patch.models !== undefined
          ? ({ ...exclusion.models, ...patch.models } as SpiderByteConfigShape['models'])
          : (userModels as SpiderByteConfigShape['models']),
      defaultModel:
