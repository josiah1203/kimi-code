/**
 * `kosongConfig` domain — `SPIDERBYTE_MODEL_*` effective-config overlay.
 *
 * When `SPIDERBYTE_MODEL_NAME` is set, synthesizes one model id (bound to the
 * reserved `__spiderbyte_env__` provider whose schema kosong owns) from the
 * `SPIDERBYTE_MODEL_*` environment variables and overlays it onto the resolved
 ...
 * provider-definition registry, not from a hardcoded vendor table. A model
 * supplied through environment variables therefore requires an explicit
 * provider type and endpoint when the selected adapter needs them.
 */
...
export const ENV_MODEL_ALIAS_KEY = '__spiderbyte_env_model__';
...
export const spiderbyteModelEnvOverlay: ConfigEffectiveOverlay = {
  apply(effective, getEnv, validate) {
    const model = trimmed(getEnv('SPIDERBYTE_MODEL_NAME'));
    const temperature = parseFloatEnv(
      getEnv('SPIDERBYTE_MODEL_TEMPERATURE'),
      'SPIDERBYTE_MODEL_TEMPERATURE',
    );
    const topP = parseFloatEnv(getEnv('SPIDERBYTE_MODEL_TOP_P'), 'SPIDERBYTE_MODEL_TOP_P');
    const thinkingKeep = trimmed(getEnv('SPIDERBYTE_MODEL_THINKING_KEEP'));
    const maxCompletionTokens =
      parseCompletionTokens(getEnv('SPIDERBYTE_MODEL_MAX_COMPLETION_TOKENS')) ??
      parseCompletionTokens(getEnv('SPIDERBYTE_MODEL_MAX_TOKENS'));
    ...
    const maxContextRaw = trimmed(getEnv('SPIDERBYTE_MODEL_MAX_CONTEXT_SIZE'));
    const maxContextSize =
      maxContextRaw === undefined
        ? DEFAULT_MAX_CONTEXT_SIZE
        : parsePositiveInt(maxContextRaw, 'SPIDERBYTE_MODEL_MAX_CONTEXT_SIZE');

    const maxOutputRaw = trimmed(getEnv('SPIDERBYTE_MODEL_MAX_OUTPUT_SIZE'));
    const maxOutputSize =
      maxOutputRaw === undefined
        ? undefined
        : parsePositiveInt(maxOutputRaw, 'SPIDERBYTE_MODEL_MAX_OUTPUT_SIZE');
    const capabilities = parseCapabilities(getEnv('SPIDERBYTE_MODEL_CAPABILITIES')) ?? DEFAULT_CAPABILITIES;
    const displayName = trimmed(getEnv('SPIDERBYTE_MODEL_DISPLAY_NAME'));
    const reasoningKey = trimmed(getEnv('SPIDERBYTE_MODEL_REASONING_KEY'));
    const adaptiveThinking = parseBooleanVar(
      getEnv('SPIDERBYTE_MODEL_ADAPTIVE_THINKING'),
      'SPIDERBYTE_MODEL_ADAPTIVE_THINKING',
    );
    ...
    const providerType =
      typeof envProvider['type'] === 'string' ? envProvider['type'] : 'openai-compatible';
    ...
    if (envProvider['type'] === undefined) providerPatch['type'] = 'openai-compatible';
...
registerConfigOverlay(spiderbyteModelEnvOverlay);
