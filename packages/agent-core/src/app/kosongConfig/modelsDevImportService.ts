...
export class ModelsDevImportService implements IModelsDevImportService {
  ...
  private async doImportModelsDevProvider(
    ...
    if (existing?.oauth !== undefined) {
      throw new Error2(
        codes.PROVIDER_OAUTH_UNSUPPORTED,
        `provider ${targetId} uses an OAuth record that is not supported in Open Core`,
      );
    }
  ...
  private async doImportCustomRegistry(
    ...
    for (const entry of Object.values(entries)) {
      if (providers[entry.id]?.oauth !== undefined) {
        throw new Error2(
          codes.PROVIDER_OAUTH_UNSUPPORTED,
          `provider ${entry.id} uses an OAuth record that is not supported in Open Core`,
        );
      }
    }
