...
export const MASTER_ENV = 'SPIDERBYTE_EXPERIMENTAL_FLAG';
...
export class FlagService extends Disposable implements IFlagService {
  ...
  explain(id: FlagId): ExperimentalFeatureState | undefined {
    ...
    if (
      def.emergencyDisableEnv !== undefined &&
      parseBooleanEnv(this.bootstrap.getEnv(def.emergencyDisableEnv)) === true
    ) {
      return this.state(def, false, 'emergency-disable-env', configValue);
    }
  ...
  private state(
    ...
    return {
      ...
      surface: def.surface,
      env: def.env,
      emergencyDisableEnv: def.emergencyDisableEnv,
