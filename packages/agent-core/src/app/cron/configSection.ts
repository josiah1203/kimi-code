 ...
 *
 * Declares the `SPIDERBYTE_CRON_*` environment bindings for the cron operational
...
export const cronEnvBindings: EnvBindings<CronConfig> = envBindings(cronConfigSchema, {
  debug: { env: 'SPIDERBYTE_CRON_DEBUG', parse: on },
  noJitter: { env: 'SPIDERBYTE_CRON_NO_JITTER', parse: on },
  noStale: { env: 'SPIDERBYTE_CRON_NO_STALE', parse: on },
  disabled: { env: 'SPIDERBYTE_DISABLE_CRON', parse: on },
  manualTick: { env: 'SPIDERBYTE_CRON_MANUAL_TICK', parse: on },
  clock: 'SPIDERBYTE_CRON_CLOCK',
  pollIntervalMs: { env: 'SPIDERBYTE_CRON_POLL_INTERVAL_MS', parse: parsePollIntervalMs },
});
