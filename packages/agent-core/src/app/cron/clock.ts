 ...
 *
 * `wallNow` resolution is driven by the `SPIDERBYTE_CRON_CLOCK` env var; see
...
function debugInvalidSpec(spec: string, reason: string, debug: boolean): void {
  if (debug) {
    process.stderr.write(
      `[cron/clock] invalid SPIDERBYTE_CRON_CLOCK spec ${JSON.stringify(spec)}: ${reason} — falling back to system clock\n`,
    );
  }
}
