---
name: custom-theme
description: Create or edit a spiderbyte custom color theme — a JSON file under the resolved SPIDERBYTE_HOME data directory that recolors the TUI. Use when the user wants their own theme, asks for a specific palette or mood, or wants to tweak an existing custom theme's colors.
---

# Create a spiderbyte custom theme (custom-theme)

Help the user design, write, and apply a custom color theme for the spiderbyte TUI. A theme is a single JSON file; the TUI ships with `dark`, `light`, and `auto`, and any file the user adds becomes selectable alongside them.
...
- **Never assume the data directory is `~/.spiderbyte`.** Always resolve `$SPIDERBYTE_HOME` first with the Bash command below.
...
The spiderbyte runtime resolves the data directory as `SPIDERBYTE_HOME` first, falling back to `~/.spiderbyte`. Theme files live inside the `themes/` subdirectory of that data directory.

Before doing anything, resolve the actual data root with Bash so you don't write to the wrong place. Check whether `SPIDERBYTE_HOME` is set and fall back to `~/.spiderbyte` when it is empty:
...
Use the first line when it is non-empty; otherwise use the second line. In the rest of this skill, `<SPIDERBYTE_HOME>` means that resolved data root — **never assume `~/.spiderbyte`**. Theme files live at `<SPIDERBYTE_HOME>/themes/<name>.json`. Create the `themes/` directory if it doesn't exist.

## What a theme is

- A theme lives at `<SPIDERBYTE_HOME>/themes/<name>.json`.
...
```
https://github.com/SpiderByte/spiderbyte/tree/main/docs/en/customization/themes.html
```
...
2. **Resolve the actual theme directory and current theme(s).**
   - Resolve the data root by checking `echo "$SPIDERBYTE_HOME"`; if empty, use `echo "$HOME/.spiderbyte"`. Use `<root>/themes` for every subsequent step.
   - If tweaking an existing custom theme, **Read** `<SPIDERBYTE_HOME>/themes/<name>.json` first — never overwrite a theme you haven't read.
   - Starting fresh: build a `colors` object from the token table. You can `ls <SPIDERBYTE_HOME>/themes/` and Read one of the user's existing themes as a reference for the format.
...
4. **Create a candidate file; never edit the live theme in place.**
   - Use Bash to create a candidate. If the target theme already exists, copy it verbatim: `cp <name>.json <name>.json.new` (inside `<SPIDERBYTE_HOME>/themes/`). If it doesn't exist, use **Write** to create a minimal skeleton named `<name>.json.new`.
