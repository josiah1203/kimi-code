...
The spiderbyte runtime resolves the user-global directory as `SPIDERBYTE_HOME`
first, falling back to `~/.spiderbyte`. Before touching the user-global file,
resolve the actual directory with Bash so you don't read or write the wrong
one. Check whether `SPIDERBYTE_HOME` is set and fall back to `~/.spiderbyte`
when it is empty:
...
rest of this skill, `<SPIDERBYTE_HOME>` means that resolved data root —
**never assume `~/.spiderbyte`**.

- User-global: `<SPIDERBYTE_HOME>/mcp.json`. Use for servers you want
  everywhere.
...
- Project-local: `<cwd>/.spiderbyte/mcp.json`. Use for SpiderByte-specific
...
or the `SPIDERBYTE_MCP_STARTUP_TIMEOUT_MS` / `SPIDERBYTE_MCP_TOOL_TIMEOUT_MS` env vars;
...
1. **Pick a scope.** Infer it from the user's words when you can
   ...
   current directory / SpiderByte-specific / `.spiderbyte` → project-local). When
...
3. **Write and tell them how to reload MCP servers.** Preserve unrelated
   ...
   restart `spiderbyte` for the change to take effect.
