---
name: update-config
description: Inspect or edit spiderbyte's own config — `config.toml` (model, provider, permission, hooks) and `tui.toml` (theme, editor, notifications, auto-update). Use when the user asks what a setting does, wants to change one, or needs to fix a deprecated config key / environment variable warning.
---

# Configure spiderbyte (update-config)

Help the user inspect, change, and validate spiderbyte's configuration files. The files are **TOML** with **snake_case** keys.

## The two config files

spiderbyte has two TOML config files, both under `<SPIDERBYTE_HOME>/`, both snake_case, but with different ownership — decide which one the user means before doing anything.

The runtime resolves the data directory as `SPIDERBYTE_HOME` first, falling back to `~/.spiderbyte`. Before doing anything, resolve the actual directory with Bash so you don't write to the wrong place. Check whether `SPIDERBYTE_HOME` is set and fall back to `~/.spiderbyte` when it is empty:

```bash
echo "$SPIDERBYTE_HOME"
echo "$HOME/.spiderbyte"
```

Use the first line when it is non-empty; otherwise use the second line. In the rest of this skill, `<SPIDERBYTE_HOME>` means that resolved root — **never assume `~/.spiderbyte`**.
...
```
https://github.com/SpiderByte/spiderbyte/tree/main/docs/en/configuration/config-files.html
```
...
- Location: `<SPIDERBYTE_HOME>/config.toml` or `<SPIDERBYTE_HOME>/tui.toml`. For other scopes/files, defer to the official docs.
...
3. **Copy out a candidate (do not create from scratch)**: use **Bash** to copy the target verbatim — `cp config.toml config-new.toml` (same directory, `-new` suffix; for tui.toml, `cp tui.toml tui-new.toml`). **Leave the original untouched for now.**
   - Only when the target doesn't exist (nothing to copy) should you use **Write** to create a minimal skeleton candidate (e.g. just the comment line `# <SPIDERBYTE_HOME>/config.toml`).
...
5. **Validate the candidate** (see Capability 3, via `spyderbyte doctor`). **If anything fails, keep Editing the candidate and re-validate, looping until it all passes.**
...
Use **`spyderbyte doctor`** to validate the candidate you wrote — it doesn't start the TUI and doesn't modify any file; it runs spyderbyte's own parser + schema (syntax and schema together), so it's the authoritative check. Pick the subcommand by which file you changed, and pass the **candidate** path explicitly:

- changed `config.toml` → `spyderbyte doctor config <config-new.toml path>`
- changed `tui.toml` → `spyderbyte doctor tui <tui-new.toml path>`
...
Then do two checks `spyderbyte doctor` can't:
...
> To also check whether the currently **active** config is OK overall, run `spyderbyte doctor` with no path (it checks the default `config.toml` + `tui.toml`, showing a missing one as skipped).
...
- **Deprecated environment variable** — e.g. `Environment variable SPIDERBYTE_LOOP_MAX_RETRIES_PER_STEP is deprecated; use SPIDERBYTE_LOOP_MAX_ATTEMPTS_PER_STEP instead.` The old variable still works, but this is **not** fixable by editing `config.toml`/`tui.toml` — tell the user to rename the variable where they set it (shell profile, CI environment, launcher script). Do not add anything to the config files for this.
