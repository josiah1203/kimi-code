...
description: Import Claude Code and Codex instructions, skills, and MCP settings into SpiderByte.
...
Help them migrate selected local Claude Code and Codex assets into SpiderByte.
...
- Do **not** migrate `.agents` content. SpiderByte already supports `.agents`
...
- Only write under SpiderByte targets:
  - User-global: `$SPIDERBYTE_HOME` if set, otherwise `~/.spiderbyte`.
  - Project instructions/skills: `<project root>/.spiderbyte`, where the project
    root is the nearest parent directory containing `.git`; if no `.git` exists,
    use the current working directory.
  - Project-local MCP: `<cwd>/.spiderbyte/mcp.json`, because SpiderByte reads the
    current working directory's SpiderByte-specific MCP file, not every project-root
    `.spiderbyte/mcp.json` from subdirectories.
- Preserve existing SpiderByte files. Never overwrite existing skills or replace an
...
Resolve paths explicitly; `~` is the real OS home, and SpiderByte home follows
`$SPIDERBYTE_HOME` before `~/.spiderbyte`.
...
SpiderByte-readable, and project-root `.mcp.json` is already read by SpiderByte as a
...
- `$SPIDERBYTE_HOME/AGENTS.md`, or `~/.spiderbyte/AGENTS.md` if the env var is not
  set.

Map project-level instruction sources to:

- `<project root>/.spiderbyte/AGENTS.md`
...
- `$SPIDERBYTE_HOME/skills/`, or `~/.spiderbyte/skills/` if the env var is not set.

Map project-level skill sources to:

- `<project root>/.spiderbyte/skills/`
...
- Read a bundle's `SKILL.md` enough to verify that directory skills have
  frontmatter with non-empty `name` and `description`, because SpiderByte requires
...
- If two source entries would write the same target path, keep the first one in
  ...
  3. user Claude
  4. user Codex
- Warn when a source skill uses Claude/Codex-specific fields or syntax that SpiderByte
...
For the preview, collect MCP candidates and normalize them into SpiderByte's MCP shape
when possible:
...
- Map Codex fields to SpiderByte fields:
  - `command` -> `command`
  - `args` -> `args`
  - `env` -> `env`
  - `cwd` -> `cwd`
...
- Do not import project-root `.mcp.json`; SpiderByte already reads it.

For each MCP candidate, choose the target scope in the preview:

- User-level source -> user-global MCP target (`$SPIDERBYTE_HOME/mcp.json` or
  `~/.spiderbyte/mcp.json`).
- Project-level source -> project-local SpiderByte MCP target (`<cwd>/.spiderbyte/mcp.json`). If `<cwd>` is not the project root, call this out in the preview so the user understands when SpiderByte will load it.
...
- collision policy: keep existing SpiderByte entries on name conflict,
...
- Tell the user to start a new session (for example `/new`) or restart SpiderByte
