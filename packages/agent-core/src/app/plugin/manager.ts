...
export interface PluginManagerOptions {
  readonly spiderByteHomeDir: string;
...
export class PluginManager {
  private readonly spiderByteHomeDir: string;
  ...
  constructor(options: PluginManagerOptions) {
    this.spiderByteHomeDir = options.spiderByteHomeDir;
    this.discoverSkills = options.discoverSkills ?? discoverFileSkills;
  }

  async load(): Promise<void> {
    const file = await readInstalled(this.spiderByteHomeDir);
  ...
  async install(source: string): Promise<PluginRecord> {
    ...
    try {
      if (resolved.kind === 'local-path') {
        ...
        zipTmpDir = await mkdtemp(path.join(tmpdir(), 'spiderbyte-plugin-zip-'));
      ...
      managedCopy = await copyPluginToManagedRoot(this.spiderByteHomeDir, id, sourceRoot);
    ...
    for (const record of this.records.values()) {
      ...
      for (const hook of record.manifest.hooks ?? []) {
        out.push({
          ...
          cwd: record.root,
          env: {
            SPIDERBYTE_HOME: this.spiderByteHomeDir,
            SPIDERBYTE_PLUGIN_ROOT: record.root,
          },
        });
      }
    }
  ...
  private async persist(records: ReadonlyMap<string, PluginRecord>): Promise<void> {
    ...
    await writeInstalled(this.spiderByteHomeDir, { version: 1, plugins: installed });
  }
...
async function copyPluginToManagedRoot(
  spiderByteHomeDir: string,
  id: string,
  sourceRoot: string,
): Promise<ManagedPluginCopy> {
  const managedRoot = path.join(spiderByteHomeDir, 'plugins', 'managed', id);
...
const SPIDERBYTE_NODE_FALLBACK_SUBCOMMAND = '__plugin_run_node';
...
function withPluginMcpRuntime(
  ...
  const env = {
    ...config.env,
    SPIDERBYTE_HOME: spiderByteHomeDir,
    SPIDERBYTE_PLUGIN_ROOT: pluginRoot,
  };
  ...
  if (config.command === 'node' && isNativeBinary()) {
    return {
      ...config,
      command: process.execPath,
      args: [SPIDERBYTE_NODE_FALLBACK_SUBCOMMAND, ...(config.args ?? [])],
      cwd: config.cwd ?? pluginRoot,
      env,
    };
  }
