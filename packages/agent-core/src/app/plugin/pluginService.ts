 ...
 * skill discovery, and resolves installed plugin endpoint settings through the
...
export class PluginService extends Service implements IPluginService {
  ...
  constructor(
    ...
    this.manager = new PluginManager({
      spiderByteHomeDir: this.homeDir,
    ...
    const reload = this.enqueueMutation(async () => {
      try {
        ...
        throw new Error2(
          ...
          { cause: this.loadError, details: { spiderByteHomeDir: this.homeDir } },
        );
      }
    });
