...
export async function readInstalled(spiderByteHomeDir: string): Promise<InstalledFile> {
  const filePath = path.join(spiderByteHomeDir, INSTALLED_REL);
  let text: string;
  try {
...
export async function writeInstalled(spiderByteHomeDir: string, data: InstalledFile): Promise<void> {
  const dir = path.join(spiderByteHomeDir, 'plugins');
