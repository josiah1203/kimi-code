...
const SPIDERBYTE_PLUGIN_ROOT_PATH = 'spiderbyte.plugin.json';
const SPIDERBYTE_PLUGIN_DIR_PATH = '.spiderbyte-plugin/plugin.json';
...
export async function parseManifest(pluginRoot: string): Promise<ParsedManifestResult> {
  const rootJsonPath = path.join(pluginRoot, SPIDERBYTE_PLUGIN_ROOT_PATH);
  const dirJsonPath = path.join(pluginRoot, SPIDERBYTE_PLUGIN_DIR_PATH);
  ...
  if (!rootJsonExists && !dirJsonExists) {
    return {
      diagnostics: [
        {
          severity: 'error',
          message: `No manifest at ${SPIDERBYTE_PLUGIN_ROOT_PATH} or ${SPIDERBYTE_PLUGIN_DIR_PATH}`,
        },
      ],
    };
  }
  ...
  const manifestKind: PluginManifestKind = rootJsonExists ? 'spiderbyte-plugin-root' : 'spiderbyte-plugin-dir';
...
function recordUnsupportedRuntimeFields(
  ...
  for (const field of UNSUPPORTED_RUNTIME_FIELDS) {
    ...
    diagnostics.push({
      severity: 'info',
      message: `"${field}" is present but not supported by SpiderByte plugins`,
    });
  }
}
