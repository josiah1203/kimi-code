...
export interface IBootstrapOptions {
  ...
  readonly clientIdentity: SpiderByteHostIdentity;
...
export function resolveBootstrapOptions(input: BootstrapInput): IBootstrapOptions {
  ...
  const homeDir = resolveSpiderByteHome(input.homeDir, env, osHomeDir);
...
export function resolveSpiderByteHome(
  homeDir?: string,
  env: NodeJS.ProcessEnv = process.env,
  osHomeDir: string = homedir(),
): string {
  return homeDir ?? env['SPIDERBYTE_HOME'] ?? join(osHomeDir, '.spiderbyte');
}
...
export function resolveConfigPath(input: {
  ...
  return input.configPath ?? join(resolveSpiderByteHome(input.homeDir), 'config.toml');
}

export function ensureSpiderByteHome(homeDir: string): void {
