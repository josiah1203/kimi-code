 ...
 * (`AGENTS_MD_PLAIN_NAMES` / `dotSpiderByteAgentsMdPath` / `findAgentsMdInDir`)
...
export function dotSpiderByteAgentsMdPath(dir: string): string {
  return join(dir, '.spiderbyte', 'AGENTS.md');
}

export function agentsMdCandidatePaths(dir: string): string[] {
  return [dotSpiderByteAgentsMdPath(dir), ...AGENTS_MD_PLAIN_NAMES.map((name) => join(dir, name))];
}
...
export async function findAgentsMdInDir(
  ...
  const found: string[] = [];
  const dotSpiderByte = dotSpiderByteAgentsMdPath(dir);
  if (await isNonEmptyFile(deps, dotSpiderByte)) found.push(dotSpiderByte);
...
export async function loadAgentsMdForRoots(
  ...
  const brandDir = brandHome ?? join(realHome, '.spiderbyte');
  ...
  for (const workDir of workDirs) {
    ...
    for (const dir of dirs) {
      await collect(dotSpiderByteAgentsMdPath(dir));
      for (const fileName of AGENTS_MD_PLAIN_NAMES) {
