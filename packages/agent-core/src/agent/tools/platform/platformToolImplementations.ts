...
function providerValidationSummary(result: {
  ...
  // Token counters remain in the local usage record; they are not part
...
async function createRun(
  ...
  const run = await runs.create({
    ...
    metadata: { kind, tool: 'spiderbyte-platform', ...metadata, required: true },
  });
...
function platformWhen(accessor: ServicesAccessor): boolean {
  ...
  // The master experimental switch is also used by isolated provider tests and
...
export class ProviderTool implements IProviderTool {
  ...
  readonly description =
    'Inspect configured provider connections, select a governed provider/model for subsequent model requests, validate a connection through its real provider boundary, or discover models. Secret material is never returned.';
  ...
  private async execute(args: ProviderToolInput, ctx: ExecutableToolContext): Promise<ExecutableToolResult> {
    try {
      ...
      if (args.operation === 'clear') {
        ...
        return success({
          selected: false,
          message: 'Platform provider binding cleared; subsequent model requests use the configured model.',
        });
      }
      if (args.operation === 'select') {
        ...
        return success({
          ...
          message: 'Subsequent model requests in this agent use this governed platform connection.',
        });
      }
...
export class ExecutionTargetTool implements IExecutionTargetTool {
  ...
  readonly description =
    'Discover and govern local and customer-managed execution targets, including readiness and leases. Credentials remain opaque and are never returned.';
...
export class AutomationTool implements IAutomationTool {
  ...
  readonly description =
    'Create, inspect, pause, resume, and trigger durable automations. A queued automation creates the normal local prompt/Run linkage and keeps approval and retry state in the platform service.';
