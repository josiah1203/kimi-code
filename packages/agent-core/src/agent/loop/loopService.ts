...
import { BugIndicatingError, ErrorCodes, Error2, isError2, toSpiderByteErrorPayload } from '#/errors';
...
export class AgentLoopService extends Disposable implements IAgentLoopService {
  ...
  private async runTurn(
    ...
    try {
      ...
      if (result !== undefined) {
        const error = result.type === 'failed' ? toSpiderByteErrorPayload(result.error) : undefined;
