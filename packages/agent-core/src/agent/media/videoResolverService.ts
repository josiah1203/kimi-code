 ...
 *
 * Resolves each `spiderbyte-file://` video reference in the projected wire messages
...
import { type SpiderByteFileRef, isSpiderByteFileUrl, parseSpiderByteFileUrl } from './spiderbyteFileUrl';
...
export class AgentVideoResolverService implements IAgentVideoResolverService {
  ...
  async resolve(
    ...
    if (!messages.some(hasSpiderByteFileVideoPart)) return messages;

    let changed = false;
    const out: Message[] = [];
    for (const message of messages) {
      if (!hasSpiderByteFileVideoPart(message)) {
        out.push(message);
        continue;
      }
      ...
      for (const part of message.content) {
        const ref =
          part.type === 'video_url' ? parseSpiderByteFileUrl(part.videoUrl.url) : undefined;
...
function hasSpiderByteFileVideoPart(message: Message): boolean {
  return message.content.some(
    (part) => part.type === 'video_url' && isSpiderByteFileUrl(part.videoUrl.url),
  );
}

function tag(ref: SpiderByteFileRef): ContentPart {
