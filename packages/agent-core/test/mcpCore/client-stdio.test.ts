...
describe('StdioMcpClient', () => {
  ...
  it('uses defaultCwd when config.cwd is omitted', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'spiderbyte-mcp-default-cwd-'));
  ...
  it('prefers explicit config.cwd over defaultCwd', async () => {
    const defaultCwd = mkdtempSync(join(tmpdir(), 'spiderbyte-mcp-default-cwd-'));
    const configuredCwd = mkdtempSync(join(tmpdir(), 'spiderbyte-mcp-configured-cwd-'));
    const client = new StdioMcpClient(
      {
        transport: 'stdio',
        command: process.execPath,
  ...
  it('resolves relative config.cwd from defaultCwd', async () => {
    const defaultCwd = mkdtempSync(join(tmpdir(), 'spiderbyte-mcp-relative-cwd-'));
  ...
  it('forwards configured env to the spawned server', async () => {
    const client = new StdioMcpClient({
      ...
      command: process.execPath,
      args: [stdioFixture],
      env: { SPIDERBYTE_TEST_ENV: 'forwarded-value' },
    });
    try {
      await client.connect();
      const result = await client.callTool('read_env', { name: 'SPIDERBYTE_TEST_ENV' });
  ...
  it('inherits parent process env so PATH/HOME survive; config.env overrides on conflict', async () => {
    const parentOnly = `SPIDERBYTE_TEST_PARENT_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    const shared = `SPIDERBYTE_TEST_SHARED_${Date.now()}_${Math.random().toString(36).slice(2)}`;
  ...
  it('captures recent stderr into a snapshot the manager can attach to errors', async () => {
    const banner = `spiderbyte-test-stderr-${Date.now()}`;
    const client = new StdioMcpClient({
      transport: 'stdio',
      command: process.execPath,
      args: [stderrThenExitFixture],
      env: { SPIDERBYTE_TEST_MCP_STDERR: banner },
    });
    try {
  ...
  it('notifies an unexpected-close listener when the child exits after connect', async () => {
    const banner = `spiderbyte-test-crash-${Date.now()}`;
    const client = new StdioMcpClient({
      transport: 'stdio',
      command: process.execPath,
      args: [crashAfterConnectFixture],
      env: { SPIDERBYTE_TEST_MCP_EXIT_AFTER_MS: '50', SPIDERBYTE_TEST_MCP_STDERR: banner },
    });
  ...
  it('buffers an early close and replays it on listener registration', async () => {
    const banner = `spiderbyte-test-early-${Date.now()}`;
    const client = new StdioMcpClient({
      transport: 'stdio',
      command: process.execPath,
      args: [crashAfterConnectFixture],
      env: { SPIDERBYTE_TEST_MCP_STDERR: banner, SPIDERBYTE_TEST_MCP_EXIT_CODE: '0' },
    });
    try {
      await client.connect();
...
describe('mergeStdioEnv', () => {
  ...
  it('does not depend on a filesystem cwd fixture for env merging', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'spiderbyte-mcp-env-'));
