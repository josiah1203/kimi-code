...
import { SPIDERBYTE_MCP_CLIENT_NAME } from '#/mcpCore/client-shared';
...
describe('McpConnectionManager', () => {
  ...
  it('starts stdio servers in stdioCwd when config.cwd is omitted', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'spiderbyte-mcp-manager-cwd-'));
    ...
    try {
      await cm.connectAll({
        crashy: {
          ...
          env: { SPIDERBYTE_TEST_MCP_EXIT_AFTER_MS: '500', SPIDERBYTE_TEST_MCP_STDERR: 'fatal: out of memory' },
    ...
    try {
      await cm.connectAll({
        nope: {
          ...
          env: { SPIDERBYTE_TEST_MCP_STDERR: 'fatal: missing API token SPIDERBYTE_X' },
          startupTimeoutMs: 4_000,
        },
      });
      const entry = cm.get('nope');
      expect(entry?.status).toBe('failed');
      expect(entry?.error).toContain('fatal: missing API token SPIDERBYTE_X');
    } finally {
      await cm.shutdown();
    }
  }, 10000);
