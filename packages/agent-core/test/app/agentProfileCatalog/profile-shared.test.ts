...
describe('systemPromptVars', () => {
  ...
  it('defaults host-identity variables to the CLI text', () => {
    ...
    expect(vars['product_name']).toBe('SpiderByte CLI');
  ...
  it('lets the context override host-identity variables', () => {
    const vars = systemPromptVars(
      { productName: 'SpiderByte Desktop', replyStyleGuide: 'GUI_STYLE' },
      { skillActive: true },
    );

    expect(vars['product_name']).toBe('SpiderByte Desktop');
...
describe('renderSystemPromptResult', () => {
  ...
  it('renders the host identity from the context, defaulting to the CLI text', () => {
    ...
    expect(fallback).toContain('You are SpiderByte CLI,');
    ...
    expect(overridden).toContain('You are SpiderByte Desktop,');
    expect(overridden).toContain('GUI_STYLE');
    expect(overridden).not.toContain('SpiderByte CLI');
  });
