...
describe('sessionExport', () => {
  ...
  it('exports a v2 session directory with per-agent wire activity and optional global log', async () => {
    ...
    await writeFile(join(sessionDir, 'logs', 'spiderbyte.log'), '{"msg":"session"}\n', 'utf-8');
    await writeFile(
    ...
    const globalLogPath = join(tmp, 'logs', 'spiderbyte.log');
    ...
    expect(result.entries).toEqual([
      ...
      'logs/spiderbyte.log',
      'state.json',
      'logs/global/spiderbyte.log',
    ]);
    expect(result.manifest).toMatchObject({
      sessionId: 'ses_demo',
      spiderbyteVersion: '1.0.0-test',
      title: 'Demo',
      workspaceDir: '/workspace/demo',
      sessionFirstActivity: '2023-11-14T22:13:20.000Z',
      sessionLastActivity: '2023-11-14T22:13:25.000Z',
      sessionLogPath: 'logs/spiderbyte.log',
      globalLogPath: 'logs/global/spiderbyte.log',
    });
  });
  ...
  it('uses a timestamped default output path when outputPath is omitted', async () => {
    ...
    try {
      ...
      expect(basename(result.zipPath)).toMatch(/^spiderbyte-debug-ses_defa-\d{8}-\d{6}\.zip$/);
  ...
  it('keeps the session log bound when it rotates as wire scanning starts', async () => {
    ...
    const logPath = join(sessionDir, 'logs', 'spiderbyte.log');
    ...
    try {
      ...
      expect(result.manifest.sessionLogPath).toBe('logs/spiderbyte.log');
      expect(result.entries).toContain('logs/spiderbyte.log');
      await expect(readZipEntry(outputPath, 'logs/spiderbyte.log')).resolves.toEqual(log);
    } finally {
  ...
  it('keeps the global log bound when it rotates as wire scanning starts', async () => {
    ...
    try {
      ...
      expect(rotated).toBe(true);
      expect(result.manifest.globalLogPath).toBe('logs/global/spiderbyte.log');
      expect(result.entries).toContain('logs/global/spiderbyte.log');
      await expect(readZipEntry(outputPath, 'logs/global/spiderbyte.log')).resolves.toEqual(log);
    } finally {
      fsOpenHook.afterOpen = undefined;
    }
  });
  ...
  it('closes pre-opened logs when manifest creation fails before writer ownership', async () => {
    ...
    const sessionLogPath = join(sessionDir, 'logs', 'spiderbyte.log');
    const globalLogPath = join(tmp, 'logs', 'spiderbyte.log');
    const wirePath = join(sessionDir, 'agents', 'main', 'wire.jsonl');
    ...
    expect((await readdir(tmp)).filter((entry) => entry.startsWith('.spiderbyte-session-export-'))).toEqual(
      [],
    );
  });
  ...
  it('omits the optional global log when the configured file is missing', async () => {
    ...
    expect(result.entries).not.toContain('logs/global/spiderbyte.log');
  });
  ...
  it('closes a pre-opened source when it conflicts with the output path', async () => {
    ...
    await expect(
      writeExportZip({
        ...
        sessionDir: tmp,
        sessionFiles: [],
        extraEntries: [{ source, target: 'logs/global/spiderbyte.log' }],
      }),
    ).rejects.toMatchObject({ code: 'session.export_output_conflict' });
  ...
  it('archives a bound session log after its path is rotated', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'session-export-test-'));
    const logPath = join(tmp, 'logs', 'spiderbyte.log');
    const rotatedPath = `${logPath}.1`;
    ...
    ).resolves.toContain('logs/spiderbyte.log');
    await expect(readZipEntry(outputPath, 'logs/spiderbyte.log')).resolves.toEqual(original);
    expect(closeCalls).toBe(1);
  });
  ...
  it('includes a bounded Web log in the exported archive', async () => {
    ...
    expect(result.entries).toContain('logs/spiderbyte-web.jsonl');
    expect(result.manifest.webLogPath).toBe('logs/spiderbyte-web.jsonl');
    await expect(readZipEntry(outputPath, 'logs/spiderbyte-web.jsonl')).resolves.toEqual(
  ...
  it('includes the desktop app log when given', async () => {
    ...
    await mkdir(sessionDir, { recursive: true });
    await writeFile(join(sessionDir, 'state.json'), '{}\n', 'utf-8');
    const desktopLogPath = join(tmp, 'logs', 'spiderbyte-desktop.log');
    await mkdir(join(tmp, 'logs'), { recursive: true });
    ...
    expect(result.entries).toContain('logs/spiderbyte-desktop.log');
    expect(result.manifest.desktopLogPath).toBe('logs/spiderbyte-desktop.log');
    await expect(readZipEntry(outputPath, 'logs/spiderbyte-desktop.log')).resolves.toEqual(
  ...
  it('skips a missing desktop app log silently', async () => {
    ...
    expect(result.entries).not.toContain('logs/spiderbyte-desktop.log');
