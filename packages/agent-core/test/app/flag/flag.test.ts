...
const exampleFlag: FlagDefinitionInput = {
  ...
  env: 'SPIDERBYTE_EXPERIMENTAL_EXAMPLE_FLAG',
  default: true,
  surface: 'core',
};
...
const defaultOnFlag: FlagDefinitionInput = {
  ...
  env: 'SPIDERBYTE_EXPERIMENTAL_DEFAULT_ON_FLAG',
  emergencyDisableEnv: 'SPIDERBYTE_DISABLE_DEFAULT_ON_FLAG',
  default: true,
  surface: 'core',
};
...
describe('FlagRegistryService', () => {
  it('registers and resolves by id', () => {
    ...
    expect(reg.get('example_flag')?.env).toBe('SPIDERBYTE_EXPERIMENTAL_EXAMPLE_FLAG');
  });
...
describe('FlagService', () => {
  ...
  beforeEach(() => {
    ...
    homeDir = `/tmp/spiderbyte-flag-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  });
  ...
  it('lets per-feature env override config', async () => {
    const { config, flags } = makeFlags({
      SPIDERBYTE_EXPERIMENTAL_EXAMPLE_FLAG: 'true',
    });
    await config.set(EXPERIMENTAL_SECTION, { example_flag: false });
    const state = flags.explain('example_flag');
    expect(state?.enabled).toBe(true);
  ...
  it('lets an emergency disable switch win over the master enable switch', () => {
    const { flags } = makeFlags({
      [MASTER_ENV]: '1',
      SPIDERBYTE_DISABLE_DEFAULT_ON_FLAG: 'true',
    });
  ...
  it('treats truthy env values case-insensitively', () => {
    const { flags } = makeFlags({ SPIDERBYTE_EXPERIMENTAL_EXAMPLE_FLAG: 'YES' });
    expect(flags.enabled('example_flag')).toBe(true);
  });

  it('treats falsy env values case-insensitively', () => {
    const { flags } = makeFlags({ SPIDERBYTE_EXPERIMENTAL_EXAMPLE_FLAG: 'off' });
    expect(flags.enabled('example_flag')).toBe(false);
  });

  it('reads only the env name declared in the registry', () => {
    const { flags } = makeFlags({ SPIDERBYTE_EXPERIMENTAL_UNKNOWN: 'false' });
    expect(flags.enabled('example_flag')).toBe(true);
  });

  it('ignores garbage env values', () => {
    const { flags } = makeFlags({ SPIDERBYTE_EXPERIMENTAL_EXAMPLE_FLAG: 'maybe' });
    expect(flags.enabled('example_flag')).toBe(true);
  });
