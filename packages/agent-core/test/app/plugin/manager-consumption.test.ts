...
async function isolatedTmpdir(): Promise<string> {
  const dir = await mkdtemp(path.join(tmpdir(), 'spiderbyte-isolated-tmp-'));
...
async function makePlugin(
  ...
  await writeFile(path.join(root, 'spiderbyte.plugin.json'), JSON.stringify(manifest), 'utf8');
  return realpath(root);
}
...
describe('PluginManager consumption plane', () => {
  ...
  it('pluginSkillRoots() excludes plugins in error state', async () => {
    ...
    await writeFile(
      path.join(await managedPluginRoot(manager, 'demo'), 'spiderbyte.plugin.json'),
      '{ not json',
      'utf8',
    );
  ...
  it('removes the zip temp dir and reports the original source when a zip plugin has no manifest', async () => {
    ...
    expect(message).not.toContain('example-plugin-zip');
    expect(await zipTempLeftovers(isolated)).toEqual([]);
  ...
  it('setMcpServerEnabled() persists explicit MCP server state with cwd + env + runtime name', async () => {
    ...
    expect(manager.enabledMcpServers()).toEqual(
      expect.objectContaining({
        'plugin-demo:finance': expect.objectContaining({
          command: 'finance-mcp',
          cwd: managedRoot,
          env: expect.objectContaining({ SPIDERBYTE_HOME: home, SPIDERBYTE_PLUGIN_ROOT: managedRoot }),
        }),
    ...
    const reloaded = new PluginManager({ spiderByteHomeDir: home });
    await reloaded.load();
  ...
  it('reload() picks up edits to the managed plugin copy', async () => {
    ...
    const managedRoot = await managedPluginRoot(manager, 'demo');
    await writeFile(
      path.join(managedRoot, 'spiderbyte.plugin.json'),
    ...
    expect(manager.enabledHooks()).toEqual([
      {
        ...
        timeout: 10,
        cwd: installedRoot,
        env: { SPIDERBYTE_HOME: home, SPIDERBYTE_PLUGIN_ROOT: installedRoot },
      },
    ]);
  });
  ...
  it('install() from github URL overwrites an existing zip-url install (CDN migration)', async () => {
    ...
    await writeFile(
      path.join(cdnSource, 'spiderbyte.plugin.json'),
    ...
    await writeFile(
      path.join(ghSource, 'spiderbyte.plugin.json'),
      ...
      expect(server).toEqual(
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            SPIDERBYTE_HOME: home,
            SPIDERBYTE_PLUGIN_ROOT: managedRoot,
