...
describe('plugin archive extraction', () => {
  ...
  it('extracts a zip and detects a nested plugin root', async () => {
    ...
    await writeFile(join(nested, 'spiderbyte.plugin.json'), JSON.stringify({ name: 'zip-demo' }), 'utf8');
    ...
    await expect(readFile(join(detectedRoot, 'spiderbyte.plugin.json'), 'utf8')).resolves.toContain('zip-demo');
  });
});
