 ...
 *
 * Run: pnpm --filter @spiderbyte/agent-core exec vitest run test/app/plugin/pluginService.test.ts
 */
...
function makeHost(homeDir: string): ScopedTestHost {
...
describe('PluginService (plugin boundary)', () => {
  ...
  async function makeHome(): Promise<string> {
    const home = await mkdtemp(path.join(tmpdir(), 'spiderbyte-home-'));
  ...
  it('restores the previous managed copy when reinstall persistence fails', async () => {
    ...
    try {
      ...
      await expect(readFile(path.join(previous.root, 'spiderbyte.plugin.json'), 'utf8')).resolves.toContain(
        '"version":"1.0.0"',
      );
  ...
  it('does not inject managed env when neither env nor the spyderbyte provider supplies it', async () => {
    const home = await makeHome();
    await writeValidInstalledFile(home);
    const host = makeHost(home);
    try {
      ...
      expect(env).not.toHaveProperty('SPIDERBYTE_BASE_URL');
      expect(env).not.toHaveProperty('SPIDERBYTE_OAUTH_HOST');
    } finally {
      host.dispose();
    }
  });
