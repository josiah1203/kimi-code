 ...
 * Run: pnpm --filter @spiderbyte/agent-core exec vitest run test/app/plugin/manager.test.ts
 */
...
describe('PluginManager', () => {
  ...
  beforeEach(async () => {
    ...
    await writeFile(
      join(root, 'spiderbyte.plugin.json'),
      JSON.stringify({
        name: 'demo',
        commands: ['./commands'],
    ...
    expect(manager.enabledHooks()).toEqual([
      {
        ...
        command: 'echo stop',
        cwd: root,
        env: { SPIDERBYTE_HOME: home, SPIDERBYTE_PLUGIN_ROOT: root },
      },
    ]);
  ...
  it('installs a local-path plugin into the managed root', async () => {
    ...
    try {
      await writeFile(join(sourceRoot, 'spiderbyte.plugin.json'), JSON.stringify({ name: 'other' }), 'utf8');
      const manager = new PluginManager({ spiderByteHomeDir: home });
  ...
  it('installs a zip-url plugin', async () => {
    ...
    try {
      await writeFile(join(sourceRoot, 'spiderbyte.plugin.json'), JSON.stringify({ name: 'zip-plugin' }), 'utf8');
  ...
  it('installs a github plugin through codeload', async () => {
    ...
    try {
      await writeFile(join(sourceRoot, 'spiderbyte.plugin.json'), JSON.stringify({ name: 'github-plugin' }), 'utf8');
      execFileSync('zip', ['-qr', zipPath, '.'], { cwd: sourceRoot });
