...
describe('AgentIdentityService freeze', () => {
  ...
  it('ignores a config edit made after the freeze', async () => {
    const { identity, config } = createIdentity(
      { name: 'Acme' },
      { hostRequestHeaders: { 'User-Agent': 'spiderbyte-cli/1.0' } },
    );
...
describe('buildAgentIdentitySnapshot products', () => {
  const HOST = { 'User-Agent': 'spiderbyte-cli/1.2.3 (darwin)', 'X-Msh-Device-Id': 'device-1' };
  ...
  it.each(['user-agent', 'USER-AGENT'])(
    ...
    (key) => {
      const snapshot = buildAgentIdentitySnapshot({
        slug: 'acme',
        hostRequestHeaders: { [key]: 'spiderbyte-cli/1.2.3', 'X-Msh-Device-Id': 'device-1' },
      });
  ...
  it('passes a lowercase spelling through untouched when no identity is claimed', () => {
    const snapshot = buildAgentIdentitySnapshot({
      hostRequestHeaders: { 'user-agent': 'spiderbyte-cli/1.2.3' },
    });
    expect(snapshot.thirdPartyUserAgent).toBe('spiderbyte-cli/1.2.3');
    expect(snapshot.requestHeaders).toEqual({ 'user-agent': 'spiderbyte-cli/1.2.3' });
  });
});
