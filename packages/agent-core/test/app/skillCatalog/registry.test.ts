...
describe('InMemorySkillCatalog skill listing', () => {
  it('groups skills by source under canonical section headings', () => {
    ...
    const rendered = registry.getSpiderByteSkillsDescription();
  ...
  it('omits source headings that have no skills', () => {
    const rendered = makeRegistry([makeSkill('alpha', 'user')]).getSpiderByteSkillsDescription();
  ...
  it('renders a No skills placeholder for an empty registry', () => {
    const rendered = new InMemorySkillCatalog().getSpiderByteSkillsDescription();
  ...
  it('sorts skills alphabetically within a source', () => {
    ...
    ]).getSpiderByteSkillsDescription();
  ...
  it('registerBuiltinSkill stamps non-builtin skills as builtin', () => {
    ...
    expect(sectionFor(registry.getSpiderByteSkillsDescription(), '### Built-in')).toContain('theme');
  });
});
...
describe('InMemorySkillCatalog prompt rendering', () => {
  it('expands raw, positional, named, and context placeholders', () => {
    const rendered = new InMemorySkillCatalog().renderSkillPrompt(
      stubSkill('commit', {
        ...
        content:
          'raw=$ARGUMENTS zero=$0 one=$1 second=$ARGUMENTS[1] flag=$flag message=$message dir=${SPIDERBYTE_SKILL_DIR} session=${SPIDERBYTE_SESSION_ID}',
  ...
  it('expands context placeholders and still appends args when no argument placeholder is used', () => {
    const rendered = new InMemorySkillCatalog().renderSkillPrompt(
      stubSkill('review', {
        dir: '/skills/review',
        content: 'Use ${SPIDERBYTE_SKILL_DIR}/references/checklist.md.',
      }),
      'src/app.ts',
      { sessionId: 'ses_1' },
    );
