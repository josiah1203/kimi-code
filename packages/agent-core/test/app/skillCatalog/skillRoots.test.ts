...
describe('skillRoots', () => {
  ...
  describe('projectRoots', () => {
    it('resolves the brand .spiderbyte/skills directory at the .git root', async () => {
      await markGitRoot();
      await mkdir(join(root, '.spiderbyte/skills/commit'), { recursive: true });

      const roots = await projectRoots(root);

      expect(roots.some((r) => r.path.endsWith('.spiderbyte/skills') && r.source === 'project')).toBe(
        true,
      );
    });
    ...
    it('falls back to the generic .agents/skills directory', async () => {
      ...
      expect(roots.some((r) => r.path.endsWith('.spiderbyte/skills'))).toBe(false);
    });
    ...
    it('walks up from a child directory to the .git root', async () => {
      ...
      expect(roots.some((r) => r.path.endsWith('.spiderbyte/skills'))).toBe(true);
    });
    ...
    it('orders the brand directory before the generic directory', async () => {
      ...
      const roots = await projectRoots(root);
      const brandIdx = roots.findIndex((r) => r.path.endsWith('.spiderbyte/skills'));
