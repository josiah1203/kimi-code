...
describe('BootstrapService (scoped)', () => {
  ...
  it('resolves homeDir/configPath from the seeded context token', () => {
    const host = createScopedTestHost(
      bootstrapSeed({ homeDir: '/tmp/spiderbyte-home', clientIdentity: stubClientIdentity }),
    );
    const svc = host.app.accessor.get(IBootstrapService);
    expect(svc.homeDir).toBe('/tmp/spiderbyte-home');
    expect(svc.configPath).toBe('/tmp/spiderbyte-home/config.toml');
...
describe('resolveBootstrapOptions', () => {
  it('prefers explicit homeDir over SPIDERBYTE_HOME over osHomeDir', () => {
    expect(
...
describe('bootstrap() storage seeding', () => {
  it('seeds IFileSystemStorageService as a FileStorageService instance', () => {
    const { app } = bootstrap({ homeDir: '/tmp/spiderbyte-home', clientIdentity: stubClientIdentity });
    try {
