 ...
 * persistence wrapper), and the `SPIDERBYTE_MODEL_*` env overlay.
 *
...
import { spiderbyteModelEnvOverlay, ENV_MODEL_ALIAS_KEY } from '#/app/kosongConfig/envOverlay';
import {
...
describe('effectiveModelConfig', () => {
  ...
  it('infers Anthropic effort metadata for an unknown Claude-marked model on a non-vendor Anthropic provider', () => {
    expect(
      effectiveModelConfig(
        {
          provider: 'custom',
  ...
  it('infers Anthropic effort metadata for a bare Claude family alias on a non-vendor Anthropic provider', () => {
    expect(
      effectiveModelConfig(
        {
          provider: 'custom',
  ...
  it('does not infer Anthropic effort metadata for a clearly non-Claude model on a non-vendor Anthropic provider', () => {
    expect(
      effectiveModelConfig(
        {
          provider: 'custom',
  ...
  it('does not infer Anthropic effort metadata for an external provider routed through the Anthropic protocol', () => {
...
function applyModelEnvOverlay(
  ...
  const changed = spiderbyteModelEnvOverlay.apply(
    effective,
    (name) => env[name],
    (domain, value) => {
...
describe('spiderbyteModelEnvOverlay', () => {
  it('does nothing when SPIDERBYTE_MODEL_NAME is absent', () => {
    const effective = {
      models: {
  ...
  it('maps extended model metadata and request overrides', () => {
    const { changed, effective } = applyModelEnvOverlay({
      ...
      SPIDERBYTE_MODEL_CAPABILITIES: 'Image_In, thinking , tool_use',
      SPIDERBYTE_MODEL_DISPLAY_NAME: 'Custom Model',
      SPIDERBYTE_MODEL_REASONING_KEY: 'reasoning',
      SPIDERBYTE_MODEL_ADAPTIVE_THINKING: 'true',
      SPIDERBYTE_MODEL_TEMPERATURE: '0.3',
      SPIDERBYTE_MODEL_TOP_P: ' 0.95 ',
      SPIDERBYTE_MODEL_THINKING_KEEP: 'all',
      SPIDERBYTE_MODEL_MAX_COMPLETION_TOKENS: '4096',
  ...
  it.each([
    ['SPIDERBYTE_MODEL_MAX_CONTEXT_SIZE', '0'],
    ['SPIDERBYTE_MODEL_MAX_CONTEXT_SIZE', '1.5'],
    ['SPIDERBYTE_MODEL_MAX_OUTPUT_SIZE', 'nope'],
    ['SPIDERBYTE_MODEL_ADAPTIVE_THINKING', 'maybe'],
    ['SPIDERBYTE_MODEL_TEMPERATURE', 'abc'],
    ['SPIDERBYTE_MODEL_TEMPERATURE', '1.2.3'],
  ...
  it('strips env-only model values before write-back', () => {
    expect(
      spiderbyteModelEnvOverlay.strip?.(
        'models',
        {
    ...
    expect(
      spiderbyteModelEnvOverlay.strip?.('defaultModel', ENV_MODEL_ALIAS_KEY, {
        default_model: 'user',
      }),
    ).toBe('user');
    expect(spiderbyteModelEnvOverlay.strip?.('modelOverrides', { temperature: 0.3 }, {})).toBeUndefined();
  });

  it('self-registers into ConfigRegistry without ModelService instantiation', () => {
    const freshRegistry = new ConfigRegistry();
    expect(freshRegistry.listEffectiveOverlays()).toContain(spiderbyteModelEnvOverlay);
  });
});
