...
async function createHost(
  ...
  const host = createScopedTestHost([
    ...
    [
      IBootstrapService,
      stubBootstrap('/tmp/spiderbyte-home', {}, { requestHeaders: { 'User-Agent': 'spiderbyte-test/1.0' } }),
    ],
    [
      IAgentIdentity,
      stubAgentIdentity({ hostRequestHeaders: { 'User-Agent': 'spiderbyte-test/1.0' } }),
    ],
  ]);
        ...
        return {
          ...
          json: async () => ({
            acme: {
              ...
              api: 'https://acme.example.test/v1',
              type: 'openai',
              models: { 'acme-model': { id: 'acme-model', name: 'Acme Model' } },
            },
          }),
        };
      });
    ...
    try {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          headers: expect.objectContaining({ 'User-Agent': 'spiderbyte-test/1.0' }),
        }),
      );
    } finally {
      host.dispose();
