...
describe('AppLogService (scoped)', () => {
  ...
  function buildHost(cfg = resolveLoggingConfig({ homeDir, env: { SPIDERBYTE_LOG_LEVEL: 'info' } })) {
  ...
  it('reads its level from ILogOptions', async () => {
    const host = buildHost(resolveLoggingConfig({ homeDir, env: { SPIDERBYTE_LOG_LEVEL: 'debug' } }));
    const log = host.app.accessor.get(ILogService);
