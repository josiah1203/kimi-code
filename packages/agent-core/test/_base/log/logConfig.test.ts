...
describe('resolveLoggingConfig', () => {
  it('uses defaults when env is empty', () => {
    const cfg = resolveLoggingConfig({ homeDir: '/home/spiderbyte', env: {} });
    expect(cfg.level).toBe(DEFAULT_LOG_LEVEL);
    expect(cfg.globalLogPath).toBe('/home/spiderbyte/logs/spiderbyte.log');
  ...
  it('reads level and sizes from env', () => {
    const cfg = resolveLoggingConfig({
      homeDir: '/h',
      env: {
        SPIDERBYTE_LOG_LEVEL: 'debug',
        SPIDERBYTE_LOG_GLOBAL_MAX_BYTES: '1024',
        SPIDERBYTE_LOG_GLOBAL_FILES: '7',
        SPIDERBYTE_LOG_SESSION_MAX_BYTES: '2048',
        SPIDERBYTE_LOG_SESSION_FILES: '4',
      },
    });
  ...
  it('ignores invalid level and non-positive sizes', () => {
    const cfg = resolveLoggingConfig({
      homeDir: '/h',
      env: {
        SPIDERBYTE_LOG_LEVEL: 'verbose',
        SPIDERBYTE_LOG_GLOBAL_MAX_BYTES: '-5',
        SPIDERBYTE_LOG_GLOBAL_FILES: 'abc',
      },
    });
    expect(cfg.level).toBe(DEFAULT_LOG_LEVEL);
    expect(cfg.globalMaxBytes).toBe(DEFAULT_GLOBAL_MAX_BYTES);
  ...
  it('resolves the log path regardless of env', () => {
    ...
    expect(cfg.globalLogPath).toBe('/h/logs/spiderbyte.log');
  });
});

describe('path resolution', () => {
  it('resolves the global log path under homeDir/logs', () => {
    expect(resolveGlobalLogPath('/home/spiderbyte')).toBe('/home/spiderbyte/logs/spiderbyte.log');
  });

  it('resolves the session log path under sessionDir/logs', () => {
    expect(resolveSessionLogPath('/sessions/s1')).toBe('/sessions/s1/logs/spiderbyte.log');
  });
});

describe('logSeed', () => {
  it('seeds ILogOptions into a App scope', () => {
    const cfg = resolveLoggingConfig({ homeDir: '/h', env: { SPIDERBYTE_LOG_LEVEL: 'warn' } });
    const host = createScopedTestHost(logSeed(cfg));
    const opts = host.app.accessor.get(ILogOptions);
    expect(opts.level).toBe('warn');
    expect(opts.globalLogPath).toBe('/h/logs/spiderbyte.log');
    host.dispose();
  });
});
