...
describe('Workspace MCP initialization', () => {
  ...
  beforeEach(() => {
    cwd = mkdtempSync(join(tmpdir(), 'spiderbyte-session-mcp-cwd-'));
    homeDir = mkdtempSync(join(tmpdir(), 'spiderbyte-session-mcp-home-'));
  ...
  it('times out tool calls using the session MCP timeout preference', async () => {
    await writeProjectMcpJson(cwd, {
      slowTool: {
        ...
        env: { SPIDERBYTE_TEST_MCP_TOOL_DELAY_MS: '300' },
      },
    });
...
async function writeProjectMcpJson(
  ...
  await mkdir(join(cwd, '.spiderbyte'), { recursive: true });
  await writeFile(
    join(cwd, '.spiderbyte', 'mcp.json'),
