 ...
 * - a persisted `addDir` writes `.spiderbyte/local.toml` and refreshes every
 ...
 * Run:
 * `pnpm --filter @spiderbyte/agent-core exec vitest run
...
describe('workspace add-dir (handler chain)', () => {
  ...
  async function makeProjectRoot(): Promise<string> {
    const root = await makeRoot('spiderbyte-add-dir-proj-');
  ...
  it('persists addDir to local.toml and every session of the workspace sees the dir', async () => {
    const homeDir = await makeRoot('spiderbyte-add-dir-home-');
    const root = await makeProjectRoot();
    const extra = await makeRoot('spiderbyte-add-dir-extra-');
    ...
    expect(result.configPath).toBe(join(root, '.spiderbyte', 'local.toml'));
    expect(result.additionalDirs).toEqual([extra]);
    const toml = await readFile(join(root, '.spiderbyte', 'local.toml'), 'utf8');
  ...
  it('persist: false stays in handler memory, never touches the disk, and is shared', async () => {
    ...
    expect(result.additionalDirs).toEqual([extra]);
    expect(dirsOf(s1)).toEqual([extra]);
    await expect(readFile(join(root, '.spiderbyte', 'local.toml'), 'utf8')).rejects.toThrow();
    const s2 = await service.create({ sessionId: 's2', workDir: root });
    expect(dirsOf(s2)).toEqual([extra]);
  });
  ...
  it('refreshes live session views when another process edits local.toml', async () => {
    ...
    await mkdir(join(root, '.spiderbyte'), { recursive: true });
    const writeLocalToml = () =>
      writeFile(join(root, '.spiderbyte', 'local.toml'), `[workspace]\nadditional_dir = ["${extra}"]\n`);
  ...
  it('registers the additional-directory sets into the workspace state container', async () => {
    const homeDir = await makeRoot('spiderbyte-add-dir-home-');
    const root = await makeProjectRoot();
    const persisted = await makeRoot('spiderbyte-add-dir-persisted-');
    const ephemeral = await makeRoot('spiderbyte-add-dir-ephemeral-');
    const host = buildHost(homeDir);
    const handler = await host.app.accessor.get(IWorkspaceLifecycleService).handlerFor({ root });
