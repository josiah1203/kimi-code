...
describe('agentRoots', () => {
  ...
  describe('projectRoots', () => {
    it('resolves the brand .spiderbyte/agents directory at the .git root', async () => {
      await markGitRoot();
      await mkdir(join(root, '.spiderbyte/agents'), { recursive: true });

      const roots = await projectAgentRoots(hostFs, root);

      expect(
        roots.some((r) => r.path.endsWith('.spiderbyte/agents') && r.source === 'project'),
      ).toBe(true);
    });
    ...
    it('falls back to the generic .agents/agents directory', async () => {
      ...
      expect(roots.some((r) => r.path.endsWith('.spiderbyte/agents'))).toBe(false);
    });
    ...
    it('walks up from a child directory to the .git root', async () => {
      ...
      expect(roots.some((r) => r.path.endsWith('.spiderbyte/agents'))).toBe(true);
    });
    ...
    it('orders the brand directory before the generic directory', async () => {
      ...
      const roots = await projectAgentRoots(hostFs, root);
      const brandIdx = roots.findIndex((r) => r.path.endsWith('.spiderbyte/agents'));
