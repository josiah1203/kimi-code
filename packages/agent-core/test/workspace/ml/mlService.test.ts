...
import type { Artifact, ArtifactCreateInput, Dataset, PolicyEvaluateInput } from '@spiderbyte/protocol';

const context = {
  _serviceBrand: undefined,
  workspaceId: 'wd_workspace_ml_0123456789ab',
  cwd: '/tmp/workspace-ml',
  source: 'local' as const,
  meta: {
    id: 'wd_workspace_ml_0123456789ab',
    root: '/tmp/workspace-ml',
    name: 'ml-test',
    createdAt: Date.now(),
    lastOpenedAt: Date.now(),
  },
  persistenceScope: 'workspaces/wd_workspace_ml_0123456789ab',
  osBackendId: 'local',
...
describe('WorkspaceMlService', () => {
  ...
  it('dispatches configured local Python training and persists redacted outputs', async () => {
    const previousCommand = process.env['SPIDERBYTE_ML_TRAIN_COMMAND'];
    const previousArgs = process.env['SPIDERBYTE_ML_TRAIN_ARGS'];
    process.env['SPIDERBYTE_ML_TRAIN_COMMAND'] = 'fake-ml-trainer';
    delete process.env['SPIDERBYTE_ML_TRAIN_ARGS'];
    let request = '';
    ...
    try {
      ...
      if (previousCommand === undefined) delete process.env['SPIDERBYTE_ML_TRAIN_COMMAND'];
      else process.env['SPIDERBYTE_ML_TRAIN_COMMAND'] = previousCommand;
      if (previousArgs === undefined) delete process.env['SPIDERBYTE_ML_TRAIN_ARGS'];
      else process.env['SPIDERBYTE_ML_TRAIN_ARGS'] = previousArgs;
    }
  });

  it('cancels an in-flight configured local trainer and makes the experiment retryable', async () => {
    const previousCommand = process.env['SPIDERBYTE_ML_TRAIN_COMMAND'];
    process.env['SPIDERBYTE_ML_TRAIN_COMMAND'] = 'blocking-ml-trainer';
    let started = false;
    try {
      const localTrainer: ISessionProcessRunner = {
        _serviceBrand: undefined,
