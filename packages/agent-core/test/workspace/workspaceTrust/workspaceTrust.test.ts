...
describe('WorkspaceTrustService', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'spiderbyte-workspace-trust-home-'));
    cwd = mkdtempSync(join(tmpdir(), 'spiderbyte-workspace-trust-cwd-'));
  ...
  it('tracks different roots independently', async () => {
    const other = mkdtempSync(join(tmpdir(), 'spiderbyte-workspace-trust-other-'));
    try {
      const { service: first } = createService(cwd);
      await first.ready;
      await first.trust();
