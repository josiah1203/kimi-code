...
describe('WorkspaceInstructionsService', () => {
  ...
  beforeEach(() => {
    workDir = mkdtempSync(join(tmpdir(), 'spiderbyte-instructions-work-'));
    osHomeDir = mkdtempSync(join(tmpdir(), 'spiderbyte-instructions-os-'));
    brandHomeDir = mkdtempSync(join(tmpdir(), 'spiderbyte-instructions-brand-'));
