...
describe('workspace resource sharing (handler chain)', () => {
    ...
    host = createScopedTestHost([
      ...
      stubPair(IProjectLocalConfigService, {
        ...
        readAdditionalDirs: (workDir: string) =>
          Promise.resolve({
            projectRoot: workDir,
            configPath: `${workDir}/.spiderbyte/local.toml`,
            additionalDirs: [],
          }),
  ...
  it('runs one shared MCP manager and one initial connect for two concurrent sessions', async () => {
    const root = await makeRoot('example-ws-mcp-');
    await mkdir(join(root, '.spiderbyte'), { recursive: true });
    await writeFile(
      join(root, '.spiderbyte', 'mcp.json'),
      JSON.stringify({
        mcpServers: {
    ...
    const home = await makeRoot('example-ws-mcp-home-');
  ...
  it('does not rescan skills when a second session of the workspace lists them', async () => {
    const root = await makeRoot('example-ws-skill-');
    await mkdir(join(root, '.spiderbyte', 'skills'), { recursive: true });
    const home = await makeRoot('example-ws-skill-home-');
  ...
  it('refreshes a live session skill list when .agents/skills changes on disk', async () => {
    const root = await makeRoot('example-ws-watch-');
    const home = await makeRoot('example-ws-watch-home-');
