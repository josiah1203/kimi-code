...
describe('WorkspaceMcpConfigService', () => {
  ...
  beforeEach(() => {
    cwd = mkdtempSync(join(tmpdir(), 'spiderbyte-workspace-mcp-config-cwd-'));
    homeDir = mkdtempSync(join(tmpdir(), 'spiderbyte-workspace-mcp-config-home-'));
