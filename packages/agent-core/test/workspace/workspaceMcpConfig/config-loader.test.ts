 ...
 * --filter @spiderbyte/agent-core exec vitest run
...
function makeTempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), 'spiderbyte-mcp-loader-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('resolveMcpJsonPaths', () => {
  it('returns the canonical user, project-root, and project-local paths', async () => {
    ...
    const paths = await resolveMcpJsonPaths({ fs, cwd, homeDir: '/home/user/.spiderbyte' });

    expect(paths.user).toBe('/home/user/.spiderbyte/mcp.json');
    expect(paths.projectRoot).toBe(join(repoRoot, '.mcp.json'));
    expect(paths.project).toBe(join(cwd, '.spiderbyte', 'mcp.json'));
  });
});
...
describe('loadMcpServers', () => {
  ...
  it('merges project-local mcp.json with user-global, project overriding on conflict', async () => {
    ...
    await writeJson(join(cwd, '.spiderbyte', 'mcp.json'), {
      mcpServers: {
  ...
  it('honors SPIDERBYTE_HOME env var when homeDir is not supplied', async () => {
    const home = makeTempDir();
    const cwd = makeTempDir();
    await writeJson(join(home, 'mcp.json'), {
      mcpServers: { from_env: { transport: 'stdio', command: 'env-cmd' } },
    });
    const saved = process.env['SPIDERBYTE_HOME'];
    process.env['SPIDERBYTE_HOME'] = home;
    try {
      const servers = await loadMcpServers({ fs, cwd });
      expect(servers['from_env']).toEqual({ transport: 'stdio', command: 'env-cmd' });
    } finally {
      if (saved === undefined) delete process.env['SPIDERBYTE_HOME'];
      else process.env['SPIDERBYTE_HOME'] = saved;
    }
  });
});
