...
describe('findExistingRg', () => {
  ...
  beforeEach(() => {
    fakeShare = join(tmpdir(), `spiderbyte-rg-${String(Date.now())}-${String(Math.random()).slice(2)}`);
...
describe('verifyArchiveChecksum', () => {
  let fakeDir: string;
  beforeEach(() => {
    fakeDir = join(tmpdir(), `spiderbyte-rg-sha-${String(Date.now())}-${String(Math.random()).slice(2)}`);
...
describe('ensureRgPath download branch', () => {
  ...
  beforeEach(() => {
    fakeShare = join(
      tmpdir(),
      `spiderbyte-rg-dl-${String(Date.now())}-${String(Math.random()).slice(2)}`,
    );
    mkdirSync(join(fakeShare, 'bin'), { recursive: true });
...
describe('ensureRgPath Windows download branch', () => {
  ...
  beforeEach(() => {
    fakeShare = join(
      tmpdir(),
      `spiderbyte-rg-win-${String(Date.now())}-${String(Math.random()).slice(2)}`,
    );
    mkdirSync(join(fakeShare, 'bin'), { recursive: true });
    savedFetch = globalThis.fetch;
    savedPath = process.env['PATH'];
