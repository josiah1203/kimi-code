...
interface TestModelProviderOptions {
  ...
  readonly providerRequestHeaders?: Record<string, string>;
}
...
export interface TestAgentOptions {
  ...
  | undefined;
  readonly initialConfig?: Partial<TestConfig> | undefined;
...
export function configServices(readConfig: () => TestConfig): TestAgentServiceOverride {
...
export class AgentTestContext {
  ...
  constructor(overrides: readonly TestAgentServiceOverride[] = [], options: TestAgentOptions = {}) {
    ...
    this.testConfig = applyTestAgentOptionsToConfig(emptyConfig(), options);
      ...
      [
        (reg) => {
          ...
          reg.defineInstance(
            ...
            {
              level: 'off',
              globalLogPath: '/tmp/spiderbyte-agent-app-v2-test/logs/spiderbyte.log',
  ...
  configureRuntimeModel(
    ...
    this.testConfig = configWithProvider(this.testConfig, provider, modelCapabilities);
  ...
  async expectResumeMatches(): Promise<void> {
    ...
    await this.drainWirePersistence();
    const configSnapshot = structuredClone(this.get(IConfigService).getAll() as TestConfig);
...
function emptyConfig(): TestConfig {
  return configWithProvider({ providers: {} }, MOCK_PROVIDER, undefined);
}

function applyTestAgentOptionsToConfig(config: TestConfig, options: TestAgentOptions): TestConfig {
...
function configService(readConfig: () => TestConfig): IConfigService {
  const effectiveConfig = () => {
    ...
    return effective as unknown as TestConfig;
  };
...
function configWithEnvOverrides(config: TestConfig): TestConfig {
  const maxCompletionTokens =
    parseEnvCompletionTokens(process.env['SPIDERBYTE_MODEL_MAX_COMPLETION_TOKENS']) ??
    parseEnvCompletionTokens(process.env['SPIDERBYTE_MODEL_MAX_TOKENS']);
  const temperature = parseEnvFloat(process.env['SPIDERBYTE_MODEL_TEMPERATURE']);
  const topP = parseEnvFloat(process.env['SPIDERBYTE_MODEL_TOP_P']);
  const forcedEffort = process.env['SPIDERBYTE_MODEL_THINKING_EFFORT']?.trim();
  const thinkingKeep = process.env['SPIDERBYTE_MODEL_THINKING_KEEP']?.trim();
...
function cronEnvOverrides(base: Record<string, unknown>): Record<string, unknown> | undefined {
  ...
  setBoolean('debug', 'SPIDERBYTE_CRON_DEBUG');
  setBoolean('noJitter', 'SPIDERBYTE_CRON_NO_JITTER');
  setBoolean('noStale', 'SPIDERBYTE_CRON_NO_STALE');
  setBoolean('disabled', 'SPIDERBYTE_DISABLE_CRON');
  setBoolean('manualTick', 'SPIDERBYTE_CRON_MANUAL_TICK');
  const pollIntervalMs = parseEnvCronPollIntervalMs(process.env['SPIDERBYTE_CRON_POLL_INTERVAL_MS']);
  if (pollIntervalMs !== undefined) {
    next['pollIntervalMs'] = pollIntervalMs;
    changed = true;
  }
  if (process.env['SPIDERBYTE_CRON_CLOCK'] !== undefined) {
    next['clock'] = process.env['SPIDERBYTE_CRON_CLOCK'];
    changed = true;
  }
...
function providerConfigForAlias(provider: TestProviderConfig): TestConfig['providers'][string] {
  return {
    type: provider.type,
