...
describe('apiKey env suppression (probe 1)', () => {
  ...
  it('uses the SPIDERBYTE_API_KEY env fallback when composing kimi', () => {
    process.env['SPIDERBYTE_API_KEY'] = 'sk-kimi-from-env';
...
describe('resolveProviderEndpoint', () => {
  it('resolves the kimi endpoint chain from process.env', () => {
    process.env['SPIDERBYTE_API_KEY'] = 'sk-kimi-env';
  ...
  it('reads a caller-supplied env bag instead of process.env', () => {
    process.env['SPIDERBYTE_API_KEY'] = 'sk-kimi-env';
    expect(resolveProviderEndpoint('kimi', { SPIDERBYTE_BASE_URL: 'https://example.com/v1' })).toEqual({
...
describe('spyderbyte provider definitions', () => {
  it('registers one definition per transport, with shared vendor-level facts', () => {
    ...
    for (const definition of [native, anthropic]) {
      expect(definition?.endpoint).toEqual({
        apiKeyEnv: 'SPIDERBYTE_API_KEY',
        baseUrlEnv: 'SPIDERBYTE_BASE_URL',
