...
const HOST_HEADERS = { 'User-Agent': 'spiderbyte-test/1.0', 'X-Msh-Device-Id': 'device-1' };
...
beforeEach(() => {
  savedCustomHeaders = process.env['SPIDERBYTE_CUSTOM_HEADERS'];
  delete process.env['SPIDERBYTE_CUSTOM_HEADERS'];
});

afterEach(() => {
  if (savedCustomHeaders === undefined) delete process.env['SPIDERBYTE_CUSTOM_HEADERS'];
  else process.env['SPIDERBYTE_CUSTOM_HEADERS'] = savedCustomHeaders;
});
...
describe('Model assembly (pure data)', () => {
  it('assembles a kimi model: protocol resolves to the vendor base, never a vendor', () => {
    ...
    try {
      ...
      expect(model.headers).toMatchObject({
        'User-Agent': 'spiderbyte-test/1.0',
  ...
  it('forwards only the User-Agent to vendors without a full hostHeaders declaration', () => {
    ...
    try {
      ...
      expect(model.headers).toEqual({ 'User-Agent': 'spiderbyte-test/1.0' });
    } finally {
      host.dispose();
    }
  });
  ...
  describe('custom identity', () => {
    ...
    it('preserves a parenthesized User-Agent suffix while rewriting', () => {
      const { host, catalog } = createHost(THIRD_PARTY, stubModelOAuthTokens(), {
        headers: { 'User-Agent': 'spiderbyte-test/1.0 (web)' },
        identitySlug: 'acme-dev',
      });
      try {
    ...
    it('changes nothing when no identity is configured', () => {
      ...
      try {
        expect(catalog.get('gpt').headers).toEqual({ 'User-Agent': 'spiderbyte-test/1.0' });
      } finally {
        host.dispose();
      }
    });
    ...
    it('attributes the User-Agent provenance for a lowercase host spelling', () => {
      const { host, catalog } = createHost(THIRD_PARTY, stubModelOAuthTokens(), {
        headers: { 'user-agent': 'spiderbyte-test/1.0' },
        identitySlug: 'acme-dev',
      });
      try {
        expect(catalog.get('gpt').headers).toEqual({ 'User-Agent': 'acme-dev/1.0' });
  ...
  it('resolves provider env-bag credentials and endpoints through the registry', () => {
    const { host, catalog } = createHost({
      providers: {
        kimi: { type: 'kimi', env: { SPIDERBYTE_API_KEY: 'env-token', SPIDERBYTE_BASE_URL: 'https://kimi-env.example.test/v1' } },
  ...
  it('attributes env-bag credentials and endpoints by env-var name', () => {
    const { host, catalog } = createHost({
      providers: {
        kimi: {
          type: 'kimi',
          env: { SPIDERBYTE_API_KEY: 'env-token', SPIDERBYTE_BASE_URL: 'https://kimi-env.example.test/v1' },
        },
      },
    ...
    try {
      ...
      expect(view.sources['resolved.auth']).toMatchObject({
        kind: 'env',
        detail: expect.stringContaining('SPIDERBYTE_API_KEY'),
      });
      expect(view.sources['resolved.baseUrl']).toMatchObject({
        kind: 'env',
        detail: expect.stringContaining('SPIDERBYTE_BASE_URL'),
      });
...
describe('ModelCatalog enumeration', () => {
  ...
  it('detects env-bag credentials through the vendor endpoint declarations', async () => {
    const { host, catalog } = createHost({
      providers: {
        kimi: { type: 'kimi', env: { SPIDERBYTE_API_KEY: 'kimi-env-key' } },
