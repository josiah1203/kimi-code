...
describe('task tool descriptions', () => {
  ...
  it('exposes the canonical model-facing task names and schemas', () => {
    expect(new TaskListTool(tasks).name).toBe('TaskList');
    expect(new TaskOutputTool(tasks).name).toBe('TaskOutput');
    expect(new TaskStopTool(tasks).name).toBe('TaskStop');
  });
