...
beforeEach(async () => {
  ...
  homeDir = await mkdtemp(join(tmpdir(), 'spiderbyte-reminder-home-'));
  workDir = await mkdtemp(join(tmpdir(), 'spiderbyte-reminder-work-'));
...
describe('agentsMdReminder path-carrying tools', () => {
  ...
  it('discovers the .spiderbyte/AGENTS.md variant alongside the plain one', async () => {
    const h = createHarness();
    const subDir = join(workDir, 'packages', 'kap-server');
    const dotSpiderByte = normalize(join(subDir, '.spiderbyte', 'AGENTS.md'));
    await writeAgentsMd(join(subDir, '.spiderbyte'), 'dot spiderbyte instructions');
...
describe('agentsMdReminder probing boundaries', () => {
  ...
  it('probes only the immediate directory outside any project', async () => {
    const h = createHarness();
    const outside = await mkdtemp(join(tmpdir(), 'spiderbyte-reminder-outside-'));
  ...
  it('discovers a symlinked directory through the link at its lexical address', async () => {
    const h = createHarness();
    const target = await mkdtemp(join(tmpdir(), 'spiderbyte-reminder-target-'));
