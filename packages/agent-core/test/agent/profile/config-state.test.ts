...
type TestConfig = ReturnType<Parameters<typeof configServices>[0]>;
type TestProtocolModelConfig = NonNullable<TestConfig['models']>[string] &
...
describe('ConfigState model capabilities', () => {
  ...
  it('computes provider and model capabilities from config metadata', () => {
    ...
    profile.update({ modelAlias: 'spiderbyte/kimi-for-coding' });

    expect(profile.getModel()).toBe('spiderbyte/kimi-for-coding');
    expect(ctx.modelResolver.get('spiderbyte/kimi-for-coding').name).toBe('kimi-for-coding');
  ...
  it('does not infer Kimi capabilities from the provider catalogue', () => {
    ...
    profile.update({ modelAlias: 'spiderbyte' });
...
describe('ConfigState prompt cache hint', () => {
  ...
  it('uses session id as a provider prompt cache hint without storing it on Agent', () => {
    ...
    const model = ctx.modelResolver.get('spiderbyte');
...
describe('ConfigState thinking clamp for always-thinking models', () => {
  ...
  it('clamps thinkingLevel off to the configured effort', () => {
    profile.update({ modelAlias: 'spiderbyte/deep', thinkingLevel: 'off' });
  ...
  it('keeps thinking off working for toggleable models', () => {
    profile.update({ modelAlias: 'spiderbyte/toggle', thinkingLevel: 'off' });

    expect(profile.data().thinkingLevel).toBe('off');
  });

  it('resolves an explicit on request to the model default effort', () => {
    profile.update({ modelAlias: 'spiderbyte/custom', thinkingLevel: 'on' });
  ...
  it('re-clamps when switching to an always-on model after thinking was off', () => {
    ...
    profile.update({ modelAlias: 'spiderbyte/deep' });
    expect(profile.data().thinkingLevel).toBe('high');
  });

  it('falls back to the target default when a model switch carries an unsupported effort', () => {
    profile.update({ modelAlias: 'spiderbyte/ultra', thinkingLevel: 'ultra' });

    profile.update({ modelAlias: 'spiderbyte/custom' });
  ...
  it('projects an inherited concrete effort to on when switching to a boolean model', () => {
    ...
    profile.update({ modelAlias: 'spiderbyte/toggle' });
  ...
  it('rejects an unsupported effort explicitly set on the current Kimi model', () => {
    ...
    expect(() => {
      ...
      'Thinking effort "ultra" is not supported by model "spiderbyte/custom"',
    );
  });
  ...
  it.each([
    ...
    profile.update({ modelAlias: 'spiderbyte/ultra' });
  ...
  it('uses the model default when the runtime effort is blank', () => {
    profile.update({ modelAlias: 'spiderbyte/custom', thinkingLevel: 'low' });
  ...
  it('preserves unlisted efforts with a warning for Kimi-managed Anthropic models', () => {
    profile.update({ modelAlias: 'spiderbyte/compatible', thinkingLevel: 'max' });
...
describe('ConfigState.provider applies global SPIDERBYTE_MODEL_* request config', () => {
  ...
  beforeEach(() => {
    testConfig = {
      ...
      models: {
        ...
        'spiderbyte-anthropic': {
          provider: 'kimi',
          protocol: 'anthropic',
          model: 'spiderbyte-anthropic',
          maxContextSize: 128_000,
          capabilities: ['thinking'],
          supportEfforts: ['low', 'high'],
        } as TestProtocolModelConfig,
  ...
  it('injects SPIDERBYTE_MODEL_TEMPERATURE into the per-turn sampling intent (the compaction request also uses)', async () => {
    vi.stubEnv('SPIDERBYTE_MODEL_TEMPERATURE', '0.3');
    createAgentWithEnv();
  ...
  it('injects SPIDERBYTE_MODEL_THINKING_KEEP into the per-turn thinking intent when thinking is on (so compaction keeps it)', async () => {
    vi.stubEnv('SPIDERBYTE_MODEL_THINKING_KEEP', 'all');
    createAgentWithEnv();

    profile.update({ modelAlias: 'spiderbyte', thinkingLevel: 'high' });
    await requester.request({}, undefined, new AbortController().signal);
  ...
  it('does NOT inject thinking.keep into the per-turn intent when thinking is off', async () => {
    ...
    profile.update({ modelAlias: 'spiderbyte', thinkingLevel: 'off' });
    await requester.request({}, undefined, new AbortController().signal);
  ...
  it('injects forced effort through the Anthropic protocol for a Kimi provider', async () => {
    ...
    profile.update({ modelAlias: 'spiderbyte-anthropic', thinkingLevel: 'high' });
    expect(profile.data().thinkingLevel).toBe('high');
