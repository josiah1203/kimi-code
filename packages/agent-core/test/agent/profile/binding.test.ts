...
describe('AgentProfileService.bind', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'spiderbyte-bind-home-'));
  });
  ...
  it('binds a profile + model atomically and becomes runnable', async () => {
    ...
    expect(svc.getSystemPrompt()).toContain('SpiderByte CLI');
  });
  ...
  it('refreshes the system prompt from the session cwd after a default bind', async () => {
    const workDir = await mkdtemp(join(tmpdir(), 'spiderbyte-bind-work-'));
    try {
  ...
  it('rejects an unsupported thinking effort atomically before first bind', async () => {
    ...
    expect(svc.data().profileName).toBeUndefined();
    await svc.bind({ profile: DEFAULT_AGENT_PROFILE_NAME, model: 'spiderbyte/kimi-for-coding' });
    expect(svc.data().profileName).toBe(DEFAULT_AGENT_PROFILE_NAME);
  });
...
describe('AgentToolPolicyService tool denylist', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'spiderbyte-deny-home-'));
  });
...
describe('AgentToolPolicyService global [tools] config', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'spiderbyte-tools-config-home-'));
  });
...
describe('AgentToolPolicyService.setSessionDisabledTools', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'spiderbyte-session-deny-home-'));
  });
...
describe('AgentToolPolicyService executor enforcement', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'spiderbyte-executor-policy-home-'));
  });
...
describe('AgentProfileService tool-pattern warnings', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'spiderbyte-tool-pattern-home-'));
  });
...
describe('agentsMdReminder seeding', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'spiderbyte-seed-home-'));
    workDir = await mkdtemp(join(tmpdir(), 'spiderbyte-seed-work-'));
  });
