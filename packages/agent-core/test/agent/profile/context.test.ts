...
beforeEach(async () => {
  homeDir = await mkdtemp(join(tmpdir(), 'spiderbyte-agents-home-'));
  workDir = await mkdtemp(join(tmpdir(), 'spiderbyte-agents-work-'));
  extraDirs = [];
  fs = createFs();
});
...
describe('loadAgentsMd user-level discovery', () => {
  ...
  it('does not load the same file twice when the work dir is the home dir', async () => {
    await mkdir(join(homeDir, '.spiderbyte'), { recursive: true });
    await writeFile(join(homeDir, '.spiderbyte', 'AGENTS.md'), 'home branded', 'utf-8');
...
describe('loadAgentsMd symlinked files', () => {
  it('follows symlinks when loading user-level and project-level AGENTS.md', async () => {
    const targetDir = await mkdtemp(join(tmpdir(), 'spiderbyte-agents-target-'));
    ...
    await mkdir(join(homeDir, '.spiderbyte'), { recursive: true });
    await symlink(brandTarget, join(homeDir, '.spiderbyte', 'AGENTS.md'));
...
describe('loadAgentsMd unreadable paths', () => {
  it('warns when an instruction file exists but is a dangling symlink', async () => {
    const brandHome = await mkdtemp(join(tmpdir(), 'spiderbyte-agents-brand-'));
...
describe('loadAgentsMd brand home (SPIDERBYTE_HOME)', () => {
  let brandHome: string;

  beforeEach(async () => {
    brandHome = await mkdtemp(join(tmpdir(), 'spiderbyte-agents-brand-'));
  });
  ...
  it('ignores the real-home .spiderbyte/AGENTS.md when the brand home is elsewhere', async () => {
    await writeFile(join(brandHome, 'AGENTS.md'), 'brand wins', 'utf-8');
    await mkdir(join(homeDir, '.spiderbyte'), { recursive: true });
    await writeFile(join(homeDir, '.spiderbyte', 'AGENTS.md'), 'stale real-home brand', 'utf-8');
  ...
  it('falls back to the real-home .spiderbyte/AGENTS.md when no brand home is given', async () => {
    await mkdir(join(homeDir, '.spiderbyte'), { recursive: true });
    await writeFile(join(homeDir, '.spiderbyte', 'AGENTS.md'), 'fallback branded', 'utf-8');
...
describe('loadAgentsMd nested project hierarchy', () => {
  it('loads AGENTS.md from the project root down to the cwd in root→leaf order', async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), 'spiderbyte-agents-project-'));
...
describe('prepareSystemPromptContext additional directories', () => {
  it('includes additional directory listings without loading their AGENTS.md', async () => {
    const brandHome = await mkdtemp(join(tmpdir(), 'spiderbyte-agents-empty-brand-'));
    extraDirs.push(brandHome);
    const extraDir = await mkdtemp(join(tmpdir(), 'spiderbyte-agents-extra-'));
  ...
  it('loads user-level AGENTS.md once and skips additional directory AGENTS.md', async () => {
    const brandHome = await mkdtemp(join(tmpdir(), 'spiderbyte-agents-empty-brand-'));
    extraDirs.push(brandHome);
    const extraDirA = await mkdtemp(join(tmpdir(), 'spiderbyte-agents-extra-a-'));
    const extraDirB = await mkdtemp(join(tmpdir(), 'spiderbyte-agents-extra-b-'));
...
describe('loadAgentsMdDetailed discovered paths', () => {
  ...
  it('returns the normalized paths of every injected file in collection order', async () => {
    ...
    expect(result.paths).toEqual([
      normalize(join(homeDir, '.spiderbyte', 'AGENTS.md')),
      normalize(join(workDir, '.spiderbyte', 'AGENTS.md')),
