 ...
 * Run: `pnpm --filter @spiderbyte/agent-core exec vitest run test/agent/goal/goal.test.ts`.
 */
...
import { ErrorCodes, Error2, errorInfo, toSpiderByteErrorPayload } from '#/errors';
...
function endTurn(
  ...
  const error = result.error !== undefined ? toSpiderByteErrorPayload(result.error) : undefined;
  eventBus.publish({
    type: 'turn.ended',
    turnId: turn.id,
