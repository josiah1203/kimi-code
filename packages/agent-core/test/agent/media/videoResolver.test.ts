...
import { buildSpiderByteFileUrl, parseSpiderByteFileUrl } from '#/agent/media/spiderbyteFileUrl';
...
describe('spiderbyteFileUrl', () => {
  it('round-trips a file id and an escaped materialization path', () => {
    const url = buildSpiderByteFileUrl('file_1', '/a b/clip.mp4');
    expect(url).toBe(`spiderbyte-file://file_1?path=${encodeURIComponent('/a b/clip.mp4')}`);
    expect(parseSpiderByteFileUrl(url)).toEqual({ fileId: 'file_1', path: '/a b/clip.mp4' });
  });

  it('omits the query when no path is given', () => {
    expect(buildSpiderByteFileUrl('file_1')).toBe('spiderbyte-file://file_1');
    expect(parseSpiderByteFileUrl('spiderbyte-file://file_1')).toEqual({ fileId: 'file_1' });
  });

  it('returns undefined for any non-spiderbyte-file url', () => {
    expect(parseSpiderByteFileUrl('ms://prov-1')).toBeUndefined();
    expect(parseSpiderByteFileUrl('data:video/mp4;base64,AAAA')).toBeUndefined();
    expect(parseSpiderByteFileUrl('https://example.com/clip.mp4')).toBeUndefined();
  });
});

describe('AgentVideoResolverService', () => {
  it('uploads a spiderbyte-file video once and reuses the cached reference on later steps', async () => {
    ...
    const message = videoMessage(buildSpiderByteFileUrl(FILE_ID, FALLBACK_PATH));
  ...
  it('falls back to a path tag when the model cannot ingest video', async () => {
    ...
    const out = await new AgentVideoResolverService(
      ...
      telemetry,
      new AgentStateService(),
    ).resolve([videoMessage(buildSpiderByteFileUrl(FILE_ID, FALLBACK_PATH))], requester({ videoIn: false, uploadVideo: upload }));
  ...
  it('inlines base64 for a no-upload provider whose wire carries video', async () => {
    const out = await new AgentVideoResolverService(
      ...
      telemetry,
      new AgentStateService(),
    ).resolve([videoMessage(buildSpiderByteFileUrl(FILE_ID, FALLBACK_PATH))], requester({ protocol: 'anthropic', uploadVideo: undefined }));
  ...
  it('tags for a no-upload provider whose wire drops inline video (openai family)', async () => {
    const out = await new AgentVideoResolverService(
      ...
      telemetry,
      new AgentStateService(),
    ).resolve([videoMessage(buildSpiderByteFileUrl(FILE_ID, FALLBACK_PATH))], requester({ protocol: 'openai', uploadVideo: undefined }));
  ...
  it('rethrows an auth failure so it can drive credential refresh', async () => {
    ...
    await expect(
      resolver.resolve([videoMessage(buildSpiderByteFileUrl(FILE_ID, FALLBACK_PATH))], requester({ uploadVideo: upload })),
  ...
  it('tags when the bytes do not sniff as a video', async () => {
    ...
    const out = await new AgentVideoResolverService(
      ...
      telemetry,
      new AgentStateService(),
    ).resolve([videoMessage(buildSpiderByteFileUrl(FILE_ID, FALLBACK_PATH))], requester({ uploadVideo: upload }));
  ...
  it('tags a stale reference by its materialization path', async () => {
    const out = await new AgentVideoResolverService(fileService(new Map()), blobStore(), telemetry, new AgentStateService()).resolve(
      [videoMessage(buildSpiderByteFileUrl('missing', FALLBACK_PATH))],
  ...
  it('emits an unavailable placeholder when a stale reference has no fallback path', async () => {
    const out = await new AgentVideoResolverService(fileService(new Map()), blobStore(), telemetry, new AgentStateService()).resolve(
      [videoMessage(buildSpiderByteFileUrl('missing'))],
      requester({ uploadVideo: vi.fn() }),
    );
  ...
  it('leaves messages without a spiderbyte-file video untouched', async () => {
    const resolver = new AgentVideoResolverService(
      fileService(new Map([[FILE_ID, { name: 'clip.mp4', bytes: VIDEO_BYTES }]])),
      blobStore(),
      telemetry,
