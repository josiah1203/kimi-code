...
describe('resolveSwarmMaxConcurrency', () => {
  ...
  it('returns undefined for empty or whitespace-only values', () => {
    expect(
      resolveSwarmMaxConcurrency({ SPIDERBYTE_AGENT_SWARM_MAX_CONCURRENCY: '' }),
    ).toBeUndefined();
    expect(
      resolveSwarmMaxConcurrency({ SPIDERBYTE_AGENT_SWARM_MAX_CONCURRENCY: '   ' }),
    ).toBeUndefined();
  ...
  it('throws for non-positive, non-integer, or non-numeric values', () => {
    for (const raw of ['0', '-1', '2.5', 'abc']) {
      expect(() =>
        resolveSwarmMaxConcurrency({ SPIDERBYTE_AGENT_SWARM_MAX_CONCURRENCY: raw }),
      ).toThrow(/SPIDERBYTE_AGENT_SWARM_MAX_CONCURRENCY.*positive integer/);
    }
  });

  it('returns the integer for a positive integer value', () => {
    expect(resolveSwarmMaxConcurrency({ SPIDERBYTE_AGENT_SWARM_MAX_CONCURRENCY: '3' })).toBe(3);
    expect(resolveSwarmMaxConcurrency({ SPIDERBYTE_AGENT_SWARM_MAX_CONCURRENCY: ' 8 ' })).toBe(8);
  });
});
...
describe('SessionSwarmService metadata compatibility', () => {
    ...
    createAgent.mockImplementationOnce((opts: CreateAgentOptions = {}) => {
      ...
      const handle = agentHandle(
        ...
        eventBus,
        {
          ...
          modelAlias: opts.binding?.model ?? 'example-test',
  ...
  it('points at the secondary model config when a spawn task binding is invalid', async () => {
    ...
    await expect(
      ...
      {
        status: 'failed',
        error: expect.stringContaining('comes from [secondary_model].model / SPIDERBYTE_SECONDARY_MODEL'),
      },
    ]);
