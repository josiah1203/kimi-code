...
//
// "Without minidb" search baseline over the SAME real ~/.spiderbyte sessions
// used by import-spiderbyte.ts. Loads every session's extracted text into an
...
// range). Paired with import-spiderbyte.ts, this shows what minidb's indexes
// buy you on real data.
//
// Run:  node --import tsx bench/search-baseline.ts [--data ~/.spiderbyte]
...
const FULL = argv.includes('--full'); // also index full tool results (matches import-spiderbyte --full)
...
async function main() {
  ...
  {
    const { value: res, ms } = med(() => docs.filter((d) => d.workspaceName === 'spiderbyte-dev-1'));
    console.log(`  naive lookup (workspace=spiderbyte-dev-1) -> ${res.length} in ${ms.toFixed(2)} ms`);
  }
