...
describe('BrowserPlatformClient', () => {
  it('uses authenticated REST projections without returning secret material', async () => {
    ...
    const client = new BrowserPlatformClient({
      baseUrl: 'https://kimi.example.test',
    ...
    expect(calls[0]?.url).toBe(`https://kimi.example.test/api/v2/workspaces/${workspaceId}/platform/connections`);
  ...
  it('recovers a WebSocket cursor gap through replay and suppresses duplicates', async () => {
    ...
    expect(protocols).toEqual(['kimi-code.bearer.opaque-ws-token']);
    subscription.dispose();
  });

  it('reads transcript snapshots and op catch-up through the existing Kimi REST surface', async () => {
    const fetch: BrowserFetch = async (url) => {
    ...
    const client = new BrowserPlatformClient({ baseUrl: 'https://kimi.example.test', fetch });
