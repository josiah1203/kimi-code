...
export interface HostCheckOptions {
  ...
  /** Extra allowed hosts / domain-suffix patterns (from `SPIDERBYTE_ALLOWED_HOSTS`). */
  readonly extra?: readonly string[];
  /** Disable the check entirely (`SPIDERBYTE_DISABLE_HOST_CHECK=1`; test-only). */
...
/**
 * Parse `SPIDERBYTE_ALLOWED_HOSTS` into an `extra` allowlist.
 *
...
export function parseAllowedHosts(env: NodeJS.ProcessEnv = process.env): string[] {
  const raw = env['SPIDERBYTE_ALLOWED_HOSTS'];
  if (raw === undefined) {
    return [];
  }
  return raw
...
/** True when `SPIDERBYTE_DISABLE_HOST_CHECK=1` (test/controlled env only). */
export function isHostCheckDisabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env['SPIDERBYTE_DISABLE_HOST_CHECK'] === '1';
}
...
export function formatHostErrorMessage(host: string | undefined): string {
  ...
  return `Invalid Host header: ${hostLabel}; allow this host with SPIDERBYTE_ALLOWED_HOSTS=${hostArg} or 'spyderbyte web --allowed-host ${hostArg}'.`;
}
