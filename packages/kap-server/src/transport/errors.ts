...
import { ErrorCodes, Error2 } from '@spiderbyte/agent-core';
...
const SPIDERBYTE_TO_PROTOCOL: Record<string, ErrorCode> = {
...
export function mapError(err: unknown, requestId: string): ReturnType<typeof errEnvelope> {
  if (err instanceof Error2) {
    const code = SPIDERBYTE_TO_PROTOCOL[err.code] ?? ErrorCode.INTERNAL_ERROR;
