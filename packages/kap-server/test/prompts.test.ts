...
describe('server-v2 /api/v1 prompts', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'spiderbyte-server-prompts-'));
  ...
  it('carries an uploaded video into the prompt as an internal spiderbyte-file reference', async () => {
    const id = await createSession(home as string);
    await createMainAgent(id);
    ...
    // is only ever produced from an internal `spiderbyte-file://` url, so this proves
  ...
  it('binds a discovered custom agent profile on the first prompt', async () => {
    // A user-level agent file under $SPIDERBYTE_HOME/agents is discovered into
