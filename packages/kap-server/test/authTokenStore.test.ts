...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'spiderbyte-server-auth-token-'));
});
...
describe('password', () => {
  it('resolvePasswordHash returns undefined when env is unset or empty', async () => {
    ...
    expect(await resolvePasswordHash({ SPIDERBYTE_PASSWORD: '' })).toBeUndefined();
  });

  it('hashes a set password with bcrypt and verifies correctly', async () => {
    const passwordHash = await resolvePasswordHash({
      SPIDERBYTE_PASSWORD: 'correct-horse-battery-staple',
    });
