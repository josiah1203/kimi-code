 ...
 * `spiderbyte.bearer.<token>` subprotocol.
 */
...
describe('WS upgrade auth', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'spiderbyte-server-ws-upgrade-auth-'));
  ...
  describe('/api/v1/ws', () => {
    ...
    it('accepts a valid bearer subprotocol and echoes it', async () => {
      const { ws, firstFrame } = await openConn(url(), {
        protocols: [`spiderbyte.bearer.${TOKEN}`],
      });
      sockets.push(ws);
      expect(ws.protocol).toBe(`spiderbyte.bearer.${TOKEN}`);
    ...
    it('rejects a wrong bearer token', async () => {
      await expectRejected(url(), { protocols: ['spiderbyte.bearer.wrong'] });
    });
  ...
  it('rejects upgrades to a non-WS path', async () => {
    ...
    await expectRejected(badUrl, { protocols: [`spiderbyte.bearer.${TOKEN}`] });
  });
});
