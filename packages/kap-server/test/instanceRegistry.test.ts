 ...
 * `~/.spiderbyte/server/instances` is never touched. Dead-pid simulation uses
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'spiderbyte-instance-registry-test-'));
...
describe('startServer — instance registry wiring', () => {
  ...
  it('lets two servers share one homeDir, each registering a distinct instance and port', async () => {
    home = mkdtempSync(join(tmpdir(), 'spiderbyte-server-multi-server-'));
