 ...
 *   - Windows resolves Git Bash via `SPIDERBYTE_SHELL_PATH`, `git.exe` on PATH
...
describe('detectEnvironment', () => {
  ...
  it('uses SPIDERBYTE_SHELL_PATH override when set and the file exists', async () => {
    const env = await detectEnvironment(
      stubDeps({
        platform: 'win32',
        env: { SPIDERBYTE_SHELL_PATH: 'D:\\custom\\bash.exe' },
