...
import type { ArtifactCreateInput } from '@moonshot-ai/protocol';

const context: WorkspaceContext = {
  _serviceBrand: undefined,
  workspaceId: 'wd_kimi_dataset_0123456789ab',
  cwd: '/tmp/kimi-dataset',
  source: 'local',
  meta: {
    id: 'wd_kimi_dataset_0123456789ab',
    root: '/tmp/kimi-dataset',
    name: 'dataset-test',
    createdAt: Date.now(),
    lastOpenedAt: Date.now(),
  },
  persistenceScope: 'workspaces/wd_kimi_dataset_0123456789ab',
  osBackendId: 'local',
...
describe('WorkspaceDatasetService', () => {
  ...
  beforeEach(() => {
    ...
    ix.stub(ISessionProcessRunner, {
      ...
      exec: async (_command: readonly string[], options?: ProcessExecOptions) => {
        ...
        const jsonl = options?.env?.['KIMI_DATASET_FORMAT'] === 'jsonl';
  ...
  it('registers versions, profiles data, persists metadata, and returns SQL artifacts', async () => {
    ...
    const stored = await ix
      ...
      .get<unknown>('workspaces/wd_kimi_dataset_0123456789ab/platform', 'datasets.json');
