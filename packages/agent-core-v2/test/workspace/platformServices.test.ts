...
const context: WorkspaceContext = {
  ...
  cwd: '/tmp/kimi-platform',
  source: 'local',
  meta: {
    id: 'wd_platform_services_0123456789ab',
    root: '/tmp/kimi-platform',
    name: 'platform',
    createdAt: Date.now(),
...
describe('workspace platform services', () => {
  ...
  it('leases execution targets, schedules approval-gated automations, and exposes commercial meters', async () => {
    ...
    const commercial = ix.get(IWorkspaceCommercialService);
    await expect(
      commercial.upsertMember({
        request_id: 'member_owner',
    ...
    await expect(
      commercial.recordUsage({
        request_id: 'usage_intelligence',
        actor_id: 'member_owner',
        run_id: 'run_target',
        meter: 'intelligence',
    ...
    ).resolves.toMatchObject({ meter: 'intelligence' });
