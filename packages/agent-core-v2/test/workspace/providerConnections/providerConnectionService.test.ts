...
import { PLATFORM_NO_CREDENTIAL_SECRET_REF } from '@moonshot-ai/protocol';

const context: WorkspaceContext = {
  _serviceBrand: undefined,
  workspaceId: 'wd_kimi_0123456789ab',
  cwd: '/tmp/kimi',
  source: 'local',
  meta: {
    id: 'wd_kimi_0123456789ab',
    root: '/tmp/kimi',
    name: 'kimi',
    createdAt: Date.now(),
    lastOpenedAt: Date.now(),
  },
  persistenceScope: 'workspaces/wd_kimi_0123456789ab',
  osBackendId: 'local',
...
describe('WorkspaceProviderConnectionService', () => {
  ...
  it('persists reference-only connections, validates, activates, discovers, and revokes', async () => {
    ...
    const stored = await ix
      ...
      .get<unknown>('workspaces/wd_kimi_0123456789ab/platform', 'provider-connections.json');
