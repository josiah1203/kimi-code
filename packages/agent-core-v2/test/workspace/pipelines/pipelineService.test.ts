...
import type { ExecutionTarget, PolicyEvaluateInput } from '@moonshot-ai/protocol';

const context = {
  _serviceBrand: undefined,
  workspaceId: 'wd_kimi_pipeline_0123456789ab',
  cwd: '/tmp/kimi-pipeline',
  source: 'local' as const,
  meta: { id: 'wd_kimi_pipeline_0123456789ab', root: '/tmp/kimi-pipeline', name: 'pipeline-test', createdAt: Date.now(), lastOpenedAt: Date.now() },
  persistenceScope: 'workspaces/wd_kimi_pipeline_0123456789ab',
  osBackendId: 'local',
...
describe('WorkspacePipelineService', () => {
  ...
  it('propagates completed dependency artifacts to a downstream remote step', async () => {
    executionTarget = {
      id: 'target_customer_cloud_dependencies',
      workspace_id: context.workspaceId,
  ...
  it('cancels an in-flight remote step and leaves the pipeline ready to run again', async () => {
    executionTarget = {
      id: 'target_customer_cloud_cancel',
      workspace_id: context.workspaceId,
