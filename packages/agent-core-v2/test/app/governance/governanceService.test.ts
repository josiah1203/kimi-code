...
describe('PlatformGovernanceService', () => {
  ...
  it('does not represent hosted tenancy without the hosted identity adapter', async () => {
    const service = ix.get(IPlatformGovernanceService);

    await expect(service.createOrganization({
      request_id: 'hosted_organization_create',
      actor_id: 'user_owner',
      name: 'Hosted test',
      mode: 'hosted',
    })).rejects.toMatchObject({ code: 'governance.hosted_identity_required' });
  });
