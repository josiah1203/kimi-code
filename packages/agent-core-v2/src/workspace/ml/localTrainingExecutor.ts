 ...
 * `KIMI_CODE_ML_TRAIN_COMMAND`. The command is invoked directly (never through
 ...
 * durable Run and artifact records remain owned by Kimi.
 */

import type { Readable } from 'node:stream';

import type { DatasetFormat, Experiment } from '@moonshot-ai/protocol';
...
export function configuredLocalTrainingExecutor(): LocalTrainingExecutorConfig | undefined {
  const command = process.env['KIMI_CODE_ML_TRAIN_COMMAND']?.trim();
  if (command === undefined || command.length === 0) return undefined;

  const args = parseArgs(process.env['KIMI_CODE_ML_TRAIN_ARGS']);
  const timeout_ms = parseTimeout(process.env['KIMI_CODE_ML_TRAIN_TIMEOUT_MS']);
...
function parseArgs(raw: string | undefined): readonly string[] {
  ...
  try {
    ...
    if (!Array.isArray(parsed) || parsed.some((value) => typeof value !== 'string')) {
      throw new Error('KIMI_CODE_ML_TRAIN_ARGS must be a JSON array of strings');
    }
    return parsed;
  } catch (error) {
...
function parseTimeout(raw: string | undefined): number {
  ...
  if (!Number.isInteger(value) || value < 1 || value > MAX_TIMEOUT_MS) {
    throw new Error(`KIMI_CODE_ML_TRAIN_TIMEOUT_MS must be an integer between 1 and ${MAX_TIMEOUT_MS}`);
  }
  return value;
}
