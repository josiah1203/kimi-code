...
async function runSqliteQuery(
  ...
  try {
    process = await runner.exec(['python3', '-c', script], {
      env: {
        KIMI_DATASET_QUERY: Buffer.from(sql, 'utf8').toString('base64'),
        KIMI_DATASET_FORMAT: format,
      },
    });
  } catch (error) {
    throw new DatasetServiceError(
