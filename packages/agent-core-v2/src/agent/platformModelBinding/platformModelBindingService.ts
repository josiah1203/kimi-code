 ...
 * Kimi's agent-level model requester contract and owns the active selection.
...
import type { PlatformModelSelection as PlatformModelSelectionProjection } from '@moonshot-ai/protocol';
...
class PlatformModelRequester implements ModelRequester {
  ...
  private async requestProviderApproval(
    ...
    const response = await approvals.request({
      ...
      display: {
        kind: 'generic',
        summary: `Use ${this.providerName} / ${this.modelName} for this Kimi request`,
