 ...
 * Run: pnpm exec vitest run packages/sdk/test/session-context.test.ts
 */
...
describe('Session context', () => {
  it('restores a session-only additional directory after close and resume', async () => {
    const homeDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-additional-home-');
    const workDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-additional-work-');
    const additionalDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-additional-dir-');
  ...
  it('clears context without replacing the session', async () => {
    const homeDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-context-home-');
    const workDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-context-work-');
  ...
  it('appends old-compatible user context markup when importing raw content', async () => {
    const homeDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-context-import-home-');
    const workDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-context-import-work-');
    await writeTestConfig(homeDir, 200_000);
    const harness = createSpiderByteHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('emits the estimated context token status after an import', async () => {
    const homeDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-context-status-home-');
    const workDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-context-status-work-');
    await writeTestConfig(homeDir, 200_000);
    const harness = createSpiderByteHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('restores the imported message with its estimated token count after resume', async () => {
    const homeDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-context-resume-home-');
    const workDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-context-resume-work-');
    await writeTestConfig(homeDir, 200_000);
    const harness = createSpiderByteHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('rejects whitespace-only imported content without mutating context', async () => {
    const homeDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-context-empty-home-');
    const workDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-context-empty-work-');
    const harness = createSpiderByteHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('rejects an import that would push existing context beyond the model limit', async () => {
    const homeDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-context-overflow-home-');
    const workDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-context-overflow-work-');
