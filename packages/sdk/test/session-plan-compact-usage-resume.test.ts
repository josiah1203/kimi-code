...
import { createSpiderByteHarness, type Event, type SpiderByteError } from '#/index';
...
describe('Session plan, compact, usage, and resume APIs', () => {
  it('sets plan mode through manualEnterPlan and clears the active plan file', async () => {
    const homeDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-plan-home-');
    const workDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-plan-work-');
  ...
  it('prepares the plans directory without creating plan files on repeated toggles', async () => {
    const homeDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-plan-toggle-home-');
    const workDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-plan-toggle-work-');
    await writeTestConfig(homeDir);
    const harness = createSpiderByteHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('rejects manual compaction on an empty session with compaction.unable', async () => {
    const homeDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-compact-home-');
    const workDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-compact-work-');
    await writeTestConfig(homeDir);
    const harness = createSpiderByteHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('returns current session usage totals', async () => {
    const homeDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-usage-home-');
    const workDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-usage-work-');
    await writeTestConfig(homeDir);
    const harness = createSpiderByteHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('resumes a persisted session and restores runtime plan mode from wire history', async () => {
    const homeDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-resume-home-');
    const workDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-resume-work-');
    await writeTestConfig(homeDir);
    const harness = createSpiderByteHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it.todo('marks resumed plan mode active when the restored plan has no plan data', async () => {
    const homeDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-resume-legacy-plan-home-');
    const workDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-resume-legacy-plan-work-');
    await writeTestConfig(homeDir);
    const createdHarness = createSpiderByteHarness({ homeDir, identity: TEST_IDENTITY });
    let sessionId = '';
    let sessionDir = '';
    ...
    const resumedHarness = createSpiderByteHarness({ homeDir, identity: TEST_IDENTITY });
    try {
  ...
  it('forks a session, drops goal state, and returns an active fork session', async () => {
    const homeDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-fork-home-');
    const workDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-fork-work-');
    await writeTestConfig(homeDir);
    const harness = createSpiderByteHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('rejects an empty resume id', async () => {
    const homeDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-resume-empty-home-');
    const harness = createSpiderByteHarness({ homeDir, identity: TEST_IDENTITY });
