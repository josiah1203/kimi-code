/**
 * Scenario: the SpiderByte harness talks to the in-process agent-core engine
 * through the klient memory transport.
 * Responsibilities: `getExperimentalFeatures` is migrated end-to-end; every
 * unsupported methods never silently hit a legacy core.
 * Wiring: the real engine is bootstrapped on a temporary SpiderByte home; no provider calls.
 * Run: pnpm exec vitest run test/spiderbyte-sdk-client.test.ts
 */
...
async function makeHarness(): Promise<{ harness: SpiderByteHarness; homeDir: string }> {
  const homeDir = await mkdtemp(join(tmpdir(), 'spiderbyte-sdk-v2-'));
  tempDirs.push(homeDir);
  return { harness: createSpiderByteHarness({ homeDir, identity: TEST_IDENTITY }), homeDir };
}

describe('SpiderByte SDK client', () => {
  ...
  it('serves workspace skills through the local engine composition boundary', async () => {
    const { harness, homeDir } = await makeHarness();
    const workDir = await mkdtemp(join(tmpdir(), 'spiderbyte-sdk-v2-work-'));
    tempDirs.push(workDir);
    await writeSkill(join(homeDir, 'skills', 'demo-user-skill'), 'demo-user-skill');
    await writeSkill(join(workDir, '.spiderbyte', 'skills', 'demo-project-skill'), 'demo-project-skill');
    try {
  ...
  it('honors skillDirs (explicit dirs) over default user / project discovery', async () => {
    ...
    const workDir = await mkdtemp(join(tmpdir(), 'spiderbyte-sdk-v2-work-'));
    tempDirs.push(workDir);
    const explicitBase = await mkdtemp(join(tmpdir(), 'spiderbyte-sdk-v2-explicit-'));
  ...
  it('serves the plugin catalog from the v2 engine on an empty home', async () => {
    const homeDir = await mkdtemp(join(tmpdir(), 'spiderbyte-sdk-v2-'));
    tempDirs.push(homeDir);
    const rpc = new SpiderByteSdkClient({ homeDir, identity: TEST_IDENTITY });
    try {
  ...
  it('returns a stable not-found error when deleting a missing session', async () => {
    const { harness } = await makeHarness();
    try {
...
describe('SpiderByteSdkClient workspace trust', () => {
  it('reports an untrusted workspace with the project MCP servers it gates', async () => {
    ...
    await writeFile(
      join(workDir, '.spiderbyte', 'mcp.json'),
  ...
  it('trustWorkspace flips the state and persists the marker in the SpiderByte home', async () => {
    const { harness, homeDir } = await makeHarness();
    const workDir = await mkdtemp(join(tmpdir(), 'spiderbyte-sdk-v2-work-'));
    tempDirs.push(workDir);
    try {
      ...
      // The trust marker lives in the SpiderByte home, never in the checkout.
...
describe('SpiderByteSdkClient engine telemetry', () => {
  it('forwards engine-side events to the host-supplied telemetry client', async () => {
    const homeDir = await mkdtemp(join(tmpdir(), 'spiderbyte-sdk-v2-tel-'));
    tempDirs.push(homeDir);
    const workDir = await mkdtemp(join(tmpdir(), 'spiderbyte-sdk-v2-tel-work-'));
    tempDirs.push(workDir);
  ...
  it('honors telemetry = false for engine-side events', async () => {
    const homeDir = await mkdtemp(join(tmpdir(), 'spiderbyte-sdk-v2-tel-off-'));
    tempDirs.push(homeDir);
    const workDir = await mkdtemp(join(tmpdir(), 'spiderbyte-sdk-v2-tel-off-work-'));
    tempDirs.push(workDir);
