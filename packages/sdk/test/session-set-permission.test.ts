...
import { createSpiderByteHarness, type SpiderByteError, type PermissionMode } from '#/index';
...
describe('Session.setPermission', () => {
  it.each(['yolo', 'manual', 'auto'] as const)(
    ...
    async (mode: PermissionMode) => {
      const homeDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-permission-home-');
      const workDir = await makeTempDir(tempDirs, 'spiderbyte-sdk-permission-work-');
