import type { SpiderByteConfig } from '../src/types';
...
describe('fetchCatalog', () => {
  ...
  it('sends the given User-Agent, and none by default', async () => {
    ...
    await fetchCatalog(
      ...
      {
        fetchImpl: fetchMock as unknown as typeof fetch,
        userAgent: 'spiderbyte-cli/1.2.3',
      },
    );
    ...
    expect(withUaHeaders['User-Agent']).toBe('spiderbyte-cli/1.2.3');
...
describe('applyCatalogProvider', () => {
  it('writes provider, model aliases, and defaults', () => {
    const config = { providers: {} } as SpiderByteConfig;
