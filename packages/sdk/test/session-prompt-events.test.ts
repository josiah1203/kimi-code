 ...
 * Run: pnpm exec vitest run packages/sdk/test/session-prompt-events.test.ts
 */
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'spiderbyte-sdk-prompt-'));
  tempDirs.push(dir);
  return dir;
}
...
async function configureFakeProvider(harness: SpiderByteHarness): Promise<void> {
