...
const USAGE = `Usage:
  pnpm exec tsx --import ./build/register-raw-text-loader.mjs packages/sdk/examples/spiderbyte-harness-log-marker.ts --session <session-id>

Options:
  -s, --session <id>   Existing session id to resume and mark
      --home <dir>     SpiderByte home dir; defaults to SPIDERBYTE_HOME or ~/.spiderbyte
...
async function main(): Promise<void> {
  ...
  const resolvedHome = resolveSpiderByteHome(options.homeDir);
  const harness = createSpiderByteHarness({
    identity: { productName: 'spiderbyte-cli', version: 'log-marker', platform: 'spiderbyte_cli' },
  ...
  try {
    ...
    sessionLogPath = join(sessionDir, 'logs', 'spiderbyte.log');
    const payload = {
  ...
  if (!sessionMatched) {
    process.stderr.write(
      [
        ...
        'Check that SPIDERBYTE_LOG_LEVEL is not "off" and that the session id exists in this SPIDERBYTE_HOME.',
        '',
      ].join('\n'),
    );
    process.exitCode = 1;
