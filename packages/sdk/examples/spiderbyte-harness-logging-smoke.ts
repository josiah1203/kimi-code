...
import { createSpiderByteHarness, log } from '@spiderbyte/sdk';

const SESSION_LOG = 'logs/spiderbyte.log';
const GLOBAL_LOG = 'logs/global/spiderbyte.log';
const MAIN_WIRE = 'agents/main/wire.jsonl';
const TEST_HOME = join(homedir(), '.spiderbyte-test');
...
async function readSessionLogs(sessionDir: string): Promise<{
  ...
  const files = (await readdir(logsDir))
    .filter((file) => file === 'spiderbyte.log' || file.startsWith('spiderbyte.log.'))
    .toSorted();
...
async function assertRotatedFilesStayBounded(sessionDir: string, files: readonly string[]): Promise<void> {
  assert(files.includes('spiderbyte.log.1'), 'session log did not rotate after exceeding max bytes');
  assert(!files.includes('spiderbyte.log.2'), 'session log kept more archives than configured');
...
async function main(): Promise<void> {
  process.env['SPIDERBYTE_HOME'] = TEST_HOME;
  process.env['SPIDERBYTE_LOG_LEVEL'] = 'warn';
  process.env['SPIDERBYTE_LOG_SESSION_MAX_BYTES'] = String(MAX_LOG_BYTES);
  process.env['SPIDERBYTE_LOG_SESSION_FILES'] = '2';
  process.env['SPIDERBYTE_LOG_GLOBAL_MAX_BYTES'] = String(MAX_LOG_BYTES);
  process.env['SPIDERBYTE_LOG_GLOBAL_FILES'] = '2';
  ...
  const harness = createSpiderByteHarness({
    identity: { productName: 'spiderbyte-cli', version: '0.1.1', platform: 'spiderbyte_cli' },
    homeDir: TEST_HOME,
  });
  ...
  try {
    ...
    await writeFile(
      ...
      [
        ...
        'Open these files:',
        '  - logs/spiderbyte.log',
        '  - logs/spiderbyte.log.1',
        `  - ${summary.sessionDir}/logs/spiderbyte.log`,
        `  - ${summary.sessionDir}/logs/spiderbyte.log.1`,
        '',
        'Rotation evidence:',
        `  - should NOT appear in session logs: ${evictedEntry}`,
        `  - should appear in session logs: ${finalEntry}`,
        '  - spiderbyte.log.1 exists',
        '  - spiderbyte.log.2 does not exist',
        '',
        'Export evidence:',
        '  - logging-session.zip includes session logs and wire.jsonl',
        '  - logging-session.zip does not include logs/global/spiderbyte.log',
        '  - logging-with-global.zip includes only the active global log at logs/global/spiderbyte.log',
        '  - global rotated logs such as ~/.spiderbyte-test/logs/spiderbyte.log.1 are intentionally not bundled',
        '',
        'Redaction evidence:',
