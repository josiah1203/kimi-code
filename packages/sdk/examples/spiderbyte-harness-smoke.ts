...
async function main(): Promise<void> {
  const workDir = await mkdtemp(join(tmpdir(), 'spiderbyte-harness-smoke-'));
  const homeDir = await mkdtemp(join(tmpdir(), 'spiderbyte-harness-home-'));
  ...
  try {
    ...
    const model = config.defaultModel ?? 'local/example-model';
    ...
    await harness.renameSession({
      id: session.id,
      title: 'spiderbyte-harness-smoke',
    });
