import { createSpiderByteHarness, type Event, type Session } from '@spiderbyte/sdk';

import { smokeIdentityFromEnv } from './runtime-smoke-helpers';

const PROMPT =
  process.env['SPIDERBYTE_SDK_PROMPT'] ??
  'Introduce yourself in two concise sentences and mention the current working directory.';

async function main(): Promise<void> {
  const workDir = process.cwd();
  const harness = createSpiderByteHarness({ identity: smokeIdentityFromEnv() });
