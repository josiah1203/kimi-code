...
async function main(): Promise<void> {
  const homeDir = await mkdtemp(join(tmpdir(), 'spiderbyte-harness-list-home-'));
  const workDirA = await mkdtemp(join(tmpdir(), 'spiderbyte-harness-list-work-a-'));
  const workDirB = await mkdtemp(join(tmpdir(), 'spiderbyte-harness-list-work-b-'));
