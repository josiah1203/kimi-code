...
import { type SpiderByteHarness, type Session, type Event } from '@spiderbyte/sdk';

export function smokeIdentityFromEnv(): SpiderByteHostIdentity {
  const version = process.env['SPIDERBYTE_SMOKE_VERSION'];
  if (version === undefined || version.trim().length === 0) {
    throw new Error('SPIDERBYTE_SMOKE_VERSION is required for SpiderByte SDK smoke examples.');
  }
  return {
    productName: 'spiderbyte-cli',
    version,
    platform: 'spiderbyte_cli',
  };
}

export async function createConfiguredSession(harness: SpiderByteHarness): Promise<Session> {
