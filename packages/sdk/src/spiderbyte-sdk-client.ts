/**
 * Canonical SpiderByte SDK client backed by the agent-core engine
 * (DI × Scope) instead of the v1 `SpiderByteCore` RPC pair. The engine is
 ...
 *
 * The base class remains an internal transport contract while the public
 * factory below is backed exclusively by the canonical engine. Unsupported
 * optional capabilities use stable coded errors; no request falls through to
 * a legacy engine.
 *
 * Migrated so far:
 * - `getExperimentalFeatures` → `klient.global.flags.list()`
 * - `listWorkspaceSkills` → not covered by the klient facade, so the client
 *   composes the workspace handler's `IWorkspaceSkillCatalog` internally.
 * - `getConfig` / `setConfig` / `removeProvider` / `getConfigDiagnostics` →
 *   `klient.global.config.*`, with the v1 `SpiderByteConfig` shape restored by the
 ...
 *   the client's internal app-scope service accessor where it does not (explicit session ids, resume,
 ...
 *   MCP surface, rebuilt in `src/v2/global-mcp.ts`: SpiderByte Agent Core only reads
 ...
 * - `exportSession` → `ISessionExportService` (app scope, the canonical
 *   export service) through the client's internal app-scope service accessor; `listSkills` → the session
 ...
 *   The canonical client uses the engine-owned host filesystem/process
 *   boundary; caller-supplied filesystem/process injection and custom-tool
 *   callbacks are not part of this public factory.
...
import { MCP_SECTION, type McpSection } from '@spiderbyte/agent-core/app/mcpConfig/configSection';
import { IAgentIdentity } from '@spiderbyte/agent-core/app/agentIdentity/agentIdentity';
import { McpConnectionManager } from '@spiderbyte/agent-core/mcpCore/connection-manager';
import {
  AlreadyAuthorizedError,
  McpOAuthService,
  type BeginAuthorizationResult,
} from '@spiderbyte/agent-core/mcpCore/oauth/service';
import { createMcpOAuthStore } from '@spiderbyte/agent-core/app/mcpConfig/oauthStore';
import { SECONDARY_MODEL_SECTION } from '@spiderbyte/agent-core/app/kosongConfig/configSection';
import { IAtomicDocumentStore } from '@spiderbyte/agent-core/persistence/interface/atomicDocumentStore';
import { wrapSubagentModelError } from '@spiderbyte/agent-core/session/subagent/configSection';
import { loadMcpServers } from '@spiderbyte/agent-core/workspace/workspaceMcpConfig/internal/config-loader';
import {
...
import {
  ...
  planProviderRemoval,
  resolvedConfigToSpiderByteConfig,
...
import { foldAgentWireReplay, limitAgentReplayByTurns } from '#/v2/resume-replay';
import {
...
export interface SpiderByteSdkClientOptions {
...
/** Identity used by accountless SDK hosts that do not embed their own product metadata. */
const DEFAULT_SPIDERBYTE_SDK_IDENTITY: SpiderByteHostIdentity = {
  productName: 'spiderbyte-sdk',
  version: process.env['SPIDERBYTE_SDK_VERSION'] ?? 'development',
  platform: 'spiderbyte_sdk',
};
...
export class SpiderByteSdkClient extends SDKRpcClientBase {
  readonly homeDir: string;
  readonly configPath: string;
  readonly identity: SpiderByteHostIdentity;
  readonly telemetry: TelemetryClient;
   ...
   * `src/v2/global-mcp.ts`, because SpiderByte Agent Core only reads that file and
  ...
  constructor(options: SpiderByteSdkClientOptions = {}) {
    super();
    this.identity = assertSpiderByteHostIdentity(options.identity ?? DEFAULT_SPIDERBYTE_SDK_IDENTITY);
    this.homeDir = resolveSpiderByteHome(options.homeDir);
    ...
    const { app } = bootstrap(
      {
        ...
        args: {
          ...
          requestHeaders: createSpiderByteDefaultHeaders({ homeDir: this.homeDir, ...identity }),
  ...
  async ensureConfigFile(): Promise<void> {
    ...
    await writeFile(this.configPath, '', { flag: 'a', mode: 0o600 });
  }
   ...
   * client only served `SpiderByteHarness`-level events and every engine-side event
  ...
  private installEngineTelemetry(client: TelemetryClient | undefined): void {
    ...
    void this.configReady.then(() => {
      telemetry.setEnabled(this.appServices.get(IConfigService).get('telemetry') !== false);
    });
  }

  /** Internal composition point for capabilities owned by the local engine. */
  private get appServices(): ServicesAccessor {
  ...
  protected getRpc(): Promise<never> {
    throw new SpiderByteError(
      ...
      'This SDK method is not wired to SpiderByte Agent Core yet.',
    );
  }
  ...
  override async listWorkspaceSkills(workDir: string): Promise<readonly SkillSummary[]> {
    const handler = await this.appServices
   ...
   * via {@link appServices} — the same `handlerFor({ root })` path
  ...
  override async getWorkspaceTrustInfo(workDir: string): Promise<WorkspaceTrustInfo> {
    ...
    try {
      const fs = this.appServices.get(IHostFileSystem);
  ...
  /**
   * v1 returns the whole config.toml document as one `SpiderByteConfig`; v2
   ...
   * runtime config (`loadRuntimeConfigSafe` + the SPIDERBYTE_MODEL_* overlay);
   * `reload` mirrors v1's re-read-from-disk option.
   */
  override async getConfig(options?: GetConfigOptions): Promise<SpiderByteConfig> {
   ...
   * the same single-write shape as v1's `removeProvider`, so a process
   * exit can never leave the file in a halfway-cascaded state.
   */
  override async removeProvider(providerId: string): Promise<SpiderByteConfig> {
    await this.configReady;
  ...
  // Session lifecycle
  //
  // The engine splits what the old in-memory session map did
  ...
  // `appServices` escape hatch (named per method below).
  //
  // Caller-supplied filesystem/process injection is intentionally not part of
  // the supported client contract. The engine owns its host boundary at
  // bootstrap, so there is no silent fallback path for injected adapters.
  // -----------------------------------------------------------------------

  private liveSession(sessionId: string): ISessionScopeHandle | undefined {
    return getLiveSessionById(this.appServices, sessionId);
  }

  /** v1's `requireSession` / store lookup failure shape. */
  private static sessionNotFound(sessionId: string): SpiderByteError {
    return new SpiderByteError(ErrorCodes.SESSION_NOT_FOUND, `Session "${sessionId}" was not found`, {
      details: { sessionId },
    });
  }
  ...
  private requireLiveSession(sessionId: string): ISessionScopeHandle {
    ...
    if (handle === undefined) throw SpiderByteSdkClient.sessionNotFound(sessionId);
    return handle;
  }
   ...
   * full runtime `ProviderConfig` into the snapshot, SpiderByte Agent Core has no
  ...
  private async workspaceIdsFor(workDir: string): Promise<readonly string[]> {
    ...
    return this.appServices.get(IWorkspaceAliases).resolveAliasIds(match.id);
  }
  ...
  override async listSessions(input: ListSessionsOptions = {}): Promise<readonly SessionSummary[]> {
    ...
    const bootstrapService = this.appServices.get(IBootstrapService);
    ...
    for (const item of page.items) {
      ...
      const liveHandle = getLiveSessionById(this.appServices, item.id);
      const effectiveItem =
  ...
  override async createSession(input: CreateSessionOptions): Promise<SessionSummary> {
    ...
    if (input.id !== undefined) {
      const existing =
        ...
        (await this.appServices.get(ISessionIndex).get(input.id));
  ...
  override async renameSession(input: RenameSessionInput): Promise<void> {
    ...
    if (title.length === 0) {
      throw new SpiderByteError(ErrorCodes.SESSION_TITLE_EMPTY, 'Session title cannot be empty');
    }
    if (this.liveSession(input.id) !== undefined) {
      await this.klient.session(input.id).setTitle(title);
      return;
    }
    const handle = await resumeSessionById(this.appServices, input.id);
    if (handle === undefined) throw SpiderByteSdkClient.sessionNotFound(input.id);
    try {
      await this.klient.session(input.id).setTitle(title);
    } finally {
      await closeSessionById(this.appServices, input.id);
    }
  }

  /**
   * Through `appServices` (the handler chain's
   * `ISessionLifecycleService.fork`) because the klient facade fork takes no
   * explicit target id. The lifecycle service performs journal truncation
  ...
  override async deleteSession(input: SessionIdRpcInput): Promise<void> {
    this.printSteerStates.delete(input.sessionId);
    const handler = await handlerForSession(this.appServices, input.sessionId);
    if (handler === undefined) throw SpiderByteSdkClient.sessionNotFound(input.sessionId);
    await handler.accessor.get(ISessionLifecycleService).delete(input.sessionId);
    this.unwireSession(input.sessionId);
  }

  /**
   * Materializes the session through `appServices`
  ...
  override async reloadSession(input: ReloadSessionRpcInput): Promise<ResumedSessionSummary> {
    ...
    } else if ((await this.appServices.get(ISessionIndex).get(sessionId)) === undefined) {
      throw SpiderByteSdkClient.sessionNotFound(sessionId);
    }
    await this.configReady;
    await this.klient.global.config.reload();
    await this.klient.global.plugins.reload();
    if (live !== undefined) {
      await closeSessionById(this.appServices, sessionId);
    }
    ...
    const handle = await resumeSessionById(this.appServices, sessionId);
    if (handle === undefined) throw SpiderByteSdkClient.sessionNotFound(sessionId);
    this.wireSession(handle);
    return this.resumedSessionSummary(handle);
  }
   ...
   * project-local `.spiderbyte/local.toml`, `persist: false` joins the
  ...
  /**
   * Through `appServices` (`ISessionExportService`, app scope) — the v2
  ...
  override async exportSession(input: ExportSessionInput): Promise<ExportSessionResult> {
    return this.appServices.get(ISessionExportService).export({
      sessionId: input.id,
  ...
  private async agentScope(sessionId: string): Promise<IAgentScopeHandle> {
    ...
    if (agent === undefined) {
      throw new SpiderByteError(ErrorCodes.AGENT_NOT_FOUND, `Agent "${agentId}" was not found`);
    }
    return agent;
  }
  ...
  override async applyPersistedSecondaryModel(input: SessionIdRpcInput): Promise<void> {
    ...
    await this.configReady;
    await this.modelReady;
    const secondary = this.appServices
      .get(IConfigService)
    ...
    try {
      this.appServices.get(IModelCatalog).get(secondary.model);
    } catch (error) {
   ...
   * SpiderByte Agent Core builds this message), so the SDK composes v1's exact
  ...
  override async getSessionWarnings(input: SessionIdRpcInput) {
    ...
    if (warning === undefined) {
      ...
      const prepared = await prepareSystemPromptContext(
        {
          fs: this.appServices.get(IHostFileSystem),
          homeDir: this.appServices.get(IHostEnvironment).homeDir,
        },
        session.accessor.get(ISessionContext).cwd,
        this.appServices.get(IBootstrapService).homeDir,
    ...
    await applyPromptMetadataUpdate(
      {
        ...
        eventService: this.appServices.get(IEventService),
        sessionId,
      },
      text,
    );
   ...
   * re-implements the same drain inline in run-print). Same gate (drain
  ...
  // connection manager (SpiderByte Agent Core has no app-scope MCP config service —
  ...
  private resolveMcpClientName(): string | undefined {
    return this.appServices.get(IAgentIdentity).current().slug;
  }
  ...
  override async completeGlobalMcpServerAuth(
    ...
    if (active === undefined) {
      throw new SpiderByteError(ErrorCodes.REQUEST_INVALID, `Unknown MCP OAuth flow: ${input.flowId}`);
    }
    try {
  ...
  private async withGlobalMcpServerProbe<T>(
    ...
    await this.configReady;
    const section = this.appServices.get(IConfigService).get<McpSection | undefined>(MCP_SECTION);
...
export function createSpiderByteHarness(options: SpiderByteHarnessOptions): SpiderByteHarness {
  const rpc = new SpiderByteSdkClient(options);
...
function normalizeRequiredWorkDir(operation: string, workDir: string): string {
  if (typeof workDir !== 'string' || workDir.trim() === '') {
    throw new SpiderByteError(ErrorCodes.REQUEST_WORK_DIR_REQUIRED, `${operation} requires workDir`);
  }
