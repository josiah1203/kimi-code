...
import { ErrorCodes, SpiderByteError, makeSpiderByteErrorPayload } from '#/errors';
import type {
  ApprovalRequest,
  ApprovalResponse,
  Event,
...
export abstract class SDKRpcClientBase {
  ...
  async getConfig(input?: GetConfigOptions): Promise<SpiderByteConfig> {
    const rpc = await this.getRpc();
    return rpc.getSpiderByteConfig(input ?? {});
  }
  ...
  async setConfig(input: SpiderByteConfigPatch): Promise<SpiderByteConfig> {
    const rpc = await this.getRpc();
    return rpc.setSpiderByteConfig(input);
  }
  ...
  async listGlobalMcpServers(): Promise<readonly GlobalMcpServerConfig[]> {
    const rpc = await this.getRpc();
  ...
  async addGlobalMcpServer(server: GlobalMcpServerConfig): Promise<readonly GlobalMcpServerConfig[]> {
    const rpc = await this.getRpc();
    return rpc.addGlobalMcpServer({ server });
  }

  async updateGlobalMcpServer(server: GlobalMcpServerConfig): Promise<readonly GlobalMcpServerConfig[]> {
    const rpc = await this.getRpc();
    return rpc.updateGlobalMcpServer({ server });
  }

  async removeGlobalMcpServer(name: string): Promise<readonly GlobalMcpServerConfig[]> {
    const rpc = await this.getRpc();
  ...
  async getPlatformModelSelection(
    ...
    throw new SpiderByteError(
      ErrorCodes.NOT_IMPLEMENTED,
      'Platform model selection requires the SpiderByte Agent Core engine.',
    );
  }
   ...
   * contributed-command seam exists only in the SpiderByte Agent Core engine, so the
  ...
  async requestApproval(
    ...
    try {
      ...
      this.receiveEvent({
        ...
        ...makeSpiderByteErrorPayload(ErrorCodes.SESSION_APPROVAL_HANDLER_ERROR, errorMessage(error)),
      } as Event);
      return {
        decision: 'cancelled',
  ...
  async requestQuestion(
    ...
    try {
      ...
      this.receiveEvent({
        ...
        sessionId: request.sessionId,
        agentId: request.agentId,
        ...makeSpiderByteErrorPayload(ErrorCodes.SESSION_QUESTION_HANDLER_ERROR, errorMessage(error)),
      } as Event);
      return null;
    }
  }
