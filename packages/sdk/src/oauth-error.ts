...
/**
 * Classify an OAuth token-fetch failure into the public {@link SpiderByteError}
 ...
 * remediation path.
 */
export function mapOAuthTokenError(error: unknown, providerName: string): SpiderByteError | undefined {
  if (error instanceof OAuthUnauthorizedError) {
    return new SpiderByteError(
