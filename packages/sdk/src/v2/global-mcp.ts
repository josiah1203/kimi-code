/**
 * The v1 user-global MCP surface (`<SPIDERBYTE_HOME>/mcp.json` CRUD plus the
 * standalone connection probe), rebuilt for the v2 client.
 *
 * Why a replica exists: SpiderByte Agent Core only READS the user-global file (its
 ...
 * (`spiderbyte-sdk-client.ts`); the v2 credential store persists to the same
...
import { McpServerConfigSchema } from '@spiderbyte/agent-core/mcpCore/config-schema';
import type { McpConnectionManager } from '@spiderbyte/agent-core/mcpCore/connection-manager';
import { atomicWrite } from '@spiderbyte/agent-core/_base/utils/fs';

import type { GlobalMcpServerConfig, McpRemoteServerConfig, McpServerConfig, McpTestResult } from '#/types';
...
function normalizeServerName(name: string): string {
  ...
  throw new SpiderByteError(ErrorCodes.REQUEST_INVALID, 'MCP server name cannot be empty');
}

function serverNotFound(name: string): SpiderByteError {
  return new SpiderByteError(ErrorCodes.MCP_SERVER_NOT_FOUND, `MCP server "${name}" was not found`);
}

function configError(message: string, cause?: unknown): SpiderByteError {
  return new SpiderByteError(ErrorCodes.CONFIG_INVALID, message, { cause });
}
