 ...
 * own (nothing under `SpiderByte Agent Core` builds this message), but all of its
...
import { estimateTokensForMessages } from '@spiderbyte/agent-core/kosong/contract/tokens';
...
export function buildImportContextMessage(content: string, source: string): ContextMessage {
  if (content.trim().length === 0) {
    throw new SpiderByteError(ErrorCodes.REQUEST_INVALID, 'Imported context cannot be empty', {
      details: { reason: 'import_content_empty' },
    });
  }
  const normalizedSource = source.trim();
  if (normalizedSource.length === 0) {
    throw new SpiderByteError(ErrorCodes.REQUEST_INVALID, 'Imported context source cannot be empty', {
