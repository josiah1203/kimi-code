 ...
 * `proper-lockfile` are bypassed via the `SPIDERBYTE_DISABLE_OAUTH_LOCK=1`
...
const WORKER_SCRIPT = `
  ...
  import { join } from 'node:path';
  const { OAuthManager } = await import(process.env.SPIDERBYTE_OAUTH_ENTRY);

  const shareDir = process.env.SPIDERBYTE_HOME;
  ...
  async function waitForFirstLoadBarrier() {
    if (process.env.SPIDERBYTE_SYNC_FIRST_LOAD !== '1') return;
    await appendFile(readyPath, '.');
    const expected = Number(process.env.SPIDERBYTE_WORKER_COUNT || '1');
  ...
  if (process.env.DEBUG_OAUTH_WORKER === '1') {
    process.stderr.write('[worker ' + process.env.SPIDERBYTE_WORKER_ID + '] done\\n');
  }
`;
...
describe.skipIf(skipOnWindows)('OAuthManager cross-process refresh lock', () => {
  it('2 workers concurrently force-refresh → exactly one refreshImpl fires', async () => {
    ...
    const workers = await spawnInlineWorkers({
      ...
      timeoutMs: 30_000,
      env: {
        SPIDERBYTE_OAUTH_ENTRY: OAUTH_ENTRY_URL,
        SPIDERBYTE_SYNC_FIRST_LOAD: '1',
        SPIDERBYTE_WORKER_COUNT: '2',
      },
    });
...
describe.skipIf(!skipOnWindows)('OAuthManager cross-process refresh lock (Windows skip)', () => {
  it('skipped on Windows — covered by SPIDERBYTE_DISABLE_OAUTH_LOCK=1 env escape hatch', () => {
